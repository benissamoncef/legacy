/*E*/
/*Fichier : $Id: etdc_cal.c,v 1.10 2011/02/25 16:01:42 gesconf Exp $      Release : $Revision: 1.10 $        Date : $Date: 2011/02/25 16:01:42 $
------------------------------------------------------
* GTIE *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME : EQUEXT
------------------------------------------------------
* MODULE TETDC * FICHIER etdc_cal.c
------------------------------------------------------
* DESCRIPTION DU MODULE :
*
*  < Module de gestion de pile de la tache TETDC >
*
------------------------------------------------------
* HISTORIQUE :
*
...
* Volcic.F	version 1.5	27 Jun 1995	: Ajout prise en compte des variables des equations d'alerte
* JMG		version 1.8	22/11/07	: ajout site de gestion DEM/715
* JPL		version 1.9	03/10/09	: Trace cas d'erreur fonction et_lire_txt (pour DEM 909)
* JPL		25/02/11 : Migration architecture HP ia64 (DEM 975) : suppression warnings mineurs  1.10
------------------------------------------------------*/
...
static char *version = "$Id: etdc_cal.c,v 1.10 2011/02/25 16:01:42 gesconf Exp $ : etdc_cal";
...
int et_lire_txt (FILE *,
                   XDY_TexteAlerte);

int et_lire_arg(FILE *,
                  ETDCT_VAR *,
                  int,
                  char * ,
                  double *,
...
int et_evaluer (   FILE *          pa_file_in,
                   ETDCT_VAR *     pa_variables_in,
...
* ETDCT_VAR *     pa_variables_in      tableau des variables
...
{
   static char     *version = "$Id: etdc_cal.c,v 1.10 2011/02/25 16:01:42 gesconf Exp $ : et_evaluer";
   XDY_TexteAlerte  pl_texte="";
   char             vl_oper;
   double           vl_valeur;
   int              vl_indice;
   int              vl_pos_pile;
   double           vl_pile[ETDCC_pile];
   double           vl_registre[ETDCC_registre];
   int		    vl_fin;
   ...
   while (et_lire_txt(pa_file_in,pl_texte)==XDC_OK)
   {
     ...
     while (vl_fin==0)
     {
      ...
      if (et_lire_arg(pa_file_in,pa_variables_in,va_nb_variables_in,&vl_oper,&vl_valeur,&vl_indice)!=XDC_OK)
      {
      ...
      switch (vl_oper)
      {
         ...
         case '!' : if (vl_pos_pile<0) /* duplique sommet de la pile */
                    ...
                    else if (vl_pos_pile>=(ETDCC_pile-1))
                    {
         ...
         case 'S' : if (vl_pos_pile<0) /* stocke sommet de la pile dans registre et depile */
                    ...
                    else if ((vl_indice<0)||(vl_indice>=ETDCC_registre))
                    {
         ...
         case 'R' : if (vl_pos_pile>=(ETDCC_pile-1)) /* restaure le registre au sommet de la pile */
                    {
         ...
         case 'V' : if (vl_pos_pile>=(ETDCC_pile-1)) /* met la valeur au sommet de la pile */
                    {
         ...
         case '$' : if (vl_pos_pile!=0) /* fin d'equation : envoie alerte si pile!= 0 */
                    ...
                    else if ((vl_pile[0]!=0)&&(va_Test_in==XDC_NON))
                    {
                       if (ex_env_alerte ( va_Horodate_in, XDC_EQT_RAD, va_Eqt_in, XZAAC_EQA_RADT, vg_Mode_Fonct,
							XZSC_22NumeroSiteLocal(), pl_texte, NULL )!=XDC_OK)
                       {
...
int et_lire_txt (FILE *          pa_file_in,
...
------------------------------------------------------*/
{
   static char     *version = "$Id: etdc_cal.c,v 1.10 2011/02/25 16:01:42 gesconf Exp $ : et_lire_txt";
   char pl_texte[1001];
   char *pl_msg_trace;
   char *pl_cr;
   ...
   *pa_texte_out='\0';
   ...
   do
   {
      do {
         errno = 0;
         pl_cr = fgets ( pl_texte, sizeof(pl_texte), pa_file_in );
      } while ( ( pl_cr == NULL )  &&  ( errno == EINTR ) );

      if ( pl_cr == NULL )
      {
         if ( feof ( pa_file_in ) )
         {
            pl_msg_trace = "fin de fichier";
         }
         else
         {
            pl_msg_trace = "erreur de lecture";
            XZST_03EcritureTrace (XZSTC_WARNING, "%s : Erreur a la lecture du fichier, errno=%d", version, errno);
         }

         XZST_03EcritureTrace (XZSTC_FONCTION ,"OUT : %s retourne NOK (%s)", version, pl_msg_trace);
         return XDC_NOK;
      }
...
int et_lire_arg(  FILE *          pa_file_in,
                  ETDCT_VAR *     pa_variables_in,
                  int             va_nb_variables_in,
                  char *          pa_oper_out,
...
------------------------------------------------------*/
{
   static char     *version = "$Id: etdc_cal.c,v 1.10 2011/02/25 16:01:42 gesconf Exp $ : et_lire_arg";
   char pl_texte[1001];
   char *pl_mot;
   int i;
