/*E*/
/*Fichier :  $Id: ewst_dir.c,v 1.6 2020/10/02 14:47:53 pc2dpdy Exp $      Release : $Revision: 1.6 $        Date : $Date: 2020/10/02 14:47:53 $ ------------------------------------------------------
------------------------------------------------------
* GTIE *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME : EQUEXT
------------------------------------------------------
* MODULE TESWT * FICHIER ewst_dir.c
------------------------------------------------------
...
char	**argv;
{
	...
	*/
	...
	{
	   /* Ecriture Trace */
           XZST_03EcritureTrace( XZSTC_FATAL, "tewst_dir Recuperation du Nom du site impossible . " );
	} 

	/* abonnement data group */
	if (! XDG_JoindreDG(XDG_API_GW_TEL) ) //Pour le message vers la gateway
	{
		/* Ecriture Trace */
		XZST_03EcritureTrace( XZSTC_WARNING,"tewst_dir : Abonnement sur %s non effectue. \n " ,XDG_API_GW_TEL);
		return;
	}
	else
	{
		XZST_03EcritureTrace( XZSTC_INFO,"tewst_dir : Abonnement sur %s effectue. \n " ,XDG_API_GW_TEL);
	}

	if (! XDG_JoindreDG(XDG_ECMD_AUTOTEL) ) //Pour le message depuis l'ihm
	{
		/* Ecriture Trace */
		XZST_03EcritureTrace( XZSTC_WARNING,"tewst_dir : Abonnement sur %s non effectue. \n " ,XDG_ECMD_AUTOTEL);
		return;
    ...
    */
    if (TipcSrvProcessCbCreate(XDM_IdentMsg(XDM_ECMD_AUTOTEL2),
								(T_IPC_CONN_PROCESS_CB_FUNC) cb_tewst_appel_tel,
								(T_CB_ARG) NULL) == NULL)
    {
        XZST_03EcritureTrace(XZSTC_FATAL,"tewst_dir : probleme lors de la creation du callback de reception de XDM_ECMD_AUTOTEL2");
        exit (1);
    }	

    if (TipcSrvProcessCbCreate(XDM_IdentMsg(XDM_GTW_TEL_RETOUR_APPEL),
								(T_IPC_CONN_PROCESS_CB_FUNC) cb_tewst_retour_appel,
								(T_CB_ARG) NULL) == NULL)
    {
        XZST_03EcritureTrace(XZSTC_FATAL,"tewst_dir : probleme lors de la creation du callback de reception de XDM_ECMD_AUTOTEL2");
        exit (1);
