/*E*/
/* Fichier : $Id: ereg_trace.c,v 1.1 2012/07/04 14:23:05 pc2dpdy Exp $      Release : $Revision: 1.1 $        Date : $Date: 2012/07/04 14:23:05 $
------------------------------------------------------
* GTIE *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME : EQUEXT
------------------------------------------------------
* MODULE TEREG * FICHIER ereg_trace.c
------------------------------------------------------
				...
				char* Texte, ...  )
{
	...
	XZAVT_ConfTraceReg 	vl_confTrace;
	int 				vl_JourSemaine;
	...
	vl_confTrace.Condition = va_condition;
	vl_confTrace.Scenario = va_scenario;
	vl_confTrace.Zone = va_zone;
	vl_confTrace.Simulation = vg_ModeSimu;
	if (!vg_ModeSimu)
		vl_confTrace.Horodate = vl_horodate;
	else
		vl_confTrace.Horodate = vg_HorodateSimu;
	vl_confTrace.Alerte = va_alerte;
	vl_confTrace.station = va_station;
	vl_confTrace.sens = va_sens_station;

	XZST_03EcritureTrace( XZSTC_DEBUG1, "trace_process : recomposition du message \"%s\"", Texte);
	...
	XZST_03EcritureTrace( XZSTC_DEBUG1, "trace_process : Appel a XZAV01_Tracer_Regulation avec base=%s, modesimu=%d, type=%d, horodate=%f, message=\"%s\"",
									vg_Base,
									vl_confTrace.Simulation,
									vl_confTrace.Type,
									vl_confTrace.Horodate,
									vl_confTrace.Libelle);

    if (XZAV01_Tracer_Regulation(vg_Base, vl_confTrace) == XDC_NOK){
		XZST_03EcritureTrace( XZSTC_WARNING, "trace_process : Erreur lors de l'appel a XZAV01_Tracer_Regulation");
		return XDC_NOK;
	}
