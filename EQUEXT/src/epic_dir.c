...
* SOUS-SYSTEME : EQUEXT
------------------------------------------------------
* MODULE TEPMV * FICHIER epic_dir.c
------------------------------------------------------
...
static tm_callback pm_TabCB[] =
{
   ...
   { XDM_ECMD_PIC, 		(void (*)())ep_cmd_PIC_cb },
   { XDM_EInit_IHM_PIC, 	(void (*)())ep_Init_IHM_PIC_cb },
...
char	**argv;
{
	...
	*/
	...
	if ( ex_cnx_client ( 	XZEXC_ADR_SOCK_TEPIC,
	 ...
	 */
	if ( ex_cnx_client ( 	XZEXC_ADR_SOCK_PICTIM,
	...
	if(XZEA29_Abt_Etat_JourNuit(XZEAC_DEBUT_ABONNEMENT,pl_NomDG,ep_journuit,vg_NomSite,&vl_resultat) != XDC_OK)
      	   XZST_03EcritureTrace(XZSTC_WARNING,"TEPIC : Impossible de s'abonner a JourNuit");
	...
	/* Messages de commande PMV : XDG_ECMD_PIC<NomMachine> */
	XDG_EncodeDG2 ( pl_NomDG, XDG_ECMD_PIC, vg_NomMachine) ;
	XZST_03EcritureTrace( XZSTC_INFO, "  Messages de commande PIC ");
   	/* joint ce datagroup */
	...
	/* Messages de commande PIC : XDG_EInit_IHM_PIC<NomSite> */
	XDG_EncodeDG2 ( pl_NomDG, XDG_EInit_IHM_PIC, vg_NomSite) ;
  ...
  XZSC_31AbtKeepAlive(vg_NomSite);
	...
	ETIM_REVEIL_CYCLIQUE ( EPICC_REVEIL_1MN * vl_CoefTimer, EPICC_TIMER_1MN, vg_SockTIM, pl_MsgTimer )

	/* Programation timer 10s */
	ETIM_REVEIL_CYCLIQUE ( EPICC_REVEIL_10S * vl_CoefTimer*2, EPICC_TIMER_10S, vg_SockTIM, pl_MsgTimer )
	
	/* Programation de l'heure de la synchro date */
	XZST_03EcritureTrace(XZSTC_WARNING, "Programation Heure ");
	ETIM_SYNCHRO_DATE ( XZEXC_HEURE_SYNCHRO, "PIC_Timer_Synchro", vg_SockTIM, pl_MsgTimer )
