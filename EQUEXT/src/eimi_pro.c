/*E*/
/* Fichier : $Id: eimi_pro.c,v 1.8 2021/05/04 13:25:03 pc2dpdy Exp $        Release : $Revision: 1.8 $        Date : $Date: 2021/05/04 13:25:03 $
------------------------------------------------------
...
* jmg 18/02/21 : DDM pour gabeb / DAI 1.12
* jmg 15/11/21 : ne plus mettre un MSG_ACQUITT dans la liste des cmd en attente 1.13
------------------------------------------------------*/
...
int vg_RepTEzzz;
static char *version = "$Id: eimi_pro.c,v 1.8 2021/05/04 13:25:03 pc2dpdy Exp $      : eimi_pro" ;
...
void epro_envoi_trame_LCR_IP(  XZEXT_Cmde_LCR_IP *pa_CmdeLCR , XDY_Booleen va_insert)
{
    static char *version = "$Id: eimi_pro.c,v 1.8 2021/05/04 13:25:03 pc2dpdy Exp $      : epro_envoi_trame_LCR " ;
 ...
 /*recherche de la bonne socket avec la bonne adresse IP*/
    ...
    if (pl_ListSock==NULL) 
    {
        ...
        if (vl_config.protocole == XZECC_PROT_LCR_E)
        {
            vg_presence_esclave = XDC_VRAI;
            vg_ConfigE = vl_config;
        }
                                ...
                                &vg_DesSocks) != XDC_OK)
        {
            ...
            envoyer_trame_dai_ko(0,vl_config.adresseIP);
...
void epro_traitement_msg_donn( XZEXT_Msg_Socket *pa_MsgSock )
{
    static char *version = "$Id: eimi_pro.c,v 1.8 2021/05/04 13:25:03 pc2dpdy Exp $      : epro_traitement_msg_donn";
...
void epro_envoi_trame_DIASER(  XZEXT_Cmde_DIASER *pa_Cmde )
{
    ...
    */
	...
	if (ecom_init_IP(&vl_config,
		...
		envoyer_trame_dai_ko(0,pa_Cmde->adresseIP);
   }
 ...
 XZEXT_MASK          *pl_ListSock = NULL;
   ...
   /*recuperer l eqt pour lequel on a passe la commande*/
    ...
    vl_ResVerif = ex_verif_TEDI_CRC( pa_MsgSock, vl_DDM, &vl_NoBloc ) ;
      if ((vl_ResVerif==XZEXC_TEDI_NOK) && (va_esclave==XDC_VRAI)) {
      sprintf( vl_DDM, "%c000", XZEXC_CAR_ENQ );
    		vl_ResVerif = ex_verif_TEDI_CRC( pa_MsgSock, vl_DDM, &vl_NoBloc ) ;
      }
	...
	if (vl_indice==C_MAX_TAMPON)
	strcpy(vm_tampon,pa_MsgSock->msg);
	else 
    {
	...
	if (va_esclave==XDC_VRAI) {
		...
		sprintf( vl_CmdeLCR.ack, "%c%c", XZEXC_CAR_ACK, vl_NoBloc );
			...
			epro_envoi_trame_LCR_IP( &vl_CmdeLCR, FALSE ); /* jM 15/11 TRUE devient FALSE */
	return;		
      }
...
*/
	...
	ecom_delete_cmd_LCR_IP(vl_adresse, vl_port, &pg_ListeCmdesLCR);
 ...
 XZEXT_MASK          *pl_ListSock = NULL;
  ...
  sprintf( vl_MsgSock.msg, "rep. teimi %s %d %d %d %s %d %d %s %s %s",
    ...
    if( write( pl_ListSock->desc_sock, vl_MsgSock.msg, vl_MsgSock.len ) != vl_MsgSock.len )
    {
      ...
      XZST_03EcritureTrace( XZSTC_WARNING, "envoyer_trame_dai_ko : Emission message <%s> ",
			    vl_MsgSock.msg );
    sem_post(&vg_semaphore); 
