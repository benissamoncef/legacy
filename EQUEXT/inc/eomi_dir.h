/*E Fichier : $Id: eomi_dir.h,v 1.3 1995/05/22 13:15:23 volcic Exp $      Release : $Revision: 1.3 $        Date : $Date: 1995/05/22 13:15:23 $
------------------------------------------------------
* GTIE *  PROJET MIGRAZUR
------------------------------------------------------
* SOUS-SYSTEME EQUEXT
------------------------------------------------------
* MODULE TEOMi * FICHIER eomi_dir.h
------------------------------------------------------
* DESCRIPTION DU MODULE :
*
* Definition des variables et fonctions du fichier directeur
*
------------------------------------------------------
* HISTORIQUE :
*
* Nouaille M.	Version 1.1	02 Dec 1994	: Creation
* Fontaine C.   version 1.2     04 Mai 1995     : Reprise entete
* D.Mismer	Version 1.3	22 Mai 1995	: Modif gestion deconnexion ST
------------------------------------------------------ */

#ifndef eomi_dir
#define eomi_dir

/* fichiers inclus */

#include "eomi_common.h"
#include "ex_msok.h"

/* definitions de constantes exportees */

/* definitions de types exportes */

/* definition de macro exportees */

/* declarations de donnees globales */

XZEXT_MASK		*vg_SockList 	= (XZEXT_MASK*)NULL;	/* liste globale des sockets */
XZEXT_ST_Config		vg_Config  ;	/* struct config tache */	
XZEXT_ST_Sockets	vg_DesSocks  ;	/* struct. descs. sockets typiques tache TELxi */
XZEXT_Liste_Cmdes_LCR	*pg_ListeCmdesLCR = (XZEXT_Liste_Cmdes_LCR *)NULL; /* liste cmdes LCR */
XDY_Entier       	vg_sock_TEOPE = -1;			/* socket acces TEOPE */
int			vg_EtatST     = XDC_OK;

float	vg_val_timer = 30. ; /* maxi timer d'attente */

/* declaration de fonctions externes */

         
/*X*/
/*------------------------------------------------------
* SERVICE RENDU : 
*  Cette fonction effectue la lecture du fichier XDF_Config_Lignes_<NomMachine>.
*
------------------------------------------------------
* SEQUENCE D'APPEL :
*/

extern int ex_LireLigneFichi( 	int    /* va_Fd */,
				char * /* pa_LigneFich */,
				int    /* va_NbCar */);

/*
* ARGUMENTS EN ENTREE :
*   va_Fd		: Descripteur fichier lecture
*   va_NbCar		: Nombre de cars. lus
*
* ARGUMENTS EN SORTIE :
*   pa_LignFich		: Contenu de la ligne lue
* 
* CODE RETOUR : 
*   XDC_OK	succes
*   XDC_NOK	echec
*
* CONDITION D'UTILISATION
*   
*
* FONCTION 
*   
*
------------------------------------------------------*/

#endif
