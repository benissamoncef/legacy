...
axis2_char_t *
tewst_get_password (const axutil_env_t *env,
...
axis2_status_t
tewst_soap_out (const axutil_env_t *env,
...
axis2_status_t
tewst_set_attachment (const axutil_env_t *env,
						 axiom_node_t *node,
						 ...
						 int is_mtom_enabled)
{
	...
	if (axiom_node_get_node_type (node, env) == AXIOM_ELEMENT)
	{
		...
		while (child_node)
		{
			tewst_set_attachment (env, child_node, base_dir, is_mtom_enabled); 
...
axis2_status_t
tewst_get_attachment (const axutil_env_t *env,
						 axiom_node_t *node,
						 axis2_char_t *base_dir,
						 int is_mtom_enabled)
{
		...
		if (local_name)
		{
			if (!strcmp (local_name, "Include"))
			{
					...
					&& (!strcmp (ns_uri, "http://www.w3.org/2004/08/xop/include")))
				{
					...
					relative = tewst_get_relative_path (env, pwd, base_dir);
...
axis2_char_t *
tewst_get_relative_path (const axutil_env_t *env,
							axis2_char_t *pwd,
							axis2_char_t *base)
{
...
axis2_char_t *
tewst_content_type (const axutil_env_t *env,
...
axis2_status_t 
tewst_output_http_headers (const axutil_env_t *env,
...
axis2_status_t
tewst_set_https_transport_parameretes (const axutil_env_t *env,
										  axis2_svc_client_t *svc_client)
{
