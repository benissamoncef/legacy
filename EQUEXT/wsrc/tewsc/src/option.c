...
/* this is a global variable used in tewsc_stub.c */
int array_size;

/**
 *cmd_options_data [] array should be correctly mapped with enum
 *(cmd_options_em) in the tewsc_option.h. That enum value is used
 *in switch statement in tewsc_stub.c
 */

tewsc_cmd_options_t cmd_options_data [] =
{
      {"get", 0, no_argument},
...
axis2_char_t *
tewsc_destination (int argc, char **argv)
{
      char *dest_uri = NULL;
      if (argc > 1 && argv)
      {
...
axutil_array_list_t *
tewsc_process_options (int argc,
			  char **argv, 
      ...
      for (i = 0;i < array_size; i++)
      {
	    ...
	    for (ii = 0; ii < argc; ii++)
	    {
		  ...
		  if (name)
		  {
			...
			{
			      options = AXIS2_MALLOC (env->allocator, sizeof (tewsc_options_t));
			      if (options)
			      {
				    /*options->opt_name = long_name;*/
			    	strcpy(options->opt_name,long_name);
