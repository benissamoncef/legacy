...
int isComplete = 0;
long timeout_in_milli_seconds = 4000;

extern tewsc_cmd_options_t cmd_options_data[];
extern int array_size;
extern void ewsc_decoder_reponse_asf();
extern void ewsc_decoder_CR_asf();
					 ...
					 axiom_node_t *payload)
{
	...
	tewsc_options_t *tewsc_options = NULL;
	int size;
	int i;
	int ii;
	...
	for (i = 0; i < size; i++)
	{
	    tewsc_options = (tewsc_options_t *)
			axutil_array_list_get (array_list, env, i);
	    if (tewsc_options)
	    {
			register char *opt = NULL;
			register int  opt_short = 0;
			opt = tewsc_options->opt_name;
			opt_short = tewsc_options->short_name;
			...
			if (opt || opt_short)
			{
				switch (ii)
				{
					...
					{
						is_contenty_type = 1;
						if (tewsc_options->value)
						{			      
							content_type = (char *)tewsc_options->value;
						}
						AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
					...
					case SOAP_HEADER:
					{
						...
						char *header = (char *) tewsc_options->value;
					...
					case USER:
					{
						...
						username_value = (axis2_char_t *)tewsc_options->value;
					}
					break;
					case PASSWORD:
					{
						is_password = 1;
						is_soap_enabled = 1;
                        password_buffer = axutil_strdup(env,  tewsc_options->value);
						AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
                    ...
                    {
                        enable_rampart = AXIS2_TRUE;
                        private_key_file = (axis2_char_t *)tewsc_options->value;
                        AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI,
                    ...
                    {
                        enable_rampart = AXIS2_TRUE;
                        certificate_file = (axis2_char_t *)tewsc_options->value;
                        AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI,
                    ...
                    {
                        enable_rampart = AXIS2_TRUE;
                        recipient_certificate_file = (axis2_char_t *)tewsc_options->value;
                        AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI,
                    ...
                    case ALGORITHMSUITE:
                    {
                        ...
                        neethi_options_set_algorithmsuite(neethi_options, env, 
                                    (axis2_char_t *)tewsc_options->value);
                        AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI,
					...
					case PASSWORD_FILE:
					{
                        ...
                        is_username = 1;
						is_soap_enabled = 1;
						password_file = (char *) tewsc_options->value;
						AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
                    ...
                    {
                        enable_rampart = AXIS2_TRUE;
                        policy_file = (axis2_char_t *)tewsc_options->value;
                        AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI,
					...
					case XOP_IN:
					{
						...
						is_xop_in = 1;
						dir = (axis2_char_t *)tewsc_options->value;
					...
					{
						is_xop_out = 1;
						xop_out_dir = (axis2_char_t *)tewsc_options->value;
						AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
										"[tewst] xop out block");
					}
					break;
					case HTTP_HEADER:
					{
						...
						const char *header = (char *) tewsc_options->value;
					...
					case OUTPUT_HTTP_HEADERS:
					{
						...
						output_filename = (char *) tewsc_options->value;
						AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
					...
					case SERVER_CERT:
					{
						...
						server_cert = (char *)tewsc_options->value;
	...
	int actionType)
{
	...
	AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
					"[tewst] payload %s \n", input);
	XZST_03EcritureTrace(XZSTC_WARNING, "[tewst] payload %s \n", input);

	payload = tewst_payload (env, input);

	if ( ! payload )
	{
		XZST_03EcritureTrace(XZSTC_WARNING, "[tewst] payload creation FAILED: Error code: %d :: %s",
	...
	if (!svc_client)
	{
	    AXIS2_LOG_ERROR(env->log, AXIS2_LOG_SI, "Stub invoke FAILED: Error code:"
						...
						AXIS2_ERROR_GET_MESSAGE(env->error));
	    XZST_03EcritureTrace(XZSTC_FATAL,"Stub invoke FAILED: Error code: %d :: %s",
					env->error->error_number,
					AXIS2_ERROR_GET_MESSAGE(env->error));
	...
	if (payload && svc_client)
	{
		...
		if (fire_and_forget)
		{
		   ...
		   sleep(2);
			...
			AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI,
							"RETOUR axis2_svc_client_send_receive");
		}
		else
	...
	if (ret_node)
	{
	    ...
	    {
	    	if (actionType == TEWSC_ACTION_ACQUERIR){
/*	    		ewsc_decoder_CR_asf(ret_node, env);*/
	    		ewsc_decoder_reponse_asf(ret_node, env);
	    	}else
	    		ewsc_decoder_CR_asf(ret_node, env);

/*	    	axiom_stax_builder_free(axiom_node_get_builder(ret_node, env), env);
	    	axiom_document_free(axiom_node_get_document(ret_node, env), env);
	    	axiom_stax_builder_free(axiom_document_get_builder(axiom_node_get_document(ret_node, env), env), env);
*/
/* debug 21/03
...
*/
	...
	if (payload){
/*		if (axiom_node_get_document(payload, env))
			axiom_document_free(axiom_node_get_document(payload, env), env);
   	if (axiom_node_get_builder(payload, env))
			axiom_stax_builder_free(axiom_node_get_builder(payload, env), env);

		axiom_document_t *doc;
		axiom_stax_builder_t *builder;

		doc = axiom_node_get_document(payload, env);
		if (doc){
			builder = axiom_document_get_builder(doc, env);
			if (builder)
