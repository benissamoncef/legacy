...
/* this is a global variable used in tewsr_stub.c */
int array_size;

/**
 *cmd_options_data [] array should be correctly mapped with enum
 *(cmd_options_em) in the tewsr_option.h. That enum value is used
 *in switch statement in tewsr_stub.c
 */

tewsr_cmd_options_t cmd_options_data [] = 
{
      {"get", 0, no_argument},
...
axis2_char_t *
tewsr_destination (int argc, char **argv)
{
      char *dest_uri = NULL;
      if (argc > 1 && argv)
      {
...
axutil_array_list_t *
tewsr_process_options (int argc,
			  char **argv, 
