...
extern tewsr_cmd_options_t cmd_options_data[];
...
tewsr_svc_option (axis2_svc_client_t *svc_client,
                     const axutil_env_t *env,
					 ...
					 axiom_node_t *payload)
{
	...
	tewsr_options_t *tewsr_options = NULL;
	int size;
	int i;
	int ii;
	...
	for (i = 0; i < size; i++)
	{
	    tewsr_options = (tewsr_options_t *)
			axutil_array_list_get (array_list, env, i);
	    if (tewsr_options)
	    {
			register char *opt = NULL;
			register int  opt_short = 0;
			opt = tewsr_options->opt_name;
			opt_short = tewsr_options->short_name;
				...
				switch (ii)
				{
					...
					case GET:
					{
						...
						AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
										"[tewsr] processed get option ");
					}
					break;
					case CONTENT_TYPE:
					{
						is_contenty_type = 1;
						if (tewsr_options->value)
						{			      
							content_type = (char *)tewsr_options->value;
						}
						AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
										"[tewsr] processed content type ");
					}
					break;
					case SOAP:
					{
					...
					case SOAP_OUT:
					{
						...
						AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
										"[tewsr] soap out block");
					}
					break;
					case SOAP_HEADER:
					{
	                    axiom_node_t *soap_header = NULL;
						char *header = (char *) tewsr_options->value;
	                    soap_header = tewsr_soap_header (env, header);
                        if(soap_header)
                        {
                                ...
                                svc_client, env, soap_header))
                            {
                                AXIS2_LOG_ERROR(env->log, AXIS2_LOG_SI, 
                                    "[tewsr]Adding soap header failed");
                            }
                        }
                        else
                        {
                            AXIS2_LOG_ERROR(env->log, AXIS2_LOG_SI, 
                                "[tewsr]Creating soap header axiom element"\
					...
					case ACTION:
					{
						...
						if (tewsr_options->value)
						{
							axis2_options_set_action(
								options,
								env, 
								(char *)tewsr_options->value);
						}
						AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
										"[tewsr] wsa action block %a", 
										(char *)tewsr_options->value);
					}
					break;
					case SEND_ONLY:
					...
					case USER:
					{
						...
						username_value = (axis2_char_t *)tewsr_options->value;
					}
					break;
					case PASSWORD:
					{
						is_password = 1;
						is_soap_enabled = 1;
                        password_buffer = axutil_strdup(env,  tewsr_options->value);
						AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
										"[tewsr] security password block ");
					}
					break;
					case TIMESTAMP:
					{
						...
						AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
										"[tewsr] security timestamp block ");
					}
                    break;
                    ...
                    case ENCRYPT_BEFORE_SIGNING:
                    {
                        ...
                        AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI,
                                        "[tewsr] Set Encrypt Before signing ");
                    }
                    break;
                    case ENCRYPT_SIGNATURE:
                    {
                        ...
                        AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI,
                                        "[tewsr] Set Signature Protection ");
                    }
                    break;
                    case KEY:
                    {
                        enable_rampart = AXIS2_TRUE;
                        private_key_file = (axis2_char_t *)tewsr_options->value;
                        AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI,
                                        "[tewsr] Set Private Key ");
                    }
                    break;
                    case CERTIFICATE:
                    {
                        enable_rampart = AXIS2_TRUE;
                        certificate_file = (axis2_char_t *)tewsr_options->value;
                        AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI,
                                        "[tewsr] Set Certificate ");
                    }
                    break;
                    case RECIPIENT_CERTIFICATE:
                    {
                        enable_rampart = AXIS2_TRUE;
                        recipient_certificate_file = (axis2_char_t *)tewsr_options->value;
                        AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI,
                                        "[tewsr] Set receiver Certificate ");
                    }
                    break;
                    case ALGORITHMSUITE:
                    {
                        ...
                        neethi_options_set_algorithmsuite(neethi_options, env, 
                                    (axis2_char_t *)tewsr_options->value);
                        AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI,
                                        "[tewsr] Set Algorithmsuite ");
                    }
                    break;
					case PASSWORD_FILE:
					{
                        ...
                        is_username = 1;
						is_soap_enabled = 1;
						password_file = (char *) tewsr_options->value;
						AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
										"[tewsr] security password file ");
					}
					break;
					case DIGEST:
					{
						is_digest = 1;
						AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
										"[tewsr] security digest block ");
					}
					break;
                    case POLICY_FILE:
                    {
                        enable_rampart = AXIS2_TRUE;
                        policy_file = (axis2_char_t *)tewsr_options->value;
                        AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI,
                                        "[tewsr] Set policy file:%s", policy_file);
                    }
                    break;
					case XOP_IN:
					{
						...
						is_xop_in = 1;
						dir = (axis2_char_t *)tewsr_options->value;
						tewsr_set_attachment (env, payload, dir, is_mtom_enabled);
						AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
										"[tewsr] xop in block");
					}
					break;
					...
					{
						is_xop_out = 1;
						xop_out_dir = (axis2_char_t *)tewsr_options->value;
						AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
										"[tewsr] xop out block");
					}
					break;
					case HTTP_HEADER:
					{
						...
						const char *header = (char *) tewsr_options->value;
						...
						AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
										"[tewsr] add http header");
					}
					break;
					case OUTPUT_HTTP_HEADERS:
					{
						is_output_http_headers = 1;
						output_filename = (char *) tewsr_options->value;
						AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
										"[tewsr] output http header %s", 
										(char *) tewsr_options->value);
					}
					break;
					case SERVER_CERT:
					{
						...
						server_cert = (char *)tewsr_options->value;
						...
						is_server_cert = 1;
						AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
										"[tewsr] server certificate path  %s",
										(char *) tewsr_options->value);
					}
					break;
				};
		...
		if (is_contenty_type)
		{
			...
			if(content_type)
			{
				axutil_hash_set (content_type_hash, 
                                 ...
                                 tewsr_content_type (env, content_type));
			}
...
/* 			AXUTIL_PROPERTY_SET_FREE_FUNC (property, env, axutil_hash_free); */
	...
	{
		AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
						"[tewsr] addressing  enabled");
	    if (is_send_only)
	    {
			AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
							"[tewsr] addressing  enabled sendonly ");
			if(!is_reply_to)
			{
				reply_to = axis2_endpoint_ref_create(env,
	    ...
	    {
			AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI,
							"[tewsr] adding reply-to to options");
			if (reply_to)
			{
				axis2_options_set_reply_to (options, env, reply_to);
			}
	    ...
	    {
			AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI,
							"[tewsr] adding in-reply-to to options");
			if (in_reply_to)
			{
	    ...
	    {
			AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
							"[tewsr] adding fault-to to options");
			if (fault_to)
			{
	...
	{
		AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
						"[tewsr] setting soap-dump property to options");
	...
	axis2_char_t *input)
{
	...
	AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
					"[tewsr] payload %s \n", input);
	payload = tewsr_payload (env, input);
	...
	if (!client_home)
	{
        ...
        if (!client_home)
        {
		    ...
		    AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
		        "[tewsr] WSFC_HOME is empty");
            return TEWSR_FAILURE;
#ifdef WSFC_PREFIX    
        }
        else
    		AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI,
	...
	{
		AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
						"[tewsr] destination uri not available");
	    return TEWSR_ERROR_NO_DESTURI;
	}
	...
	if (!svc_client)
	{
	    ...
	    return -1 /* TEWSR_ERROR_SENDING_MSG */;
	}

	options = tewsr_svc_option (svc_client, env, array_list, payload);
	AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
					"[tewsr] svc client created");
	...
	if (is_soap_enabled)
	{
		if (is_wsa_enabled)
		{
			...
			AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
							"[tewsr] addressing module engaged");
		}
        ...
        if(enable_rampart)
		{
            ...
            AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, "[tewsr] setting creating policy node ");

            if (rampart_context_set_user(rampart_context, env,
                                     (axis2_char_t *)username_value) == AXIS2_SUCCESS)
                AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, "[tewsr] setting username ");
            ...
            {
                AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI,
                            "[tewsr] processing password file");
                if (username_value)
                {
                    password_buffer = tewsr_get_password (env, username_value, password_file);
                    if (password_buffer)
                    {
                        is_password = 1;
                        is_soap_enabled = 1;
            ...
            {
                AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI,
                            "[tewsr] adding digest password property");
                if(rampart_context_set_password_type(rampart_context, env,
                                                 (axis2_char_t *)"Digest") == AXIS2_SUCCESS)
                    AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, "[tewsr] setting passwordType ");
            }
            ...
            if (password_buffer)
            {
                ...
                AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, "[tewsr] setting password ");
            ...
            {
                AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, "[tewsr] Private key file not specified. ");
                return TEWSR_FAILURE;
            }
            if(certificate_file)
            {
            ...
            {    
                AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, "[tewsr] Certificate file not specified. ");
                return TEWSR_FAILURE;
            }
            ...
            {
                AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, "[tewsr] Reciever Certificate not specified. ");
                return TEWSR_FAILURE;
            }

			rampart_context_set_obtain_security_context_token_fn(
                rampart_context, env, tewsr_obtain_sct_default);
            rampart_context_set_store_security_context_token_fn(
                rampart_context, env, tewsr_store_sct_default);
            rampart_context_set_delete_security_context_token_fn(
                rampart_context, env, tewsr_delete_sct_default);
            rampart_context_set_validate_security_context_token_fn(
                rampart_context, env, tewsr_validate_sct_default);
	...
	if (payload && svc_client)
	{
		...
		AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
						"[tewsr] compiled with ssl enabled");
		if (strstr (dest_uri, "https"))
		{
			if (is_server_cert)
				tewsr_set_https_transport_parameretes (env, svc_client);
			else
			{
				printf ("server certificate not found \n");
				AXIS2_LOG_ERROR(env->log, AXIS2_LOG_SI, 
								"[tewsr] server certificate not found");
				return TEWSR_FAILURE;
			}
		}
#endif
		AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
						"[tewsr] prepareing for svc client send receive ");
		...
		if (is_soap_dump)
		{
			tewsr_soap_out (env, svc_client, 0);
			AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
							"[tewsr] soap dump successful");
			return TEWSR_SUCCESS;
		}

		if (is_send_only && (status == AXIS2_SUCCESS))
		{
			AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
							"[tewsr] soap send successful");
			return TEWSR_SUCCESS;
		}
	}
	else
	...
	if (ret_node)
	{
		...
		AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
						"[tewsr] response return node found");
	    if(is_send_only)
	    {
			...
			if(0 != axutil_strcasecmp(node_name, "fault"))
			{
				...
				return TEWSR_ERROR_RECEIVING_RESPONSE;
			}
			AXIS2_FREE (env->allocator, node_name);
	    }

		if (is_output_http_headers)
			tewsr_output_http_headers (env, svc_client, output_filename);

		if (is_xop_out)
		{
			tewsr_get_attachment (env, ret_node, xop_out_dir, is_mtom_enabled);
			tewsr_soap_out (env, svc_client, 1);
			AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
							"[tewsr] xop out successful");
		}
		else if (is_soap_out)
	    {
			tewsr_soap_out (env, svc_client, 1);
			AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
							"[tewsr] soap out SUCCESSFUL");
	    }
	    else
	    {
			...
			if (om_str)
            {
				 ...
				 */
			    AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
					"[tewsr] response retrieval successful %s", om_str);
            }
			ret_node = NULL;
	    }
	...
	{
		AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, 
						"[tewsr] invoke response retrival failed");
		return TEWSR_FAILURE;
	}
	if (password_buffer)
