...
*************************************************
* DESCRIPTION :
* module externe de la tache serveur message tfm
* pour le PC STRADA
...
* Niepceron	2 mar 1999  : Correction rds v1.2
* Cluchage      30 Jun 2004  : Correction nom Snnnnnnn -> SESCnnnn v1.3
* niepceron	12/04/06 1.3 : LTV =5 DEM568
* niepceron	22/01/07	: correction appel xzae127 v1.5
* niepceron	11/12/06 1.3 : LTV =6 DEM732
* PNI		21/10/09 1.4 : ajout du caract�re a dans le champ SNM pour diff�rencier les action des evt DEM901
* JMG   15/02/11 : ajout nom autoroute dans XZAE146 1.5
...
* PNI	9/9/2015 : Remplacement Frontiere Italienne par Italie pour L07 lim. 35car DEM1145 1.11
* PNI	9/9/2015 : Remplacement LOC par P1.1 + integration heure hivers/ete DEM1145 1.11
* PNI	26/03/18 : Suppression du noeud STRADA=>suppression du test de presence STRADA DEM1285 1.12
...
static char *	version = "@(#)ases_tfm.c	1.9 03/28/13 : commentaire" ;


/* declaration de fonctions internes */
void	ases_tfm_cb();
int	ases_tfm_genere_strada(XDY_Entier, XDY_Octet ,XDY_Horodate, XDY_Horodate, char *, XDY_Entier , XDY_Octet );
...
XDY_NomSite		pl_NomSite;
	...
	if (TipcSrvProcessCbCreate(XDM_IdentMsg(XDM_AS_SRVEVT_STRADA), ases_tfm_cb,(T_CB_ARG)NULL) == NULL)
	{
...
/* Fichier : @(#)ases_tfm.c	1.9        Release : 1.9        Date : 03/28/13
******************************************************
* STERIA *  PROJET MIGRAZUR
******************************************************
* MODULE XZAD_TDP
...
void ases_tfm_cb()
{
  ...
  XZST_03EcritureTrace(XZSTC_INFO,"ases_tfm_cb: debut de fonction !");
  
  if (  XZAC70_Recherche_derniers_TFM(ases_tfm_genere_strada) == XDC_NOK )
...
* Pointeur de fonction.
******************************************************/
int ases_tfm_genere_strada(XDY_Entier va_action,
				XDY_Octet va_niveau,
				...
				XDY_Octet va_cle)
{
  ...
  char			vl_str1[50],vl_str2[50],vl_str3[50];
  loc_rds		vl_loc;
  ...
  XDY_Octet		vl_blesses_graves=0;
  XDY_Entier		vl_type_loc;
  char			vl_Config_Voies[256];
  char			vl_nom_loca[26];
  XDY_Horodate		vl_fin_prevue;
  char                  vl_reference[36]; /* Ajout SAGA */

  
  XZST_03EcritureTrace(XZSTC_INFO,"ases_tfm_genere_strada: IN action:%d,niv:%d,succes:%lf,fin:%lf,texte:%s,fmc:%d,cle:%d - site principal = %d-%s",
		va_action, va_niveau,va_succes,va_fin,pa_texte,va_fmc,va_cle,vg_SitePrincipal,vg_NomSitePrincipal);
  ...
  sprintf(vl_bloc.Bloc_Oblig.DOB,"DOB#INF#\n");
  sprintf(vl_bloc.Bloc_Oblig.PHR,"PHR#INF#\n");
  ...
  */
  if (XZAE127_Infos_obligatoires(va_fmc,
				...
				&vl_gravite,
				&vl_nbvoiesop,
				vl_reference /*Ajout SAGA*/) != XDC_OK) {
    XZST_03EcritureTrace(XZSTC_WARNING,"ases_tfm_genere_strada: pb appel a XZAE127_Infos_obligatoires %d %d",
				va_fmc,
				va_cle);
    return (XDC_NOK);
  }
  ...
  if (XZAE146_Recherche_Loc_Rds(vl_pr,
    ...
    XZST_03EcritureTrace(XZSTC_WARNING,"ases_tfm_genere_strada: pb appel a XZAE146_Recherche_Loc_Rds %d %d",
				vl_pr,
				va_cle);
    return (XDC_NOK);
  }
  ...
  XZST_03EcritureTrace(XZSTC_INFO,"ases_tfm_genere_strada: pr 1: %d   pr 2: %d",vl_loc.l11,vl_loc.l12);
...
XZST_03EcritureTrace(XZSTC_INFO,"ases_tfm_genere_strada: l11: %s - l12 : %s",vl_bloc.loc.l11,vl_bloc.loc.l12);
  ...
  if (sscanf( vl_horo_chaine, "%2d/%2d/%4d %2d:%2d",
			&vl_jour,
                        ...
                        &vl_heure,
                        &vl_minute) != 5) {
    XZST_03EcritureTrace(XZSTC_WARNING,"ases_tfm_genere_strada: format date incorrect");
    return (XDC_NOK);
  }
 ...
 if (XZSM_07LireHorodate(XZSMC_TYPE_SYSTEME,&vl_Jour_De_La_Semaine,&vl_Horodate) != XZSMC_OK)
  ...
  if ( ases_tfm_ecrit_strada(vl_bloc) == XDC_NOK )
  {
                ...
                if (XZAC75_Ecrire_ExporterTFM( va_action, vl_Horodate, XDC_ACT_ETAT_ECHEC) == XDC_NOK)
                		XZST_03EcritureTrace(XZSTC_WARNING, "Impossible de mettre a jour l'action intergestionnaire") ;
                
  }
  else
  {
                if (XZAC75_Ecrire_ExporterTFM( va_action, vl_Horodate, vl_type_horodate) == XDC_NOK)
                		XZST_03EcritureTrace(XZSTC_WARNING, "Impossible de mettre a jour l'action intergestionnaire") ;
...
* Pointeur de fonction.
******************************************************/
int ases_tfm_ecrit_strada(T_Fichier	va_bloc)
{
  FILE *pl_fd;
  FILE *pl_fp;
  char	vl_str[300];
  ...
  char	pl_ligne[300];
        ...
        /* ecriture trace */
        XZST_03EcritureTrace(   XZSTC_FONCTION, "IN  : ases_tfm_ecrit_strada" );
        ...
        /* On lit les lignes du fichier compteur_tfm.txt une par une */
...
XZST_03EcritureTrace(   XZSTC_INFO, " nom du fichier = %s", pl_nom_fichier);
  ...
  if ((pl_fd=fopen(pl_nom_fichier,"w"))==NULL) {
    XZST_03EcritureTrace(XZSTC_WARNING,"ases_tfm_ecrit_strada: pb creation fichier %s",
		pl_nom_fichier);
    return (XDC_NOK);
  }
