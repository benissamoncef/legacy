/*E*/
/*  Fichier : $Id: xzac43sp.prc,v 1.8 2010/09/02 09:48:19 gesconf Exp $      Release : $Revision: 1.8 $        Date : $Date: 2010/09/02 09:48:19 $
------------------------------------------------------
...
* JPL	21/07/10	: Au CI ou sur secteur, si l'action est introuvable, executer sur HIS (DEM 941) 1.7
* JPL	01/09/10	: ... et retour compte-rendu OK si l'evenement est trouve 1.8
-----------------------------------------------------*/
...
as
	...
	select
		...
		@va_Manuelle_out	= HIS..ACT_ECH.manuelle,
	...
	where	HIS..ACT_GEN.numero = @va_NumeroAction_in and 
		...
		HIS..ACT_ECH.actio = @va_NumeroAction_in and 
		HIS..ACT_ECH.sit = @va_Site_in
	...
	select
		...
		@va_Manuelle_out		= EXP..ACT_ECH.manuelle,
	...
	where	EXP..ACT_GEN.numero = @va_NumeroAction_in and 
		...
		EXP..ACT_ECH.actio = @va_NumeroAction_in and 
		EXP..ACT_ECH.sit = @va_Site_in
	...
	if @vl_NbEnrg = 0
	begin
		exec @vl_Status = SQL_HIS.PRC..XZAC;43
					@va_NumeroAction_in,
					@va_Site_in,
