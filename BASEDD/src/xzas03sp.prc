/*E*/
/*  Fichier : $Id: xzas03sp.prc,v 1.11 2007/03/26 15:53:16 gesconf Exp $      Release : $Revision: 1.11 $        Date : $Date: 2007/03/26 15:53:16 $
------------------------------------------------------
...
* C.T.	07/12/94	: Correction sur ecretage des PR et
*                         la clause where pour la mesure a la
*                         date de debut (PR) (V 1.2)
*                         non teste
* C.T.+BG 18/01/95	: modif ecretage	(V 1.3)
* C.T.	18/01/95	: modif la recherche du pr de queue ou de tete
*                         et la recherche de la mesure pour la
*                         date de debut		(V 1.4)
* C.T.	06/02/95	: Ajout controle d'evt de tete suite a
*                         une incoherence de base	(V 1.5)
* C.T.	11/08/95	: Correction lors de bouchon non sur section courante (V 1.6)
* C.T.	05/09/95	: Correction lors de bouchon non sur section courante (V 1.8)
* B.G.	31/10/95	: Gestion sens inverse A57 (1.9)
* C.P.	23/06/98	: Modif separation EXP et HIS (V 1.10)
* CHI   05/09/05        : Modif lissage patate : rajout param num FMC DEM509 v1.11
-----------------------------------------------------*/
...
as
	...
	select @vl_DateFin = dateadd(hour, 6, @va_HorodateDebut_in)
	...
	begin	
		exec @vl_Status = SQL_HIS.PRC..XZAS;03
				@va_Autoroute_in, @va_PRDebut_in, @va_PRFin_in, @va_HorodateDebut_in
                        if @vl_Status = null
                                return XDC_PRC_INC
	...
	begin
		...
		begin
			...
			begin
				...
				/* au moins une fois pendant l'intrevalle de temps */
				insert #BOU_PAS_ZONE
				values(@vl_Cause, @vl_CleCause)
		end
	...
	where #GRA_BOU.numero = #BOU_INC.numero and #GRA_BOU.cle = #BOU_INC.cle
	...
	create table #GRAPHE ( numero int, horodate datetime, horodate_fin datetime, 
	PR_debut int, PR_fin int, type tinyint)
	...
	where (#BOU_NON_VAL.numero = #GRA_BOU.numero and #BOU_NON_VAL.cle = #GRA_BOU.cle) 
		or (#BOU_NON_VAL.cause = #GRA_BOU.numero and #BOU_NON_VAL.cle_cause = #GRA_BOU.cle)
	delete #BOU_NON_VAL
	from #BOU_PAS_ZONE, #BOU_NON_VAL
	where (#BOU_NON_VAL.numero = #BOU_PAS_ZONE.numero and #BOU_NON_VAL.cle = #BOU_PAS_ZONE.cle) 
		or (#BOU_NON_VAL.cause = #BOU_PAS_ZONE.numero and #BOU_NON_VAL.cle_cause = #BOU_PAS_ZONE.cle)
		...
		begin
			...
			begin
				...
				if (@vl_PRFin >= @va_PRDebut_in and
				   ...
				   @vl_PRDebut >= @va_PRFin_in)
					insert #GRAPHE
					values ( @vl_Cause, @va_HorodateDebut_in, @vl_DateFin,
						@vl_PRDebut, @vl_PRFin, @vl_Type)
			end
		end
	...
	/*A Retourner le graphe */
	select distinct * from #GRAPHE
	where  PR_debut <> PR_fin
	order by numero,horodate
