/*E*/
/* Fichier : $Id: xzat530sp.prc,v 1.2 2021/03/08 13:15:20 pc2dpdy Exp $        Release : $Revision: 1.2 $        Date : $Date: 2021/03/08 13:15:20 $
------------------------------------------------------
* STERIA *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME  BASEDD
------------------------------------------------------
* FICHIER xzat530sp.prc
------------------------------------------------------
...
/*-----------------------------------------------------
* Service rendu
* Met le compte rendu d'une action sur CFE en base
*
* Sequence d'appel
* SP	XZAT530_CompteRendu_CFE
* 
* Arguments en entree
...
if exists (select * from sysobjects where name = 'XZAT530' and type = 'P')
	drop procedure XZAT530
go

create procedure XZAT530
...
as
	...
	where numero = @va_NumAction_in and sit = @vl_Site and type = XDC_ACT_CFE
	...
	begin
		...
		update EXP..ACT_GEN set heure_fin  = @va_Horodate_in
		where equipement = @vl_NumEqt and type = XDC_EQT_CFE and sit = @vl_Site and
	...
	begin
		...
		update EXP..ACT_GEN set heure_fin = @va_Horodate_in
		where 	numero < @va_NumAction_in and sit = @vl_Site  and
			type = XDC_ACT_CFE and equipement = @vl_NumEqt and
			heure_fin is null
