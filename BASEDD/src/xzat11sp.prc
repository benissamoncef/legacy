/*E*/
/*  Fichier : $Id: xzat11sp.prc,v 1.10 2021/03/08 13:41:03 pc2dpdy Exp $      Release : $Revision: 1.10 $        Date : $Date: 2021/03/08 13:41:03 $
------------------------------------------------------
...
* C.T.	13/09/96	: Ajout cas sur serveurs DS et DC (RADT 1.4)
...
* LCL	22/01/21	: MOVIS PRA DEM-SAE93 1.10
...
as
	...
	begin
		if @vl_Site = XDC_VC
			exec @vl_Status = SQL_VC.PRC..XZAT;11 
				@va_NumEqt_in, @va_TypeEqt_in
		else if @vl_Site = XDC_DP
			exec @vl_Status = SQL_DP.PRC..XZAT;11 
				@va_NumEqt_in, @va_TypeEqt_in
		else
			return XDC_NOK
...
*/		where numero = @vl_Site)
	...
	update EXP..EQT_ACV set activable = XDC_OUI
	where equipement = @va_NumEqt_in and type = @va_TypeEqt_in
