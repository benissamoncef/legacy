...
Fichier_Compil="xzam${Site_Compil}"
...
{
    echo " "
    echo -e '\E[47;31m'"\033[1mla commande xzam_make.sh a ete  modifiee.\033[0m"
    echo " xzam_make.sh appelle  le script ${Nom_Prog} qui prend en compte les specificites de l'environement de developpement et celles de production."
    echo " " 
    ...
    echo " xzam_make.sh appelle  le script (${Nom_Prog} ) pour prendre en compte les specificites de l'environement de developpement.(directive DEV)"
    ...
    echo -e '\E[47;34m'"\033[1m Exemple: procedure pour le site CI plateforme de DEV  (via xzam_make.sh): $0 CI DEV \033[0m"
    ...
    echo " ${Nom_Prog} genere le fichier .sql a livrer en production via le script sae_liv <num_jira> -p xzam_make.sh  <site> "
...
echo "if exists (select 1 from sysobjects where name = 'XZAM' and type = 'P')" >> ${fic_tmp_c}

echo "drop procedure XZAM " >> ${fic_tmp_c} 
echo "go" >> ${fic_tmp_c}

# integration des includes utilises
echo "#include \"$includes/XDMICG/inc/xdc.h\"" >> ${fic_tmp_c}
echo "#include \"$includes/XDMICG/inc/xzamc.h\"" >> ${fic_tmp_c}    


if [[ "$Site_Compil" != "HIS" ]]
then
    # les groupees
    cat_proc "xzam01sp.prc"
    cat_proc "xzam02sp.prc"
    cat_proc "xzam03sp.prc"
    cat_proc "xzam04sp.prc"
    cat_proc "xzam05sp.prc"
    cat_proc "xzam07sp.prc"
    cat_proc "xzam10sp.prc"
    cat_proc "xzam11sp.prc"
    sae_dev_make_prc "xzam08sp.prc" $Site_Compil $cible
    sae_dev_make_prc "xzam12sp.prc" $Site_Compil $cible
   sae_dev_make_prc "xzam1000sp.prc" $Site_Compil $cible	
fi

    sae_dev_make_prc "xzam06sp.prc" $Site_Compil $cible
