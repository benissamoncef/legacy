/*E*/
/*Fichier :  $Id: xzac631sp.prc,v 1.13 2021/03/08 14:26:42 pc2dpdy Exp $      Release : $Revision: 1.13 $        Date : $Date: 2021/03/08 14:26:42 $
------------------------------------------------------
* STERIA *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME  BASEDD
------------------------------------------------------
* FICHIER xzac631sp.prc
------------------------------------------------------
...
* JBL	24/07/08	: correction FT 1711 : xzac631 ne fonctionne pas sur site, 
*                     action = 0, du coup impossible de mettre à jour la date 
*                     de lancement de fin et impossible de cloturer la fiche
* JMG           02/10/09        : secto DEM887
* JMG		02/10/09	: pastun DEM895
* PNI           19/04/16 : Correction des appels dans PRC des XZAC sur les serveurs sites v1.7 DEM1159
* JMG		27/03/17 : regio 1.8
* LCL		03/07/17 : modif PRA
* JPL		26/09/17 : Rectification variable de site  1.10
* ABE   05-02-2020 : modifications intégration ouvrage toulon
* LCL   14/09/20 (20/11/19) : MOVIS DEM-SAE93 1.11 
* GRO	12/10/2020      : MOVIS DEM-SAE93 1.12
* CGR	05/01/21	: MOVIS ajout PRA DEM-SAE93 1.13
...
*
* Sequence d'appel
* SP	XZAC631_Commande_Fin_Signalisation
*
* Arguments en entree
...
* 	ACRP631 :	Appeler XZET03 pour commander l'equipement
-----------------------------------------------------------------------*/

use PRC
go

if exists (select 1 from sysobjects where name = 'XZAC631' and type = 'P')
	drop procedure XZAC631
go


create procedure XZAC631
	...
	/* Traitement au district d'exploitation pour Escota, en local sinon */
	 begin
	 	if @vl_serveur_pra = XDC_SQLVC
			exec @vl_Status = SQL_DM.PRC..XZAC631 @va_TypEqt_in, @va_NumeroTunnel_in, @va_Site_in, @va_HeureLancementFin_in, @va_Manuelle_in, @va_NomSiteLocal_in, XDC_OUI 
		
		/* modif ABE du 05-02-2020 : spécificité toulon */
		
		else if @vl_serveur_pra = XDC_SQLDP
			exec @vl_Status = SQL_DS.PRC..XZAC631 @va_TypEqt_in, @va_NumeroTunnel_in, @va_Site_in, @va_HeureLancementFin_in, @va_Manuelle_in, @va_NomSiteLocal_in, XDC_OUI 
		
		else if @vl_serveur_pra = XDC_SQLCI
		        exec @vl_Status = SQL_CI.PRC..XZAC631 @va_TypEqt_in, @va_NumeroTunnel_in, @va_Site_in, @va_HeureLancementFin_in, @va_Manuelle_in, @va_NomSiteLocal_in, XDC_OUI
...
*/
	...
	begin
		...
		where 	EXP..ACT_GEN.equipement = @va_NumeroTunnel_in and
			EXP..ACT_GEN.sit = @va_Site_in and
			EXP..ACT_GEN.type = XDC_ACT_GTC_SIG_ECH
		if @@rowcount = 0
			select @vl_NumeroAction = 0
	end
	...
	begin
		...
		select @vl_Sequence = XDC_SIG_SEQ_RETOUR_NORMAL
		...
		if @vl_Site = XDC_CI
			exec @vl_Status = TASRV_CI...ACRP40 @va_TypEqt_in, @va_NumeroTunnel_in, @vl_Sequence,
				@vl_Commande

		else if @vl_Site = XDC_DP
			exec @vl_Status = TASRV_DP...ACRP40 @va_TypEqt_in, @va_NumeroTunnel_in, @vl_Sequence,
				@vl_Commande

		else if @vl_Site = XDC_VC
			exec @vl_Status = TASRV_VC...ACRP40 @va_TypEqt_in, @va_NumeroTunnel_in, @vl_Sequence,
				@vl_Commande
	
		else if @vl_Site = XDC_CA
			exec @vl_Status = TASRV_CA...ACRP40 @va_TypEqt_in, @va_NumeroTunnel_in, @vl_Sequence,
				@vl_Commande
	...
	begin
#if defined (DEV)
		update EXP..ACT_GEN set heure_lancement_fin = @va_HeureLancementFin_in,heure_fin = @va_HeureLancementFin_in
		where	numero = @vl_NumeroAction and sit = @va_Site_in and
			heure_lancement_fin is null


		/*on termine 'de force' les autres actions de signalisation sur le meme equipement*/
		/*pour traiter la problematique du scenario Nature*/
		update EXP..ACT_GEN set heure_lancement_fin = @va_HeureLancementFin_in, 
					heure_fin = @va_HeureLancementFin_in
		where numero != @vl_NumeroAction and sit = @va_Site_in and
			heure_lancement_fin is null
			and EXP..ACT_GEN.equipement = @va_NumeroTunnel_in
			and EXP..ACT_GEN.type = XDC_ACT_GTC_SIG
#else
		update EXP..ACT_GEN set heure_lancement_fin = @va_HeureLancementFin_in
		where	numero = @vl_NumeroAction and sit = @va_Site_in and
			heure_lancement_fin is null
