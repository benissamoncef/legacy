...
Fichier_Compil="xzaz${Site_Compil}"
...
{
    echo " "
    echo -e '\E[47;31m'"\033[1mla commande xzaz_make.sh a ete  modifiee.\033[0m"
    echo " xzaz_make.sh appelle  le script ${Nom_Prog} qui prend en compte les specificites de l'environement de developpement et celles de production."
    echo " " 
    ...
    echo " xzaz_make.sh appelle  le script (${Nom_Prog} ) pour prendre en compte les specificites de l'environement de developpement.(directive DEV)"
    ...
    echo -e '\E[47;34m'"\033[1m Exemple: procedure pour le site CI plateforme de DEV  (via xzaz_make.sh): $0 CI DEV \033[0m"
    ...
    echo " ${Nom_Prog} genere le fichier .sql a livrer en production via le script sae_liv <num_jira> -p xzaz_make.sh  <site> "
...
echo "if exists (select 1 from sysobjects where name = 'XZAZ' and type = 'P')" >> ${fic_tmp_c}

echo "drop procedure XZAZ " >> ${fic_tmp_c} 
echo "go" >> ${fic_tmp_c}
...
echo "#include \"$includes/migrazur/XDMICG/inc/xzazc.h\"" >> ${fic_tmp_c}

# groupees
if [[ "$Site_Compil" != "HIS" ]]
then

    cat_proc "xzaz01sp.prc"
