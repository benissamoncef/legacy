/* Fichier : @(#)aweb_dir.c	1.3   Release : 1.3   Date : 11/17/08 
*************************************************
...
* PCL	22/12/02	on lit l'heure pour ne lancer les diffusions qu'a MM+2 mn 1.2
* JMG	17/11/08	refonte avec utilisation de tagen 1.3 DEM 836
...
static char *	version = "@(#)aweb_dir.c	1.3 1.3 11/17/08 : commentaire" ;
...
int             vl_jour,vl_mois,vl_annee,vl_heure,vl_minute,vl_seconde;
	...
	*/
	...
	while (vl_minute%6 != OFFSET_6MN )
	{
	  ...
	  {
	    XZST_03EcritureTrace(XZSTC_WARNING, "aweb_ftp_genere : le format de la date est incorrecte !" );
            return;
          }
	...
	vl_cr = aweb_export_init();
	
	if (vl_cr != XDC_OK) 
      	{
	 	XZST_03EcritureTrace(XZSTC_WARNING,
			"erreur aweb_export_init cr = %d",vl_cr);
	 	exit;
      	}
	...
	while(1)
	{
		...
		if(isMessageNotNULL(pl_Msg))
		{
	 	...
	 	/* erreur RTWORKS */
			...
			{
				XZST_03EcritureTrace (XZSTC_WARNING, "Bouclage et arret tache taweb %d \n",vl_IndPre);
...
XZSMT_CleProg 	vl_cle;
	...
	XZST_03EcritureTrace(XZSTC_FONCTION, "aweb_sortir  : IN");

	/* Liberation des ressources utilisees dans aweb_export.c */
