/*E*/
/* Fichier : $Id: xzac903sp.prc,v 1.1 2021/05/03 12:56:22 pc2dpdy Exp $      Release : $Revision: 1.1 $        Date : $Date: 2021/05/03 12:56:22 $
------------------------------------------------------
* STERIA *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME  BASEDD
------------------------------------------------------
* FICHIER xzac903sp.prc
------------------------------------------------------
...
/*-----------------------------------------------------
* Service rendu
* Action d'affichage IMU.
* 
* Sequence d'appel
* SP	XZAC903_Commande_IMU
* 
* Arguments en entree
...
* 
* Inserer dans la table ACTION et la table E_IMU.
* 
...
* 
* ACRP903 :	Appeler XZEP903 pour piloter l'equipement IMU
* 
-----------------------------------------------------*/
...
if exists (select * from sysobjects where name = 'XZAC903' and type = 'P')
	drop procedure XZAC903
go

create procedure XZAC903
...
as
	...
	begin
		if @vl_serveur_pra=XDC_SQLDP	
			exec @vl_Status = SQL_DS.PRC..XZAC903 @va_Horodate_in,
						@va_NumEvt_in,
						@va_CleEvt_in,
						@va_Operateur_in,
						@va_NumEqt_in,
			...
			exec @vl_Status = SQL_DM.PRC..XZAC903 @va_Horodate_in,
						@va_NumEvt_in,
						@va_CleEvt_in,
						@va_Operateur_in,
						@va_NumEqt_in,
			...
			exec @vl_Status = SQL_CI.PRC..XZAC903 @va_Horodate_in,
						@va_NumEvt_in,
						@va_CleEvt_in,
						@va_Operateur_in,
						@va_NumEqt_in,
	...
	from	EXP..EQT_DSP
	where	type = XDC_EQT_IMU  and  equipement = @va_NumEqt_in  and
		sit = @vl_Site  and
		dernier = 1
...
/*#if defined (DEV)
	...
	begin tran
		...
		XDC_ACT_IMU,  @va_NumEqt_in,
		@va_Horodate_in, @vl_HeureSucces, @vl_HeureEchec,
		@va_Priorite_in, XDC_NON, @va_DistanceEvt_in, @va_Operateur_in, @va_site_origine_in)


	insert EXP..ACT_IMU (actio, IMU, chaine, sit, intervalle, cycles, duree)
		values ( @va_NumeroAction_out,  @va_NumEqt_in,
		@va_ChaineIMU_in, @vl_Site, @va_Intervalle_in, @va_Cycles_in, @va_Duree_in)

	commit tran


	/*A initialisation des var locales pour la recherche */
	/*A du nom de la machine pilotant le IMU */
	select	@vl_MachineNonTrouvee = XDC_NON,
		@vl_NumEqt = @va_NumEqt_in,
		@vl_TypeEqt = XDC_EQT_IMU

	/*A recherche le nom de la machine pilotant le IMU */
...
/*#if ! defined (DEV)*/
	...
	if @vl_Site = XDC_CI
		exec @vl_Status = TASRV_CI...ACRP70 @va_NumEqt_in, @va_ChaineIMU_in, @va_Intervalle_in, @va_Cycles_in, @va_Duree_in, @va_NumeroAction_out, @vl_NomMachine
	else if @vl_Site = XDC_VC
		exec @vl_Status = TASRV_VC...ACRP70 @va_NumEqt_in, @va_ChaineIMU_in, @va_Intervalle_in, @va_Cycles_in, @va_Duree_in, @va_NumeroAction_out, @vl_NomMachine
	else if @vl_Site = XDC_DP
		exec @vl_Status = TASRV_DP...ACRP70 @va_NumEqt_in, @va_ChaineIMU_in, @va_Intervalle_in, @va_Cycles_in, @va_Duree_in, @va_NumeroAction_out, @vl_NomMachine
	else if @vl_Site = XDC_CA
		exec @vl_Status = TASRV_CA...ACRP70 @va_NumEqt_in, @va_ChaineIMU_in, @va_Intervalle_in, @va_Cycles_in, @va_Duree_in, @va_NumeroAction_out, @vl_NomMachine
...
/*#endif*/	/* pas en mode DEV */
	...
	where	EXP..ACT_GEN.equipement = @va_NumEqt_in and 
		...
		(select max(heure_lancement) from EXP..ACT_GEN
		 where	equipement = @va_NumEqt_in and type = XDC_ACT_IMU and
	...
	begin
		...
		begin
			...
			exec 		XZAC;60 XDC_EQT_IMU, @vl_Autoroute, @vl_PR, @vl_Sens, @vl_Intitule output
	...
	if @vl_EtatPbMineur <> 0
	begin
		exec PRC..XZAA;01 @va_Horodate_in, XDC_EQT_IMU, @va_NumEqt_in, XZAAC_AFF_DIF, "Affichage IMU", @vl_NomSite
		return XDC_NOK
	end
#endif
