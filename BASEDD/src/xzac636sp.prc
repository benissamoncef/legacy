/*E*/
/*Fichier :  $Id: xzac636sp.prc,v 1.9 2021/03/08 14:26:05 pc2dpdy Exp $      Release : $Revision: 1.9 $        Date : $Date: 2021/03/08 14:26:05 $
------------------------------------------------------
* STERIA *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME  BASEDD
------------------------------------------------------
* FICHIER xzac636sp.prc
------------------------------------------------------
...
* ABE   05/02/2020	: modifications int�gration ouvrage toulon (reintegration de code)
...
*
* Sequence d'appel
* SP	XZAC636_Commande_Fin_Signalisation
*
* Arguments en entree
...
* 	ACRP636 :	Appeler XZET03 pour commander l'equipement
-----------------------------------------------------------------------*/

use PRC
go

drop procedure XZAC636
go

create procedure XZAC636
	...
	/* Traitement au district d'exploitation pour Escota, en local sinon */
 	begin
		if @vl_serveur_pra = XDC_SQLVC
			  exec @vl_Status = SQL_DM...XZAC636  @va_TypEqt_in,  @va_NumeroTunnel_in,  @va_Site_in, @va_HeureLancementFin_in,  @va_Manuelle_in, @va_NomSiteLocal_in, XDC_OUI 
		
		/* modif ABE du 05-02-2020 : sp�cificit� toulon */
		
		else if @vl_serveur_pra = XDC_SQLDP
			  exec @vl_Status = SQL_DS...XZAC636  @va_TypEqt_in,  @va_NumeroTunnel_in,  @va_Site_in, @va_HeureLancementFin_in,  @va_Manuelle_in, @va_NomSiteLocal_in, XDC_OUI

		else if @vl_serveur_pra = XDC_SQLCI
                          exec @vl_Status = SQL_CI...XZAC636  @va_TypEqt_in,  @va_NumeroTunnel_in,  @va_Site_in, @va_HeureLancementFin_in,  @va_Manuelle_in, @va_NomSiteLocal_in, XDC_OUI
...
*/
	...
	begin
		...
		select @vl_Sequence = XDC_EXP_SEQ_RETOUR_NORMAL
		...
		if @vl_Site = XDC_CI
			exec @vl_Status = TASRV_CI...ACRP45 @va_TypEqt_in, @va_NumeroTunnel_in, @vl_Sequence,
				@vl_Commande

		else if @vl_Site = XDC_DP
			exec @vl_Status = TASRV_DP...ACRP45 @va_TypEqt_in, @va_NumeroTunnel_in, @vl_Sequence,
				@vl_Commande

		else if @vl_Site = XDC_VC
			exec @vl_Status = TASRV_VC...ACRP45 @va_TypEqt_in, @va_NumeroTunnel_in, @vl_Sequence,
				@vl_Commande
	
		else if @vl_Site = XDC_CA
			exec @vl_Status = TASRV_CA...ACRP45 @va_TypEqt_in, @va_NumeroTunnel_in, @vl_Sequence,
				@vl_Commande
