#!/bin/ksh
# Fichier :  $Id: RS_sub_EXP_intersite.sh,v 1.7 2017/04/02 16:29:11 pc2dpdy Exp $      Release : $Revision: 1.7 $        Date : $Date: 2017/04/02 16:29:11 $
##################################################################################
...
##################################################################################
# DESCRIPTION DU SCRIPT : creation des abonnements replication de EXP entre site
# PC2
#
##################################################################################
# HISTORIQUE DES MODIFICATIONS :
# P.N.  27/05/98	: creation
...
# JMG	06/01/17	: ajout tables manquantes pour regionalisation
##################################################################################

if (($# < 3))
then
	echo "Syntaxe : RS_sub_EXP_intersite.sh NomSite NomSitePrim TypeSitePrim (CI,DN,DM,DC)"
...
FicErr="sub_EXP_$NomSite$NomSitePrim.err"
rm $FicErr
...
tables/sub_create.rab EXP FMC_COMMU $NomSitePrim $NomSite xx $FicErr
tables/sub_create.rab EXP FMC_REG $NomSitePrim $NomSite xx $FicErr
tables/sub_create.rab EXP REG_TRA $NomSitePrim $NomSite xx $FicErr
