/*E*/
/*Fichier :  $Id: xzac633sp.prc,v 1.9 2021/03/08 14:26:28 pc2dpdy Exp $      Release : $Revision: 1.9 $        Date : $Date: 2021/03/08 14:26:28 $
------------------------------------------------------
* STERIA *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME  BASEDD
------------------------------------------------------
* FICHIER xzac633sp.prc
------------------------------------------------------
...
* ABE   05-02-2020 : modifications int�gration ouvrage toulon (reinjection modif patch)
...
*
* Sequence d'appel
* SP	XZAC633_Commande_Fin_Ventilation
*
* Arguments en entree
...
* 	ACRP633 :	Appeler XZET03 pour commander l'equipement
-----------------------------------------------------------------------*/

use PRC
go

drop procedure XZAC633
go

create procedure XZAC633
	...
	/* Traitement au district d'exploitation pour Escota, en local sinon */
	 begin
		if @vl_serveur_pra = XDC_SQLVC
			   exec @vl_Status = SQL_DM.PRC..XZAC633  @va_TypEqt_in,  @va_NumeroTunnel_in, @va_Site_in,  @va_HeureLancementFin_in,  @va_Manuelle_in, @va_NomSiteLocal_in, XDC_OUI 
                
		else if @vl_serveur_pra = XDC_SQLDP
	                   exec @vl_Status = SQL_DS.PRC..XZAC633  @va_TypEqt_in,  @va_NumeroTunnel_in, @va_Site_in,  @va_HeureLancementFin_in,  @va_Manuelle_in, @va_NomSiteLocal_in, XDC_OUI
		
		else if @vl_serveur_pra = XDC_SQLCI
                           exec @vl_Status = SQL_CI.PRC..XZAC633  @va_TypEqt_in,  @va_NumeroTunnel_in, @va_Site_in,  @va_HeureLancementFin_in,  @va_Manuelle_in, @va_NomSiteLocal_in, XDC_OUI
...
*/
	...
	begin
		...
		select @vl_Sequence = XDC_VEN_SEQ_RETOUR_NORMAL
		...
		if @vl_Site = XDC_CI
			exec @vl_Status = TASRV_CI...ACRP42 @va_TypEqt_in, @va_NumeroTunnel_in, @vl_Sequence,
				@vl_Commande

		else if @vl_Site = XDC_DP
			exec @vl_Status = TASRV_DP...ACRP42 @va_TypEqt_in, @va_NumeroTunnel_in, @vl_Sequence,
				@vl_Commande

		else if @vl_Site = XDC_VC
			exec @vl_Status = TASRV_VC...ACRP42 @va_TypEqt_in, @va_NumeroTunnel_in, @vl_Sequence,
				@vl_Commande
	
		else if @vl_Site = XDC_CA
			exec @vl_Status = TASRV_CA...ACRP42 @va_TypEqt_in, @va_NumeroTunnel_in, @vl_Sequence,
				@vl_Commande
