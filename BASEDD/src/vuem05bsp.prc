/*E*/
/*  Fichier : $Id: vuem05bsp.prc,v 1.1 2000/07/21 17:15:37 gesconf Exp $     Release : $Revision: 1.1 $        Date : $Date: 2000/07/21 17:15:37 $
------------------------------------------------------
...
* JMG	16/04/99	: RE Modif interg. dem/1724 v1.19
-----------------------------------------------------*/
...
as
	...
	where CFG..RES_DIS.numero != 43 and
		CFG..RES_DIS.type != 0 and
	...
	from #LISTE_BASCUL, HIS..FMC_HIS, HIS..FMC_BAS
	where #LISTE_BASCUL.sens = XDC_SENS_NORD and HIS..FMC_HIS.numero = #LISTE_BASCUL.numero and
		HIS..FMC_HIS.cle = #LISTE_BASCUL.cle and
		HIS..FMC_BAS.numero = #LISTE_BASCUL.numero and
		HIS..FMC_BAS.cle = #LISTE_BASCUL.cle
	group by #LISTE_BASCUL.numero, #LISTE_BASCUL.cle, 
	...
	from #LISTE_BASCUL, HIS..FMC_HIS, HIS..FMC_BAS
	where #LISTE_BASCUL.sens = XDC_SENS_SUD and HIS..FMC_HIS.numero = #LISTE_BASCUL.numero and
		HIS..FMC_HIS.cle = #LISTE_BASCUL.cle and
		HIS..FMC_BAS.numero = #LISTE_BASCUL.numero and
		HIS..FMC_BAS.cle = #LISTE_BASCUL.cle
	group by #LISTE_BASCUL.numero, #LISTE_BASCUL.cle, 
	...
	from #LIMITE_DIS,#BASCUL
	where	#LIMITE_DIS.autoroute = #BASCUL.autoroute and
		#LIMITE_DIS.PR_debut <= #BASCUL.PR and
		#LIMITE_DIS.PR_fin > #BASCUL.PR
	...
	select distinct
		...
		mois_debut,
		mois_fin,
		#LISTE_VOIES.autoroute,
		PR_tete = isnull(min(localisation_tete),max(PR)),
		max(PR)
	from #LISTE_VOIES, HIS..FMC_HIS, HIS..FMC_TRH
	where #LISTE_VOIES.sens = XDC_SENS_NORD and HIS..FMC_HIS.numero = #LISTE_VOIES.numero and
		HIS..FMC_HIS.cle = #LISTE_VOIES.cle and
		HIS..FMC_TRH.numero = #LISTE_VOIES.numero and
		HIS..FMC_TRH.cle = #LISTE_VOIES.cle
	group by #LISTE_VOIES.numero, #LISTE_VOIES.cle, HIS..FMC_HIS.numero, HIS..FMC_HIS.cle,
		#LISTE_VOIES.numero, #LISTE_VOIES.cle
	...
	select distinct
		...
		min(PR)
	from #LISTE_VOIES, HIS..FMC_HIS, HIS..FMC_TRH
	where #LISTE_VOIES.sens = XDC_SENS_SUD and HIS..FMC_HIS.numero = #LISTE_VOIES.numero and
		HIS..FMC_HIS.cle = #LISTE_VOIES.cle and
		HIS..FMC_TRH.numero = #LISTE_VOIES.numero and
		HIS..FMC_TRH.cle = #LISTE_VOIES.cle
	group by #LISTE_VOIES.numero, #LISTE_VOIES.cle, HIS..FMC_HIS.numero, HIS..FMC_HIS.cle,
	...
	from #LIMITE_DIS,#TRV
	where	#LIMITE_DIS.autoroute = #TRV.autoroute and
		#LIMITE_DIS.PR_debut <= #TRV.PR and
		#LIMITE_DIS.PR_fin > #TRV.PR
	update #TRV set sit_tete = sit
	from #LIMITE_DIS,#TRV
	where	#LIMITE_DIS.autoroute = #TRV.autoroute and
		#LIMITE_DIS.PR_debut <= #TRV.PR_tete and
		#LIMITE_DIS.PR_fin > #TRV.PR_tete
	...
	select
		...
		#TRV_PR.VR,
		nb_voies_occultees
	from #TRV_PR, #LISTE_VOIES
	where #LISTE_VOIES.numero = #TRV_PR.numero and #LISTE_VOIES.cle = #TRV_PR.cle and
		horodate <= fin	
	group by #TRV_PR.numero, #TRV_PR.cle, #LISTE_VOIES.numero, #LISTE_VOIES.cle
	having #LISTE_VOIES.numero = #TRV_PR.numero and #LISTE_VOIES.cle = #TRV_PR.cle and
	...
	select
		...
		#TRV_PR.VR,
		nb_voies_occultees
	from #TRV_PR, #LISTE_BASCUL
	where #LISTE_BASCUL.numero = #TRV_PR.numero and #LISTE_BASCUL.cle = #TRV_PR.cle and
		horodate <= fin	
	group by #TRV_PR.numero, #TRV_PR.cle, #LISTE_BASCUL.numero, #LISTE_BASCUL.cle
	having #LISTE_BASCUL.numero = #TRV_PR.numero and #LISTE_BASCUL.cle = #TRV_PR.cle and
		horodate <= fin and horodate = max(horodate)
	...
	from #LIMITE_DIS,#GRA_TRV
	where #LIMITE_DIS.autoroute = #GRA_TRV.autoroute and PR_debut <= PR and PR_fin > PR

	/*limite de concession*/
	update #GRA_TRV set sit_queue = sit, PR_debut_sit_queue = PR_debut, PR_fin_sit_queue = PR_fin
	from #LIMITE_DIS,#GRA_TRV
	where sit_queue = null and
		#LIMITE_DIS.autoroute = #GRA_TRV.autoroute and
		PR_fin = PR and
		PR_debut <= PR

	update #GRA_TRV set sit_tete = sit, PR_debut_sit_tete = PR_debut, PR_fin_sit_tete = PR_fin
	from #LIMITE_DIS,#GRA_TRV
	where #LIMITE_DIS.autoroute = #GRA_TRV.autoroute and PR_debut <= PR_tete and PR_fin > PR_tete
