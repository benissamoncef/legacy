/*E*/
/*Fichier :  $Id: xzac124sp.prc,v 1.3 2010/09/02 09:49:30 gesconf Exp $      Release : $Revision: 1.3 $        Date : $Date: 2010/09/02 09:49:30 $
------------------------------------------------------
...
* 
* Sequence d'appel
* SP	XZAC124_Lire_Commande_Incendie
* 
* Arguments en entree
...
drop procedure XZAC124
go

create procedure XZAC124
...
as
	...
	select
			...
			@va_Manuelle_out	= HIS..ACT_GTC_INC.manuelle,
			@va_Sequence_out	= HIS..ACT_GTC_INC.sequence
	from HIS..ACT_GTC_INC, HIS..ACT_GEN
	where	HIS..ACT_GEN.numero = @va_NumeroAction_in and 
			HIS..ACT_GEN.type = XDC_ACT_GTC_INC and
			HIS..ACT_GEN.sit = @va_Site_in and
			HIS..ACT_GTC_INC.actio = @va_NumeroAction_in and 
			HIS..ACT_GTC_INC.sit = @va_Site_in
	...
	select
		...
		@va_Manuelle_out		= EXP..ACT_GTC_INC.manuelle,
		@va_Sequence_out		= EXP..ACT_GTC_INC.sequence
	from EXP..ACT_GTC_INC, EXP..ACT_GEN
	where	EXP..ACT_GEN.numero = @va_NumeroAction_in and 
		EXP..ACT_GEN.type = XDC_ACT_GTC_INC and
		EXP..ACT_GEN.sit = @va_Site_in and
		EXP..ACT_GTC_INC.actio = @va_NumeroAction_in and 
		EXP..ACT_GTC_INC.sit = @va_Site_in
	...
	if @vl_NbEnrg = 0
	begin
		exec @vl_Status = SQL_HIS.PRC..XZAC124
					@va_NumeroAction_in,
					@va_Site_in,
