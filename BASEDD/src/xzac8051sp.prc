/* Fichier : $Id: xzac8051sp.prc
------------------------------------------------------
* STERIA *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME  BASEDD
------------------------------------------------------
* FICHIER xzac8051sp.prc
------------------------------------------------------
...
* HISTORIQUE :
*
* GGY	14/11/23	:	Creation (DEM483)
-----------------------------------------------------*/

/*-----------------------------------------------------
* Service rendu
* Lecture de l'actio d'affichage PIC.
* 
* Sequence d'appel
* SP	XZAC8051_Lire_Commande_PIC
* 
* Arguments en entree
...
* 
* Fonction
* Select dans les tables ACTION et E_PIC
-----------------------------------------------------------------------*/
use PRC
go

if exists (select * from sysobjects where name = 'XZAC8051' and type = 'P')
	drop procedure XZAC8051
go

create procedure XZAC8051
	@va_NumAction_in			int				= null,
	@va_Site_in					T_SITE			= null,
	@va_NumEqt_out				T_EQUIPEMENT	= null	output,
	@va_Bandeau_out				char(XDC_LGR_Bandeau_PIC)		= null	output,
	@va_Flash_out				bit				= 0		output,
		...
		@va_HeureFin_out			= HIS..ACT_GEN.heure_fin
			from HIS..ACT_PIC, HIS..ACT_GEN
				where	HIS..ACT_GEN.numero = @va_NumAction_in and 
						...
						HIS..ACT_PIC.actio = @va_NumAction_in and 
						HIS..ACT_PIC.sit = @va_Site_in
		...
		@va_HeureFin_out			= EXP..ACT_GEN.heure_fin
			from EXP..ACT_PIC, EXP..ACT_GEN
				where	EXP..ACT_GEN.numero = @va_NumAction_in and 
						...
						EXP..ACT_PIC.actio = @va_NumAction_in and 
						EXP..ACT_PIC.sit = @va_Site_in

	select @vl_NbEnrg = @@rowcount

	/* si les infos ne sont pas trouvees     */
	/* alors les rechercher dans la base HIS */
	if @vl_NbEnrg = 0
	begin
		exec	@vl_Status = SQL_HIS.PRC..XZAC8051
				@va_NumAction_in,
				@va_Site_in,
