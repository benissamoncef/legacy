/*E*/
/*  Fichier : $Id: xzac13sp.prc,v 1.11 2020/10/13 17:18:36 pc2dpdy Exp $      Release : $Revision: 1.11 $        Date : $Date: 2020/10/13 17:18:36 $
------------------------------------------------------
...
* JMG	21/09/99	: ajout autoroute 1.9
...
as
	...
	if @va_NumEvt_in = null or @va_CleEvt_in = null or @va_Operateur_in = null or
	   @va_PR_in = null or @va_Sens_in = null or @va_HoroDebut_in = null or
	   @va_Autoroute_in = null
		return XDC_ARG_INV
	...
	/*A recherche de l'autoroute : derniere horodate de validation */
	select 
...
begin tran
		...
		values( @va_NumAction_out, @va_NumEvt_in, @va_CleEvt_in, 
			XDC_ACT_PMV_FOURGON, @va_HoroDebut_in,
...
commit tran
		...
		if exists (select * from EXP..ACT_GEN
			...
			insert EXP..ACT_FRG (actio, PR, sens, message, sit, autoroute)
			values (@va_NumAction_out, @va_PR_in, @va_Sens_in, @va_Message_in, @va_Site_in, @va_Autoroute_in)
		else	return XDC_NOK
	end
	else
	begin
		...
		if not exists (select * from EXP..ACT_GEN, EXP..ACT_FRG
				where	EXP..ACT_GEN.numero = @va_NumAction_in and 
					...
					EXP..ACT_FRG.actio = @va_NumAction_in and 
		...
		update EXP..ACT_FRG set sens = @va_Sens_in, message = @va_Message_in, PR = @va_PR_in,
