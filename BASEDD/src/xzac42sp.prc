/*E*/
/*  Fichier : $Id: xzac42sp.prc,v 1.11 2010/09/02 09:48:21 gesconf Exp $      Release : $Revision: 1.11 $        Date : $Date: 2010/09/02 09:48:21 $
------------------------------------------------------
...
* C.T.	14/11/94	: Modif recherche infos pour CI (V 1.7)
* C.T.	09/12/94	: Modif clause de lecture d'action (V 1.8)
*                         non teste
* C.P.	10/06/98	: Separation EXP et HIS (V 1.9)
* JPL	21/07/10	: Au CI ou sur secteur, si l'action est introuvable, executer sur HIS (DEM 941) 1.10
* JPL	01/09/10	: ... et retour compte-rendu OK si l'evenement est trouve 1.11
-----------------------------------------------------*/
...
as
	...
	select
			...
			@va_Manuelle_out	= HIS..ACT_TUB.manuelle,
	...
	where	HIS..ACT_GEN.numero = @va_NumeroAction_in and 
			...
			HIS..ACT_TUB.actio = @va_NumeroAction_in and 
			HIS..ACT_TUB.sit = @va_Site_in
	...
	select
		...
		@va_Manuelle_out		= EXP..ACT_TUB.manuelle,
	...
	where	EXP..ACT_GEN.numero = @va_NumeroAction_in and 
		...
		EXP..ACT_TUB.actio = @va_NumeroAction_in and 
		EXP..ACT_TUB.sit = @va_Site_in
	...
	if @vl_NbEnrg = 0
	begin
		exec @vl_Status = SQL_HIS.PRC..XZAC;42
					@va_NumeroAction_in,
					@va_Site_in,
