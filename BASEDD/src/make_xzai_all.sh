...
#  Compile les procedures stockees de XZAI sur le serveur d'un site donne.
#
#############################################################################
#
# Service rendu
...
Fichier_Compil="xzai${Site_Compil}"
...
{
    echo " "
    echo -e '\E[47;31m'"\033[1mla commande xzai_make.sh a ete  modifiee.\033[0m"
    echo " xzai_make.sh appelle  le script ${Nom_Prog} qui prend en compte les specificites de l'environement de developpement et celles de production."
    echo " " 
    ...
    echo " xzai_make.sh appelle  le script (${Nom_Prog} ) pour prendre en compte les specificites de l'environement de developpement.(directive DEV)"
    ...
    echo -e '\E[47;34m'"\033[1m Exemple: procedure pour le site CI plateforme de DEV  (via xzai_make.sh): $0 CI DEV \033[0m"
    ...
    echo " ${Nom_Prog} genere le fichier .sql a livrer en production via le script sae_liv <num_jira> -p xzai_make.sh  <site> "
...
echo "if exists (select 1 from sysobjects where name = 'XZAI' and type = 'P')" >> ${fic_tmp_c}

echo "drop procedure XZAI " >> ${fic_tmp_c} 
echo "go" >> ${fic_tmp_c}
...
# procedures pas sur HIS
if [[ "$Site_Compil" != "HIS" ]]
then

    cat_proc "xzai02sp.prc"
    cat_proc "xzai03sp.prc"
    cat_proc "xzai04sp.prc"
    cat_proc "xzai05sp.prc"
    cat_proc "xzai06sp.prc"
    cat_proc "xzai07sp.prc"
    cat_proc "xzai08sp.prc"

    # les degroupees
    sae_dev_make_prc "xzai400sp.prc" $Site_Compil $cible 
    sae_dev_make_prc "xzai501sp.prc" $Site_Compil $cible
    sae_dev_make_prc "xzai502sp.prc" $Site_Compil $cible
    sae_dev_make_prc "xzai601sp.prc" $Site_Compil $cible
   sae_dev_make_prc "xzai500sp.prc" $Site_Compil $cible
else
    
    sae_dev_make_prc "xzai600sp.prc" $Site_Compil $cible 
