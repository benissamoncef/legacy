/*E*/
/*  Fichier : $Id: xzat20sp.prc,v 1.12 2021/03/08 13:40:47 pc2dpdy Exp $      Release : $Revision: 1.12 $        Date : $Date: 2021/03/08 13:40:47 $
------------------------------------------------------
...
*                         non teste
* C.T.	13/09/96	: Ajout cas des serveurs DC et DS (RADT 1.6)
* C.P.	08/01/98	: Correction gestion multi-sites dans 
*			  les procs (V 1.7) (dem / 1536)
* PN	22/09/98	: Ajout cas sur serveurs DY DP (dem/1700) 1.8
* LCL   09/07/07        : Suppression DY 1.9
...
as
	...
	begin
		if @vl_Site = XDC_VC
			exec @vl_Status = SQL_VC.PRC..XZAT;20 
				@va_NumEqt_in, @va_TypeEqt_in
		else if @vl_Site = XDC_DP
			exec @vl_Status = SQL_DP.PRC..XZAT;20 
				@va_NumEqt_in, @va_TypeEqt_in
		else
			return XDC_NOK
...
*/		where numero = @vl_Site)
	...
	begin tran
		...
		@vl_Majeure, @vl_Mineure,  XDC_OUI, @vl_Inhibe, @vl_Horodate)
	...
	select	@vl_EtatEqt =	XDC_EQT_HS + XDC_EQT_DESACTIVE +
