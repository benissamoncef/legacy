/*E*/
/*Fichier :  $Id: xzac121sp.prc,v 1.3 2010/09/02 09:49:22 gesconf Exp $      Release : $Revision: 1.3 $        Date : $Date: 2010/09/02 09:49:22 $
------------------------------------------------------
* STERIA *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME  BASEDD
------------------------------------------------------
* FICHIER xzac121sp.prc
------------------------------------------------------
...
* 
* Sequence d'appel
* SP	XZAC121_Lire_Commande_Eclairage
* 
* Arguments en entree
...
drop procedure XZAC121
go

create procedure XZAC121
...
as
	...
	select
			...
			@va_Manuelle_out	= HIS..ACT_GTC_ECL.manuelle,
	...
	where	HIS..ACT_GEN.numero = @va_NumeroAction_in and 
			...
			HIS..ACT_GTC_ECL.actio = @va_NumeroAction_in and 
			HIS..ACT_GTC_ECL.sit = @va_Site_in
	...
	select
		...
		@va_Manuelle_out		= EXP..ACT_GTC_ECL.manuelle,
	...
	where	EXP..ACT_GEN.numero = @va_NumeroAction_in and 
		...
		EXP..ACT_GTC_ECL.actio = @va_NumeroAction_in and 
		EXP..ACT_GTC_ECL.sit = @va_Site_in
	...
	if @vl_NbEnrg = 0
	begin
		exec @vl_Status = SQL_HIS.PRC..XZAC121
					@va_NumeroAction_in,
					@va_Site_in,
