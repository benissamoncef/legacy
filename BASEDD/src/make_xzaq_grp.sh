...
#  Compile les procedures stockees de XZAQ sur le serveur d'un site donne.
#
#############################################################################
#
# Service rendu
...
Fichier_Compil="xzaq${Site_Compil}"
...
{
    echo " "
    echo -e '\E[47;31m'"\033[1mla commande xzaq_make.sh a ete  modifiee.\033[0m"
    echo " xzaq_make.sh appelle  le script ${Nom_Prog} qui prend en compte les specificites de l'environement de developpement et celles de production."
    echo " " 
    ...
    echo " xzaq_make.sh appelle  le script (${Nom_Prog} ) pour prendre en compte les specificites de l'environement de developpement.(directive DEV)"
    ...
    echo -e '\E[47;34m'"\033[1m Exemple: procedure pour le site CI plateforme de DEV  (via xzaq_make.sh): $0 CI DEV \033[0m"
    ...
    echo " ${Nom_Prog} genere le fichier .sql a livrer en production via le script sae_liv <num_jira> -p xzaq_make.sh  <site> "
...
case $Site_Compil in
    ...
    *)
		echo_err "ce $0  ne se compile que sur le site HIS. "
...
echo "go" >> ${fic_tmp_c}
echo "drop procedure XZAQ" >> ${fic_tmp_c}
echo "go" >> ${fic_tmp_c}
...
cat_proc "xzaq01sp.prc"
cat_proc "xzaq02sp.prc"



# precompil de la proc groupe
echo "cpp -E -P -C  ${directive_compil_Plateforme}  ${fic_tmp_c}  > ${fic_tmp_sql}"
