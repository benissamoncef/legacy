/*E*/
/* Fichier : $Id: xzao830sp.prc,v 1.2 2018/01/09 15:20:37 pc2dpdy Exp $     Release : $Revision: 1.2 $        Date : $Date: 2018/01/09 15:20:37 $
------------------------------------------------------
* STERIA *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME  BASEDD
------------------------------------------------------
* MODULE XZAO * FICHIER xzao830sp.prc
------------------------------------------------------
* HISTORIQUE :
*
...
/* static char *version = " $Id: xzao830sp.prc,v 1.2 2018/01/09 15:20:37 pc2dpdy Exp $ $Revision: 1.2 $ $Date: 2018/01/09 15:20:37 $: xzao830" ; */
...
if exists (select * from sysobjects where name = 'XZAO830' and type = 'P')
	drop procedure XZAO830
go


create procedure XZAO830
   @va_Basedd_in        	char(3),
		...
		where type_saga = XDC_SAGA_PMVA)
            ...
            */
            ...
            begin
                ...
                select @vl_NumZone = numero_zone from CFG..ZON_PMA_SAGA
               ...
               */
               ...
               begin
                  ...
                  */
                  ...
                  select @vl_NumZone = max(numero_zone) from CFG..ZON_PMA_SAGA

                  if @vl_NumZone = null
                    select @vl_NumZone = 0  
            
                  insert into CFG..ZON_PMA_SAGA (
			numero_zone,
			numero_pmva,
			autoroute_pmva,
			sens_pmva,
		...
		and numero in (select numero from CFT..EQT_SAGA
            ...
            */
            ...
            begin
                ...
                select @vl_NumZone = numero_zone from CFT..ZON_PMA_SAGA
                  where numero_pmva =  @vl_Numero and 
			autoroute_pmva = @vl_Autoroute and
			sens_pmva = @va_SensPMV_in and
			PR_pmva = @va_PrPMV_in and 
               ...
               */
               ...
               begin
                  ...
                  */
                  ...
                  select @vl_NumZone = max(numero_zone) from CFT..ZON_PMA_SAGA

                  if @vl_NumZone = null
                    select @vl_NumZone = 0  
            
                  insert into CFT..ZON_PMA_SAGA (
			numero_zone,
			numero_pmva,
			autoroute_pmva,
			sens_pmva,
