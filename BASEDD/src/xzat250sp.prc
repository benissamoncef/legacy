/*E*/
/*  Fichier : 	$Id: xzat250sp.prc,v 1.3 2021/03/08 13:36:37 pc2dpdy Exp $ Release : $Revision: 1.3 $ Date : $Date: 2021/03/08 13:36:37 $
------------------------------------------------------
...
* JPL	29/05/18	: Suppression prealable de la procedure stockee si elle existe  1.2
* LCL	21/01/21	: MOVIS PRA DEM-SAE93 1.3
-----------------------------------------------------*/
...
if exists (select * from sysobjects where name = 'XZAT250' and type = 'P')
	drop procedure XZAT250
go

create procedure XZAT250
...
as
	...
	where numero = @va_NumAction_in and sit = @vl_Site and type = XDC_ACT_PMVA
	...
	begin
		...
		update EXP..ACT_GEN set heure_fin  = @va_Horodate_in
		where equipement = @vl_NumEqt and type = XDC_EQT_PMVA and sit = @vl_Site and
	...
	begin
		...
		update EXP..ACT_GEN set heure_fin = @va_Horodate_in
		where 	numero < @va_NumAction_in and sit = @vl_Site  and
			type = XDC_ACT_PMVA and equipement = @vl_NumEqt and
			heure_fin is null
