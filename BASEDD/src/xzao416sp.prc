/*E*/
/*Fichier : $Id: xzao416sp.prc,v 1.7 2018/07/11 11:24:38 pc2dpdy Exp $      Release : $Revision: 1.7 $        Date : $Date: 2018/07/11 11:24:38 $
------------------------------------------------------
* STERIA *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME  BASEDD
------------------------------------------------------
* MODULE XZAO * FICHIER xzao416sp.prc
------------------------------------------------------
------------------------------------------------------
* HISTORIQUE :
* PNI	29/01/2008 : Suppression du crit�re de site pour l'ajout/modif d'une zone v1.4 DEM743
------------------------------------------------------
...
/* static char *version = " $Id: xzao416sp.prc,v 1.7 2018/07/11 11:24:38 pc2dpdy Exp $: xzao416" ; */
...
use PRC
go
drop procedure XZAO416
go


create procedure XZAO416
   @va_Basedd_in        	char(3),
   ...
   */
   ...
   begin
		...
		and sens = @va_SensPMV_in
            ...
            */
            ...
            begin
                ...
                select @vl_NumEqt = numero_zone from CFG..ZON_PMV
               ...
               */
               ...
               begin
                  ...
                  */
                  ...
                  select @vl_NumEqt = max(numero_zone) from CFG..ZON_PMV

                  if @vl_NumEqt = null
                    select @vl_NumEqt = 0  
            
                  insert into CFG..ZON_PMV(numero_zone,numero_pmv,autoroute_pmv,sens_pmv,PR_pmv,
		...
		* la zone d'influence existe dans ZON_PMV	
	    ...
	    select @vl_autoroute_1_pmv_amont = null
	    select @vl_autoroute_1_pmv_amont = numero from CFT..RES_AUT
	    where nom = @va_autoroute_1_pmv_amont

	    select @vl_autoroute_1_pmv_amont_bis = null
	    select @vl_autoroute_1_pmv_amont_bis = numero from CFT..RES_AUT
	    where nom = @va_autoroute_1_pmv_amont_bis

	    select @vl_autoroute_1_pmv_amont_ter = null
	    select @vl_autoroute_1_pmv_amont_ter = numero from CFT..RES_AUT
	    where nom = @va_autoroute_1_pmv_amont_ter
		...
		and sens = @va_SensPMV_in
            ...
            */
            ...
            begin
                ...
                select @vl_NumEqt = numero_zone from CFT..ZON_PMV
                  where numero_pmv =  @vl_Numero and 
			autoroute_pmv = @vl_Autoroute and
			sens_pmv = @va_SensPMV_in and
			PR_pmv = @va_PrPMV_in and 
               ...
               */
               ...
               begin
                  ...
                  */
                  ...
                  select @vl_NumEqt = max(numero_zone) from CFT..ZON_PMV

                  if @vl_NumEqt = null
                    select @vl_NumEqt = 0  
            
                  insert into CFT..ZON_PMV(numero_zone,numero_pmv,autoroute_pmv,sens_pmv,PR_pmv,site_pmv,
