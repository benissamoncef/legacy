/*E*/
/*  Fichier : $Id: xzac06sp.prc,v 1.17 2021/03/08 14:38:35 pc2dpdy Exp $      Release : $Revision: 1.17 $        Date : $Date: 2021/03/08 14:38:35 $
------------------------------------------------------
...
create procedure XZAC;06
	...
	begin
		...
		begin
			exec @vl_Status = SQL_CI.PRC..XZAC;06  @va_NumEvt_in,  @va_CleEvt_in, @va_PosteOperateur_in,  @va_NumEqt_in,  @va_HeureLancement_in, @va_Manuelle_in,  @va_Priorite_in,
			@va_DistanceEvt_in,  @va_Sequence_in, @va_NumeroAction_out, XDC_SQLCI
			return @vl_Status
		end
		if @vl_serveur_pra= XDC_SQLDP
		begin	 
			exec @vl_Status = SQL_DS.PRC..XZAC;06  @va_NumEvt_in,  @va_CleEvt_in, @va_PosteOperateur_in,  @va_NumEqt_in,  @va_HeureLancement_in, @va_Manuelle_in, @va_Priorite_in,
			@va_DistanceEvt_in,  @va_Sequence_in, @va_NumeroAction_out, XDC_SQLVC
			return @vl_Status
		end
	end
...
/*DEM706	if not exists ( select equipement from EXP..EQT_DSP
	...
	begin tran
		...
		XDC_ACT_TUNNEL,  @va_NumEqt_in, @va_HeureLancement_in, @va_Priorite_in, 
		XDC_NON, @va_DistanceEvt_in, @va_PosteOperateur_in, @vl_Site)

		insert EXP..ACT_TUB (actio, tube, sit, sequence, manuelle)
	...
	begin
		...
		*/
		if @vl_Site = XDC_CI
			exec @vl_Status = TASRV_CI...ACRP06 @va_NumEqt_in, @va_Sequence_in, @va_NumeroAction_out
		else /* PRA
		     if @@servername = XDC_SQLDP
		     */
		    if @vl_Site = XDC_DP
				exec @vl_Status = TASRV_DP...ACRP06 @va_NumEqt_in, @va_Sequence_in, @va_NumeroAction_out
		else /* PRA
		     if  @@servername = XDC_SQLVC
		     */
		    if @vl_Site = XDC_VC
				exec @vl_Status = TASRV_VC...ACRP06 @va_NumEqt_in, @va_Sequence_in, @va_NumeroAction_out
		else 
		    if @vl_Site = XDC_CA
				exec @vl_Status = TASRV_VC...ACRP06 @va_NumEqt_in, @va_Sequence_in,	@va_NumeroAction_out
	...
	where	EXP..ACT_GEN.equipement = @va_NumEqt_in and 
		...
		(select max(heure_lancement) from EXP..ACT_GEN
		 where	equipement = @va_NumEqt_in and type = XDC_ACT_TUNNEL and
	...
	begin
		...
		begin
			...
			exec XZAC;60 XDC_EQT_TUBE, @vl_Autoroute, @vl_PR, @vl_Sens, @vl_Intitule output
