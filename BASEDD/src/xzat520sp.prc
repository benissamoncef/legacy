/*E*/
/* Fichier : $Id: xzat520sp.prc,v 1.2 2021/03/08 13:15:46 pc2dpdy Exp $        Release : $Revision: 1.2 $        Date : $Date: 2021/03/08 13:15:46 $
------------------------------------------------------
...
if exists (select * from sysobjects where name = 'XZAT520' and type = 'P')
	drop procedure XZAT520
go

create procedure XZAT520
...
as
	...
	where numero = @va_NumAction_in and sit = @vl_Site and type = XDC_ACT_PRV
	...
	begin
		...
		update EXP..ACT_GEN set heure_fin  = @va_Horodate_in
		where equipement = @vl_NumEqt and type = XDC_EQT_PRV and sit = @vl_Site and
	...
	begin
		...
		update EXP..ACT_GEN set heure_fin = @va_Horodate_in
		where 	numero < @va_NumAction_in and sit = @vl_Site  and
			type = XDC_ACT_PRV and equipement = @vl_NumEqt and
			heure_fin is null
