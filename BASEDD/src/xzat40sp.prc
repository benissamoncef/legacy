/*E*/
/*Fichier :  $Id: xzat40sp.prc,v 1.4 2021/03/08 13:18:09 pc2dpdy Exp $      Release : $Revision: 1.4 $        Date : $Date: 2021/03/08 13:18:09 $
------------------------------------------------------
...
* JMG	22/10/09 : prise en compte nature DEM895
* LCL	22/01/21	: MOVIS PRA DEM-SAE93 1.4
-----------------------------------------------------*/

/*X*/
/*-----------------------------------------------------
* Sequence d'appel
* SP    XZAT40_CompteRendu_SIG
* 
* Arguments en entree
...
drop procedure XZAT40
go

create procedure XZAT40
...
as
    declare @vl_Status      int, 
	    ...
	    ,@vl_nature	tinyint, @vl_sequence tinyint, @vl_numero int
    ...
    select @vl_Site = numero 
        ...
        where numero=@va_NumAction_in and
	      (type   = XDC_ACT_GTC_SIG or type   = XDC_ACT_GTC_SIG_ECH) and
    ...
    where  numero = @va_NumAction_in 
      and  sit    = @vl_Site 
      and  (type   = XDC_ACT_GTC_SIG or type   = XDC_ACT_GTC_SIG_ECH) 
	...
	select @vl_numero  = EXP..ACT_GEN.numero
	from   EXP..ACT_GEN,
	      CFG..EQT_SEQ_GTC_SIG,
	     EXP..ACT_GTC_SIG
        where  EXP..ACT_GEN.equipement =  @vl_NumEqt 
          and  (EXP..ACT_GEN.type       =  XDC_ACT_GTC_SIG or EXP..ACT_GEN.type =  XDC_ACT_GTC_SIG_ECH)
          and  EXP..ACT_GEN.sit        =  @vl_Site
          and  heure_fin  is null
          and  EXP..ACT_GEN.numero     <  @va_NumAction_in
	  and  CFG..EQT_SEQ_GTC_SIG.nature_seq = @vl_nature
	  and  EXP..ACT_GEN.numero = EXP..ACT_GTC_SIG.actio
	  and  EXP..ACT_GEN.sit = EXP..ACT_GTC_SIG.sit
	  and  EXP..ACT_GTC_SIG.sequence = CFG..EQT_SEQ_GTC_SIG.numero
    ...
    begin
        ...
        where  EXP..ACT_GEN.numero = @va_NumAction_in
          ...
          and  CFG..EQT_GEN.type   = XDC_EQT_SIG
