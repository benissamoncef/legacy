/*E*/
/* Fichier : $Id: xzac900sp.prc,v 1.3 2021/03/08 14:14:08 pc2dpdy Exp $      Release : $Revision: 1.3 $        Date : $Date: 2021/03/08 14:14:08 $
------------------------------------------------------
...
if exists (select * from sysobjects where name = 'XZAC900' and type = 'P')
	drop procedure XZAC900
go

create procedure XZAC900
...
as
	...
	begin
		if @vl_serveur_pra=XDC_SQLDP
			exec @vl_Status = SQL_DS.PRC..XZAC900 @va_Horodate_in, @va_NumEvt_in, @va_CleEvt_in,
			...
			exec @vl_Status = SQL_DM.PRC..XZAC900 @va_Horodate_in, @va_NumEvt_in, @va_CleEvt_in,
						@va_Operateur_in, @va_NumEqt_in, @va_Site_in,
						@va_PlanDeFeux_in,
						@va_LibellePlanDeFeux_in,
						@va_Priorite_in,
			...
			exec @vl_Status = SQL_CI.PRC..XZAC900 @va_Horodate_in, @va_NumEvt_in, @va_CleEvt_in,
						@va_Operateur_in, @va_NumEqt_in, @va_Site_in,
						@va_PlanDeFeux_in,
						@va_LibellePlanDeFeux_in,
						@va_Priorite_in,
	...
	from	EXP..EQT_DSP
	where	type = XDC_EQT_CFE  and  equipement = @va_NumEqt_in  and
		sit = @vl_Site  and
		dernier = 1
	...
	where	EXP..ACT_GEN.equipement = @va_NumEqt_in and 
		...
		(select max(heure_lancement) from EXP..ACT_GEN
		 where	equipement = @va_NumEqt_in and type = XDC_ACT_CFE and
	...
	begin
		...
		begin
			...
			exec 		XZAC;60 XDC_EQT_CFE, @vl_Autoroute, @vl_PR, @vl_Sens, @vl_Intitule output
	...
	if @vl_EtatPbMineur <> 0
	begin
		exec PRC..XZAA;01 @va_Horodate_in, XDC_EQT_CFE, @va_NumEqt_in, XZAAC_AFF_DIF, "Affichage CFE"
		return XDC_NOK
	end
#endif
