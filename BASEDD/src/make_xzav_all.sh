...
#  Compile les procedures stockees de xzav sur le serveur d'un site donne.
#
#############################################################################
#
# Service rendu
...
Fichier_Compil="xzax${Site_Compil}"
...
{
    echo " "
    echo -e '\E[47;31m'"\033[1mla commande xzax_make.sh a ete  modifiee.\033[0m"
    echo " xzax_make.sh appelle  le script ${Nom_Prog} qui prend en compte les specificites de l'environement de developpement et celles de production."
    echo " " 
    ...
    echo " xzax_make.sh appelle  le script (${Nom_Prog} ) pour prendre en compte les specificites de l'environement de developpement.(directive DEV)"
    ...
    echo -e '\E[47;34m'"\033[1m Exemple: procedure pour le site CI plateforme de DEV  (via xzax_make.sh): $0 CI DEV \033[0m"
    ...
    echo " ${Nom_Prog} genere le fichier .sql a livrer en production via le script sae_liv <num_jira> -p xzax_make.sh  <site> "
...
echo "if exists (select 1 from sysobjects where name = 'XZAX' and type = 'P')" >> ${fic_tmp_c}

echo "drop procedure XZAX " >> ${fic_tmp_c} 
echo "go" >> ${fic_tmp_c}
...
# degroupees
sae_dev_make_prc "xzav02sp.prc" $Site_Compil $cible

if [[ $1 != "HIS" ]]                                                                          
then    
sae_dev_make_prc "xzav01sp.prc" $Site_Compil $cible
sae_dev_make_prc "xzav04sp.prc" $Site_Compil $cible
fi
