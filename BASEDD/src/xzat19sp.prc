/*E*/
/*  Fichier : $Id: xzat19sp.prc,v 1.14 2021/03/08 13:40:53 pc2dpdy Exp $      Release : $Revision: 1.14 $        Date : $Date: 2021/03/08 13:40:53 $
------------------------------------------------------
...
*                         non teste
* B.G.	30/08/95	: leve inhibition sur reactivation (1.6)
* C.T.	13/09/96	: Ajout cas des serveurs DS et DC (RADT 1.7)
* C.P.	07/01/98	: Correction gestion multi-sites dans 
*			  les procs (V 1.8) (dem / 1536)
* C.T.	29/04/98	: Correction gestion multi-sites (V 1.9) 
* PN	22/09/98	: Ajout cas sur serveurs DY DP (dem/1700) 1.10
* LCL   09/07/07	: Suppression DY 1.11
* JMG   02/10/09	: secto DEM887
* JMG   09/03/17	: REGIONALISATION - DEM1220 1.13
* LCL	22/01/21	: MOVIS PRA DEM-SAE93 1.14
...
as
	...
	begin
		if @vl_Site = XDC_VC
			exec @vl_Status = SQL_VC.PRC..XZAT;19 
				@va_NumEqt_in, @va_TypeEqt_in
		else if @vl_Site = XDC_DP
			exec @vl_Status = SQL_DP.PRC..XZAT;19 
				@va_NumEqt_in, @va_TypeEqt_in
		else
			return XDC_NOK
...
*/		where numero = @vl_Site)
	...
	begin tran
		...
		values(@va_NumEqt_in, @va_TypeEqt_in, @vl_Site, @vl_Critique, XDC_NON,
		@vl_Majeure, @vl_Mineure,  @vl_HS, XDC_NON, @vl_Horodate)
	...
	update EXP..EQT_ACV set activable = XDC_NON
	...
	select	@vl_EtatEqt =	XDC_EQT_HS * @vl_HS + 
		...
		@vl_Changt = XDC_EQT_DESACTIVE + XDC_EQT_INHIBE * @vl_Inhibe

	if @vl_NumMaitre != null
	begin
		/*A activation de l'equipement */
		if @vl_Site = XDC_CI
