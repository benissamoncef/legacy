/*E*/
/* Fichier : $Id: xzao435sp.prc,v 1.8 2017/11/15 12:52:16 devgfi Exp $        Release : $Revision: 1.8 $        Date : $Date: 2017/11/15 12:52:16 $
------------------------------------------------------
...
* PNI	30/08/13 : Correctifs pour A57 v1.6DEM1077
* PNI	26/09/16 : ajout inhibition=0 v1.7 DEM1187
...
if exists (select 1 from sysobjects where name = 'XZAO435' and type = 'P')
	drop procedure XZAO435
go


create procedure XZAO435
...
if (@va_sens = 1 and @va_autoroute!=8) or ((@va_sens = 2 ) and @va_autoroute=8)
begin
...
if (@va_sens = 2 and @va_autoroute!=8) or ((@va_sens = 1 ) and @va_autoroute=8)
begin
...
begin
	...
	insert into #TMP_VAL select  T.temps
	from 	EXP..ZDP_6MN T, 
		CFG..EQT_ZDP Z,
		CFG..EQT_GEN G
	where	G.type = 39
	...
	insert into #TMP_VAL select Z.TDP_habituel
	from 	EXP..ZDP_6MN T, 
		CFG..EQT_ZDP Z,
		CFG..EQT_GEN G
	where	G.type = 39
...
begin
        if (@vl_num_zone_destination<>@vl_num_zone_origine ) or ( @va_sens=XDC_SENS_SUD /* and @va_autoroute!=8) or (@va_sens=XDC_SENS_NORD and @va_autoroute=8 */ )
        begin
...
begin
        if (@vl_num_zone_destination<>@vl_num_zone_origine ) or ( @va_sens=XDC_SENS_NORD /* and @va_autoroute!=8) or (@va_sens=XDC_SENS_SUD and @va_autoroute=8 */ )
        begin
...
insert into #TMP_TDP values(@va_sens, @va_zone_origine, @va_zone_destination, @vl_tdp1_1, @vl_horo,
			@va_autoroute)
select * from #TMP_TDP
delete from #TMP_VAL
end
