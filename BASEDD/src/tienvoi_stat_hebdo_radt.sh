...
###############################################################################
## Fichier : tienvoi_stat_hebdo_radt.sh	                 Date : 29/10/21
###############################################################################
## ESCOTA * PROJET MIGRAZUR
###############################################################################
## SOUS-SYSTEME  GESIHM / OUTILS
###############################################################################
## MODULE MTOU * FICHIER tienvoi_stat_hebdo_radt.sh
###############################################################################
## DESCRIPTION DU MODULE :
##
##   Outil de production et envoi hebdomadaire par e-mail
...
##   une commande de diffusion par e-mail du document a un groupe
##   de destinataires donnes.
##
###############################################################################
## HISTORIQUE :
##
## JPL	29/10/2021 : Creation  1.1
## JPL	08/11/2021 : Parametres de la commande sur serveur passerelle  1.2
##
###############################################################################
...
RACINE_FICH_STAT=Statistiques-RADT-Hebdo
...
###############################################################################
##
##  FONCTION     : Produire_Statistiques_Hebdo_RADT
##
##  ARGUMENTS    :
##    Aucun
##
...
Produire_Statistiques_Hebdo_RADT ()
{
	rm -f ${FICHIER_STAT}
	...
	{
		echo ";;;;;Rapport hebdomadaire d'état de fonctionnement des stations RADT en date du ${AUJOURD_HUI}"
		echo ""
	} > ${FICHIER_STAT}

	# Effectuer la requete en base pour extraire les resultats
	{
		echo exec PRC..FM_Statistiques_Hebdo_RADT '"'${AUJOURD_HUI}'"', NULL
		echo "go"
	}  | isql -SSQL_HIS  -UMIGRAZUR -PMIGRAZUR  -w2000  -s ';'  |
	     sed -e '/^(/d'  -e '/^.---/d'   >> ${FICHIER_STAT}
}
...
# commander sur ce serveur la diffusion du document aux destinataires
# -------------------------------------------------------------------
if [[ $CR_CMD -eq 0 ]]; then
	Txt_Err_Transfert=$(xzsx_cmd_sur.sh "${SERVEUR_PROTO}"  ". ./.profile >&- 2>&- ; python exec/tisendmail_stat_radt.py /tmp/${NOM_FICH_STAT} >/dev/null" 2>&1)
	CR_CMD=$?
	TXT_CR_CMD=$(echo $Txt_Err_Transfert | head -1)

	xzsx_cmd_sur.sh "${SERVEUR_PROTO}"  "rm -f /tmp/${NOM_FICH_STAT}"
fi
