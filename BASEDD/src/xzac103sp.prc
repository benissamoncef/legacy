/*E*/
/*Fichier :  $Id: xzac103sp.prc,v 1.11 2021/03/08 14:38:02 pc2dpdy Exp $      Release : $Revision: 1.11 $        Date : $Date: 2021/03/08 14:38:02 $
------------------------------------------------------
...
* LCL	26/09/23   : Adaptation CNA DEM-483
-----------------------------------------------------*/

/*X*/
/*-----------------------------------------------------
* Service rendu
* Action de configuration des equipements domaine Energie par sequence.
* 
* Sequence d'appel
* SP	XZAC103_Commande_Seq_ENR
* 
* Arguments en entree
...
* 
* Inserer dans la table ACTION et la table AFFICHAGE_ENR.
...
* 	ACRP43 : Appeler XZET03 pour commander l'equipement
-------------------------------------------------*/

use PRC
go

create procedure XZAC;103
...
as
	...
	begin
		...
		if @vl_serveur_pra= XDC_SQLVC
			exec @vl_Status = SQL_DM.PRC..XZAC;103 @va_NumEvt_in,@va_CleEvt_in,@va_PosteOperateur_in,@va_TypEqt_in,@va_NumEqt_in,@va_HeureLancement_in,@va_Manuelle_in,@va_Priorite_in,@va_DistanceEvt_in,@va_Sequence_in,@va_NumeroAction_out, @va_NomSiteLocal_in, XDC_OUI
		
		/* modif ABE du 05-02-2020 : sp�cificit� toulon */
		else if @vl_serveur_pra= XDC_SQLDP
			exec @vl_Status = SQL_DS.PRC..XZAC;103 @va_NumEvt_in,@va_CleEvt_in,@va_PosteOperateur_in,@va_TypEqt_in,@va_NumEqt_in,@va_HeureLancement_in,@va_Manuelle_in,@va_Priorite_in,@va_DistanceEvt_in,@va_Sequence_in,@va_NumeroAction_out, @va_NomSiteLocal_in, XDC_OUI

		else if @vl_serveur_pra= XDC_SQLCI
			exec @vl_Status = SQL_CI.PRC..XZAC;103 @va_NumEvt_in,@va_CleEvt_in,@va_PosteOperateur_in,@va_TypEqt_in,@va_NumEqt_in,@va_HeureLancement_in,@va_Manuelle_in,@va_Priorite_in,@va_DistanceEvt_in,@va_Sequence_in,@va_NumeroAction_out, @va_NomSiteLocal_in, XDC_OUI
	...
	begin
		...
		*/
		if @vl_Site = XDC_CI
			exec @vl_Status = TASRV_CI...ACRP43 @va_TypEqt_in,
												@va_NumEqt_in,
			                                    @va_Sequence_in,
			                                    @va_NumeroAction_out
		else /* PRA
		     ...
		     */
		    if @vl_Site = XDC_DP
				exec @vl_Status = TASRV_DP...ACRP43 @va_TypEqt_in,
													@va_NumEqt_in, 
													@va_Sequence_in,
													@va_NumeroAction_out
		else /* PRA
		     ...
		     */
		    if @vl_Site = XDC_VC
				exec @vl_Status = TASRV_VC...ACRP43 @va_TypEqt_in,
													@va_NumEqt_in,
													@va_Sequence_in,
													@va_NumeroAction_out
		else 
		    if @vl_Site = XDC_CA
				exec @vl_Status = TASRV_CA...ACRP43 @va_TypEqt_in,
													@va_NumEqt_in,
													@va_Sequence_in,
													@va_NumeroAction_out
