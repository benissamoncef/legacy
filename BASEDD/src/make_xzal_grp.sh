...
#  Compile les procedures stockees de xzal sur le serveur d'un site donne.
#
#############################################################################
#
# Service rendu
...
Fichier_Compil="xzal${Site_Compil}"
...
{
    echo " "
    echo -e '\E[47;31m'"\033[1mla commande xzal_make.sh a ete  modifiee.\033[0m"
    echo " xzal_make.sh appelle  le script ${Nom_Prog} qui prend en compte les specificites de l'environement de developpement et celles de production."
    echo " " 
    ...
    echo " xzal_make.sh appelle  le script (${Nom_Prog} ) pour prendre en compte les specificites de l'environement de developpement.(directive DEV)"
    ...
    echo -e '\E[47;34m'"\033[1m Exemple: procedure pour le site CI plateforme de DEV  (via xzal_make.sh): $0 CI DEV \033[0m"
    ...
    echo " ${Nom_Prog} genere le fichier .sql a livrer en production via le script sae_liv <num_jira> -p xzal_make.sh  <site> "
...
echo "if exists (select 1 from sysobjects where name = 'XZAL' and type = 'P')" >> ${fic_tmp_c}

echo "drop procedure XZAL " >> ${fic_tmp_c} 
echo "go" >> ${fic_tmp_c}
...
then
    # les groupees
    cat_proc "xzal02sp.prc"
    cat_proc "xzal04sp.prc"

else

    cat_proc "xzal01sp.prc"
    cat_proc "xzal03sp.prc"
