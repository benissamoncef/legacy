/*E*/
/* Fichier : $Id: xzao256sp.prc,v 1.1 2012/07/04 11:28:56 pc2dpdy Exp $      Release : $Revision: 1.1 $        Date : $Date: 2012/07/04 11:28:56 $
------------------------------------------------------
* STERIA *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME  BASEDD
------------------------------------------------------
* MODULE XZAO * FICHIER xzao256sp.prc
------------------------------------------------------
* HISTORIQUE :
*
...
/* static char *version = " @(#)xzao256sp.prc	1.6 1.6 03/10/11: xzao256" ; */
...
* SERVICE RENDU : 
*
*  recherche les PRV en amont a un evenement
*
------------------------------------------------------
* SEQUENCE D'APPEL :
*/
...
if exists (select * from sysobjects where name = 'XZAO256' and type = 'P')
	drop procedure XZAO256
go


create procedure XZAO256
   ...
   declare @vl_distance_evt int,
	   ...
	   @vl_numero_zone smallint,
		@vl_numero_prv smallint,
		@vl_autoroute_prv tinyint,
		@vl_autoroute_zone tinyint,
		@vl_sens_prv tinyint,
		@vl_sens_prv_revu tinyint,
		@vl_sens_evt_revu tinyint,
		@vl_PR_prv int,
		@vl_direction_zone char(15),
		@vl_cr int, @vl_site_prv tinyint,
		@vl_doublon tinyint,
		@vl_sens_tmp tinyint,
   ...
   create table #TMP_PRV
   (   
    numero_zone                 smallint null,
    numero_prv                  smallint null,
    autoroute_prv               tinyint null,
    sens_prv                    tinyint null,
    PR_prv                      int    null,
    site_prv                    tinyint  null,
   ...
   create unique index prv_index on #TMP_PRV (numero_zone)
...
/**********************premiere passe : on vire les prv dans la perturbation ou non operationels ***************************/

	open prv

	fetch prv into @vl_numero_zone,
		@vl_autoroute_zone,
		@vl_numero_prv,
		@vl_autoroute_prv,
		@vl_sens_prv,
		...
		@vl_distance_equip
	      ...
	      if @vl_sens_prv=XDC_SENS_SORTIE_SUD or @vl_sens_prv=XDC_SENS_ENTREE_SUD
	      begin
		select @vl_sens_prv_revu = XDC_SENS_SUD
	      end
	      else if @vl_sens_prv=XDC_SENS_SORTIE_NORD or @vl_sens_prv=XDC_SENS_ENTREE_NORD
	      begin
		select @vl_sens_prv_revu = XDC_SENS_NORD
	      end
	      else
	      begin
		select @vl_sens_prv_revu = @vl_sens_prv
              end
	...
	while (@@sqlstatus = 0)
	  ...
	  if @vl_doublon = 0
	  begin
	    if ( @vl_autoroute_prv=@va_Autoroute_in and @vl_sens_prv_revu=@va_Sens_in and @vl_PR_prv between @vl_PR_min and @vl_PR_max)
	    begin
              update #TMP_PRV set doublon=3 where numero_prv = @vl_numero_prv  
            end
            else
	    begin
	      /* calcul de la distance entre le prv et l'evt. Si cette distance != 99999 alors le prv est en amont */
              exec @vl_cr = XZAO423 @vl_autoroute_prv   ,
		   		@vl_PR_prv          ,
				@vl_sens_prv_revu,
		      		@va_Autoroute_in   ,
			    	@vl_PR_debut_evt ,
		...
		begin
                	update #TMP_PRV set doublon=4 where numero_prv = @vl_numero_prv  
		end
	      end
	      /*else*/
		...
		select @vl_distance_evt=0
	      ...
	      begin
	        ...
	        from EXP..EQT_DSP
#endif
	        where type=XDC_EQT_PRV and dernier=1 and equipement=@vl_numero_prv
                if ( @vl_HS = XDC_VRAI or @vl_majeure=XDC_VRAI or @vl_critique=XDC_VRAI or
                     @vl_inhibe=XDC_VRAI or @vl_desactive=XDC_VRAI )
		begin
	          update #TMP_PRV set doublon = 2  where numero_prv = @vl_numero_prv
		end
		else
		begin
