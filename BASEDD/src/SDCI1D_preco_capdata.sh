...
go" | isql -SSQL_CI  -w2000 -UMIGRAZUR -PMIGRAZUR | grep "_" | grep -v rs_ | sort`
do
echo $t
echo "use CFG
go
...
go" | isql -SSQL_CI  -w2000 -UMIGRAZUR -PMIGRAZUR
done


echo "CFT"
for t in `echo "select name from CFT..sysobjects where type='U' 
go" | isql -SSQL_CI  -w2000 -UMIGRAZUR -PMIGRAZUR | grep "_" | grep -v rs_ | sort`
do
echo $t
echo "use CFT
