/*E*/
/* Fichier : $Id: xzao448sp.prc,v 1.1 2012/07/04 11:29:46 pc2dpdy Exp $      Release : $Revision: 1.1 $        Date : $Date: 2012/07/04 11:29:46 $
------------------------------------------------------
* STERIA *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME  BASEDD
------------------------------------------------------
* MODULE XZAO * FICHIER xzao448sp.prc
------------------------------------------------------
* HISTORIQUE :
*
...
/* static char *version = " @(#)xzao448sp.prc	1.6 1.6 03/10/11: xzao448" ; */
...
if exists (select * from sysobjects where name = 'XZAO448' and type = 'P')
	drop procedure XZAO448
go


create procedure XZAO448
...
/**********************premiere passe : on vire les prv dans la perturbation ou non operationels ***************************/
	...
	while (@@sqlstatus = 0)
	  ...
	  begin
	    ...
	    begin
              update #TMP_PRV set doublon=3 where numero_pmv = @vl_numero_pmv  
            end
            else
	    begin
		...
		begin
                	update #TMP_PRV set doublon=4 where numero_pmv = @vl_numero_pmv  
		end
	      end
	      /*else*/
		...
		select @vl_distance_evt=0
	      ...
	      begin
	        ...
	        from EXP..EQT_DSP
#endif
	        where type=XDC_EQT_PRV and dernier=1 and equipement=@vl_numero_pmv
                if ( @vl_HS = XDC_VRAI or @vl_majeure=XDC_VRAI or @vl_critique=XDC_VRAI or
                     @vl_inhibe=XDC_VRAI or @vl_desactive=XDC_VRAI )
		begin
	          update #TMP_PRV set doublon = 2  where numero_pmv = @vl_numero_pmv
		end
		else
		begin
