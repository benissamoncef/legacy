/*E*/
/*Fichier :  $Id: xzag13sp.prc,v 1.23 2020/09/22 08:03:32 pc2dpdy Exp $      Release : $Revision: 1.23 $        Date : $Date: 2020/09/22 08:03:32 $
------------------------------------------------------
...
* P.N.	27/01/99	: Ajouter les actions sur ACT_EXP ACT_EXPTFM ACT_PMVDAT et FMC_DAT et MES_ADA dem/1724  (V1.8)
* C.H.  20/03/03        : Ajout des clauses #ifndef PC_SIMPL pour que cette proc soit compil                          ee sur PC3 1.9
* JMG	27/04/04	: ajout purge de SAD_PAL 1.10
* CHI	19/07/05        : ajout  purge table FMC_NAT DEM487 v1.4
* PNI	12/07/06	: ajout PMA, BAF,BAD dem576 v1.5
* AAZ   29/08/2007  : ajout SIG, ECL, VEN, ENR, INC DEM 663
* JMG	19/09/11	: ajout ALT_LOC  DEM 979
* JMG	05/04/12	: ajout FMC_COMMU
* VR	14/12/11 	: Ajout PRV (DEM/1016)
* JPL	13/09/12	: Prise en compte table FMC_INC pour la classe Incendie (DEM 1037)  1.18
* JPL	07/11/12	: Purge FMC_MET_CCH pour classe Meteo et ACT_CCH pour classe Operateur (DEM 1032)  1.19
* JMG	10/06/13	: ajout FMC_REG 1.20
* JMG	06/06/17	: ajout SAD_SAGA 1.21
* LCL	02/03/18	: Ajout controleurs de feux CTRL_FEUX 1.22 DEM1284
* LPE	24/07/20	: Ajout TDP_FCD 1.23 DEM-SAE152
...
as
	...
	begin
		...
		select 	@vl_Nombre = count(*) from EXP..FMC_NAT, #EVT_SUP
		where	EXP..FMC_NAT.numero = #EVT_SUP.numero and
			EXP..FMC_NAT.cle = #EVT_SUP.cle and
			EXP..FMC_NAT.sit = @vl_Site
		if @vl_Nombre != 0
		begin
			print "FMC_NAT : %1!", @vl_Nombre
			if @va_Mode_in = XZAGC_IMP_ENR
				select * from EXP..FMC_NAT, #EVT_SUP
				where   EXP..FMC_NAT.numero = #EVT_SUP.numero and
					EXP..FMC_NAT.cle = #EVT_SUP.cle and
					EXP..FMC_NAT.sit = @vl_Site
			delete	EXP..FMC_NAT
			from EXP..FMC_NAT, #EVT_SUP
			where	EXP..FMC_NAT.numero = #EVT_SUP.numero and
				EXP..FMC_NAT.cle = #EVT_SUP.cle and
				EXP..FMC_NAT.sit = @vl_Site
			print ' '
		...
		select 	@vl_Nombre = count(*) from EXP..FMC_INC, #EVT_SUP
		where	EXP..FMC_INC.numero = #EVT_SUP.numero and
			EXP..FMC_INC.cle = #EVT_SUP.cle and
			EXP..FMC_INC.sit = @vl_Site
		if @vl_Nombre != 0
		begin
			print "FMC_INC : %1!", @vl_Nombre
			if @va_Mode_in = XZAGC_IMP_ENR
				select * from EXP..FMC_INC, #EVT_SUP
				where   EXP..FMC_INC.numero = #EVT_SUP.numero and
					EXP..FMC_INC.cle = #EVT_SUP.cle and
					EXP..FMC_INC.sit = @vl_Site
			delete	EXP..FMC_INC
			from EXP..FMC_INC, #EVT_SUP
			where	EXP..FMC_INC.numero = #EVT_SUP.numero and
				EXP..FMC_INC.cle = #EVT_SUP.cle and
				EXP..FMC_INC.sit = @vl_Site
			print ' '
		...
		select 	@vl_Nombre = count(*) from EXP..FMC_MET_CCH, #EVT_SUP
		where	EXP..FMC_MET_CCH.numero = #EVT_SUP.numero and
			EXP..FMC_MET_CCH.cle = #EVT_SUP.cle and
			EXP..FMC_MET_CCH.sit = @vl_Site
		if @vl_Nombre != 0
		begin
			print "FMC_MET_CCH : %1!", @vl_Nombre
			if @va_Mode_in = XZAGC_IMP_ENR
				select * from EXP..FMC_MET_CCH, #EVT_SUP
				where   EXP..FMC_MET_CCH.numero = #EVT_SUP.numero and
					EXP..FMC_MET_CCH.cle = #EVT_SUP.cle and
					EXP..FMC_MET_CCH.sit = @vl_Site
			delete	EXP..FMC_MET_CCH
			from EXP..FMC_MET_CCH, #EVT_SUP
			where	EXP..FMC_MET_CCH.numero = #EVT_SUP.numero and
				EXP..FMC_MET_CCH.cle = #EVT_SUP.cle and
				EXP..FMC_MET_CCH.sit = @vl_Site
			print ' '
		...
		/*B trace et suppression des enregistrements de FMC_REG a supprimer */
		select 	@vl_Nombre = count(*) from EXP..FMC_REG, #EVT_SUP
		where	EXP..FMC_REG.numero = #EVT_SUP.numero and
			EXP..FMC_REG.cle = #EVT_SUP.cle and
			EXP..FMC_REG.sit = @vl_Site
		if @vl_Nombre != 0
		begin
			print "FMC_REG : %1!", @vl_Nombre
			if @va_Mode_in = XZAGC_IMP_ENR
				select * from EXP..FMC_REG, #EVT_SUP
				where   EXP..FMC_REG.numero = #EVT_SUP.numero and
					EXP..FMC_REG.cle = #EVT_SUP.cle and
					EXP..FMC_REG.sit = @vl_Site
			delete	EXP..FMC_REG
			from EXP..FMC_REG, #EVT_SUP
			where	EXP..FMC_REG.numero = #EVT_SUP.numero and
				EXP..FMC_REG.cle = #EVT_SUP.cle and
				EXP..FMC_REG.sit = @vl_Site
			print ' '
		...
		/*B trace et suppression des enregistrements de FMC_COMMU a supprimer */
		select 	@vl_Nombre = count(*) from EXP..FMC_COMMU, #EVT_SUP
		where	EXP..FMC_COMMU.evenement = #EVT_SUP.numero and
			EXP..FMC_COMMU.cle = #EVT_SUP.cle
		if @vl_Nombre != 0
		begin
			print "FMC_COMMU : %1!", @vl_Nombre
			if @va_Mode_in = XZAGC_IMP_ENR
				select * from EXP..FMC_COMMU, #EVT_SUP
				where   EXP..FMC_COMMU.evenement = #EVT_SUP.numero and
					EXP..FMC_COMMU.cle = #EVT_SUP.cle
			delete	EXP..FMC_COMMU
			from EXP..FMC_COMMU, #EVT_SUP
			where	EXP..FMC_COMMU.evenement = #EVT_SUP.numero and
				EXP..FMC_COMMU.cle = #EVT_SUP.cle
			print ' '
		end
		...
		select 	@vl_Nombre = count(*) from EXP..FMC_DAT, #EVT_SUP
		where	EXP..FMC_DAT.numero = #EVT_SUP.numero and
			EXP..FMC_DAT.cle = #EVT_SUP.cle
		if @vl_Nombre != 0
		begin
			print "FMC_DAT : %1!", @vl_Nombre
			if @va_Mode_in = XZAGC_IMP_ENR
				select * from EXP..FMC_DAT, #EVT_SUP
				where   EXP..FMC_DAT.numero = #EVT_SUP.numero and
					EXP..FMC_DAT.cle = #EVT_SUP.cle and
					EXP..FMC_DAT.sit = @vl_Site
			delete	EXP..FMC_DAT
			from EXP..FMC_DAT, #EVT_SUP
			where	EXP..FMC_DAT.numero = #EVT_SUP.numero and
				EXP..FMC_DAT.cle = #EVT_SUP.cle and
				EXP..FMC_DAT.sit = @vl_Site
			print ' '
	...
	begin
		...
		/*B trace et suppression des enregistrements de SAD_PAL a supprimer */
                select  @vl_Nombre = count(*) from EXP..SAD_PAL
                where   numero in (select numero from #PLN_SUP)
                if @vl_Nombre != 0
                begin
                        print "SAD_PAL : %1!", @vl_Nombre
                        if @va_Mode_in = XZAGC_IMP_ENR
                                select * from EXP..SAD_PAL
                                where   numero in (select numero from #PLN_SUP)
                        delete  EXP..SAD_PAL
                        where   numero in (select numero from #PLN_SUP)
                        print ' '
		...
		/*B trace et suppression des enregistrements de SAD_PMA a supprimer */
		select 	@vl_Nombre = count(*) from EXP..SAD_PMA
		where	numero in (select numero from #PLN_SUP) 
		if @vl_Nombre != 0
		begin
			print "SAD_PMA : %1!", @vl_Nombre
			if @va_Mode_in = XZAGC_IMP_ENR
				select * from EXP..SAD_PMA
				where   numero in (select numero from #PLN_SUP)
			delete	EXP..SAD_PMA
			where	numero in (select numero from #PLN_SUP) 
			print ' '
		...
		/*B trace et suppression des enregistrements de SAD_BAF a supprimer */
		select 	@vl_Nombre = count(*) from EXP..SAD_BAF
		where	numero in (select numero from #PLN_SUP) 
		if @vl_Nombre != 0
		begin
			print "SAD_BAF : %1!", @vl_Nombre
			if @va_Mode_in = XZAGC_IMP_ENR
				select * from EXP..SAD_BAF
				where   numero in (select numero from #PLN_SUP)
			delete	EXP..SAD_BAF
			where	numero in (select numero from #PLN_SUP) 
			print ' '
		...
		/*B trace et suppression des enregistrements de SAD_BAD a supprimer */
		select 	@vl_Nombre = count(*) from EXP..SAD_BAD
		where	numero in (select numero from #PLN_SUP) 
		if @vl_Nombre != 0
		begin
			print "SAD_BAD : %1!", @vl_Nombre
			if @va_Mode_in = XZAGC_IMP_ENR
				select * from EXP..SAD_BAD
				where   numero in (select numero from #PLN_SUP)
			delete	EXP..SAD_BAD
			where	numero in (select numero from #PLN_SUP) 
			print ' '
		...
		/*B trace et suppression des enregistrements de SAD_PRV a supprimer */
		select 	@vl_Nombre = count(*) from EXP..SAD_PRV
		where	numero in (select numero from #PLN_SUP) 
		if @vl_Nombre != 0
		begin
			print "SAD_PRV : %1!", @vl_Nombre
			if @va_Mode_in = XZAGC_IMP_ENR
				select * from EXP..SAD_PRV
				where   numero in (select numero from #PLN_SUP)
			delete	EXP..SAD_PRV
			where	numero in (select numero from #PLN_SUP) 
			print ' '
		...
		/* trace et suppression des enregistrements de SAD_PIC a supprimer */
		select 	@vl_Nombre = count(*) from EXP..SAD_PIC
		where	numero in (select numero from #PLN_SUP) 
		if @vl_Nombre != 0
		begin
			print "SAD_PIC : %1!", @vl_Nombre
			if @va_Mode_in = XZAGC_IMP_ENR
				select * from EXP..SAD_PIC
				where   numero in (select numero from #PLN_SUP)
			delete	EXP..SAD_PIC
			where	numero in (select numero from #PLN_SUP) 
			print ' '
		...
		/*B trace et suppression des enregistrements de SAD_SAGA a supprimer */
		select 	@vl_Nombre = count(*) from EXP..SAD_SAGA
		where	numero in (select numero from #PLN_SUP) 
		if @vl_Nombre != 0
		begin
			print "SAD_SAGA : %1!", @vl_Nombre
			if @va_Mode_in = XZAGC_IMP_ENR
				select * from EXP..SAD_SAGA
				where   numero in (select numero from #PLN_SUP)
			delete	EXP..SAD_SAGA
			where	numero in (select numero from #PLN_SUP) 
			print ' '
	...
	begin
		...
		select 	@vl_Nombre = count(*) from EXP..ACT_EXP_ADA
		where	actio in (select numero from #ACT_SUP) and sit = @vl_Site 
		if @vl_Nombre != 0
		begin
			print "ACT_EXP_ADA : %1!", @vl_Nombre
			if @va_Mode_in = XZAGC_IMP_ENR
				select * from EXP..ACT_EXP_ADA
				where   actio in (select numero from #ACT_SUP) and 
					sit = @vl_Site
		...
		select 	@vl_Nombre = count(*) from EXP..ACT_TFM_EXP
		where	actio in (select numero from #ACT_SUP) and sit = @vl_Site 
		if @vl_Nombre != 0
		begin
			print "ACT_TFM_EXP : %1!", @vl_Nombre
			if @va_Mode_in = XZAGC_IMP_ENR
				select * from EXP..ACT_TFM_EXP
				where   actio in (select numero from #ACT_SUP) and 
					sit = @vl_Site
		...
		select 	@vl_Nombre = count(*) from EXP..ACT_PMV_ADA
		where	actio in (select numero from #ACT_SUP) and sit = @vl_Site 
		if @vl_Nombre != 0
		begin
			print "ACT_PMV_ADA : %1!", @vl_Nombre
			if @va_Mode_in = XZAGC_IMP_ENR
				select * from EXP..ACT_PMV_ADA
				where   actio in (select numero from #ACT_SUP) and 
					sit = @vl_Site
		...
		select 	@vl_Nombre = count(*) from EXP..ACT_CCH
		where	actio in (select numero from #ACT_SUP) and sit = @vl_Site 
		if @vl_Nombre != 0
		begin
			print "ACT_CCH : %1!", @vl_Nombre
			if @va_Mode_in = XZAGC_IMP_ENR
				select * from EXP..ACT_CCH
				where   actio in (select numero from #ACT_SUP) and 
					sit = @vl_Site
			delete	EXP..ACT_CCH
			where	actio in (select numero from #ACT_SUP)  and
	...
	begin
		...
		numero in (select numero from EXP..ALT_EXP where horodate <= @va_Horodate_in and sit = @vl_Site)
	...
	select @vl_Nombre = count(*) from EXP..REF_PIT_MESSAGE
	where EXP..REF_PIT_MESSAGE.reference in (select reference from EXP..PIT_LOG where EXP..PIT_LOG.horodate <= @va_Horodate_in )
	if @vl_Nombre != 0
	begin
		print "REF_PIT_MESSAGE : %1!", @vl_Nombre
		if @va_Mode_in = XZAGC_IMP_ENR
			select * from EXP..REF_PIT_MESSAGE
			where EXP..REF_PIT_MESSAGE.reference in (select reference from EXP..PIT_LOG where EXP..PIT_LOG.horodate <= @va_Horodate_in )
		delete	EXP..REF_PIT_MESSAGE
		where EXP..REF_PIT_MESSAGE.reference in (select reference from EXP..PIT_LOG where EXP..PIT_LOG.horodate <= @va_Horodate_in )
		delete	EXP..FMC_REF_EXT
		where EXP..FMC_REF_EXT.reference in (select reference from EXP..PIT_LOG where EXP..PIT_LOG.horodate <= @va_Horodate_in )
		print ' '
	...
	select @vl_Nombre = count(*) from EXP..PIT_LOG 
	where EXP..PIT_LOG.horodate <= @va_Horodate_in
	if @vl_Nombre != 0
	begin
		print "PIT_LOG : %1!", @vl_Nombre
		if @va_Mode_in = XZAGC_IMP_ENR
			select * from EXP..PIT_LOG 
			where EXP..PIT_LOG.horodate <= @va_Horodate_in
		delete	EXP..PIT_LOG 
		where EXP..PIT_LOG.horodate <= @va_Horodate_in
		print ' '
	...
	select @vl_Nombre = count(*) from EXP..TDP_FCD
	where horodate <= @va_Horodate_in
	if @vl_Nombre != 0
	begin
		print "TDP_FCD  : %1!", @vl_Nombre
		if @va_Mode_in = XZAGC_IMP_ENR
			select * from EXP..TDP_FCD
			where horodate <= @va_Horodate_in
		delete  EXP..TDP_FCD
		where horodate <= @va_Horodate_in
		print ' '
...
#else
	select @vl_Nombre = count(*) from EXP..MES_ADA
	where horodate <= @va_Horodate_in 
	if @vl_Nombre != 0
	begin
		print "MES_ADA : %1!", @vl_Nombre
		if @va_Mode_in = XZAGC_IMP_ENR
			select * from EXP..MES_ADA
			where horodate <= @va_Horodate_in 
		delete	EXP..MES_ADA
		where horodate <= @va_Horodate_in 
		print ' '
