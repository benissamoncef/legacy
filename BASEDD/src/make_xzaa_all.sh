...
# ZCH   26/03/2021  : Modification (supprimer des procs xzaa30sp et xzaa31sp)
...
Fichier_Compil="xzaa${Site_Compil}"
...
{
    echo " "
    echo -e '\E[47;31m'"\033[1mla commande xzaa_make.sh a ete  modifiee.\033[0m"
    echo " xzaa_make.sh appelle  le script ${Nom_Prog} qui prend en compte les specificites de l'environement de developpement et celles de production."
    echo " " 
    ...
    echo " xzaa_make.sh appelle  le script (${Nom_Prog} ) pour prendre en compte les specificites de l'environement de developpement.(directive DEV)"
    ...
    echo -e '\E[47;34m'"\033[1m Exemple: procedure pour le site CI plateforme de DEV  (via xzaa_make.sh): $0 CI DEV \033[0m"
    ...
    echo " ${Nom_Prog} genere le fichier .sql a livrer en production via le script sae_liv <num_jira> -p xzaa_make.sh  <site> "
...
echo "if exists (select 1 from sysobjects where name = 'XZAA' and type = 'P')" >> ${fic_tmp_c}

echo "drop procedure XZAA " >> ${fic_tmp_c} 
echo "go" >> ${fic_tmp_c}
...
# les groupees
cat_proc "xzaa06sp.prc"

if [[ "$Site_Compil" != "HIS" ]]
then

    cat_proc "xzaa01sp.prc"
    cat_proc "xzaa02sp.prc"
    cat_proc "xzaa05sp.prc"
    cat_proc "xzaa07sp.prc"
    cat_proc "xzaa10sp.prc"

# degroupees
sae_dev_make_prc "xzaa08sp.prc" $Site_Compil $cible
sae_dev_make_prc "xzaa11sp.prc" $Site_Compil $cible
sae_dev_make_prc "xzaa010sp.prc" $Site_Compil $cible
	
sae_dev_make_prc "xzaa21sp.prc" $Site_Compil $cible
