#!/bin/ksh
# Fichier : @(#)creexp.bas	1.8        Release : 1.8        Date : 05/14/97
##################################################################################
...
# P.N.	21/02/97	: AJOUT FRN_TDP et ZDP_6MN
...
echo "MES_KCS" >> $FicErr
cat exp.use tables/mes_kcs.tab | isql -S $NomServeur -U MIGRAZUR -P MIGRAZUR >>
$FicErr
# nom table : MESURE_TRAFIC_6_MINUTES, code : MES_NEC
echo "MES_NEC" >> $FicErr
cat exp.use tables/mes_nec.tab | isql -S $NomServeur -U MIGRAZUR -P MIGRAZUR >>
$FicErr
# nom table : MESURE_TRAFIC_6_MINUTES, code : MES_QPC
echo "MES_QPC" >> $FicErr
cat exp.use tables/mes_qpc.tab | isql -S $NomServeur -U MIGRAZUR -P MIGRAZUR >>
$FicErr
...
if [[ $NomServeur = "CI" ]] || [[ $NomServeur = "CA" ]]
then
