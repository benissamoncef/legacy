...
Fichier_Compil="xzar${Site_Compil}"
cible=$2       # DEV ou numero de jira
Nom_Fic_Source="XZAR"
fic_tmp_c_xzar="/tmp/make_${Nom_Fic_Source}.c"
fic_tmp_sql_xzar="/tmp/make_${Nom_Fic_Source}.sql"
# repertoire des includes
includes=$HOME
...
echo "use PRC" >  ${fic_tmp_c_xzar}
echo "go" >> ${fic_tmp_c_xzar}
echo "drop procedure XZAR" >> ${fic_tmp_c_xzar}
echo "go" >> ${fic_tmp_c_xzar}


echo "#include \"$includes/migrazur/XDMICG/inc/xdc.h\"" >>  ${fic_tmp_c_xzar}
...
cpp -E -P -C  ${directive_compil_Plateforme}  ${fic_tmp_c_xzar}  > ${fic_tmp_sql_xzar}

chmod 775 ${fic_tmp_c_xzar}  ${fic_tmp_sql_xzar}  # on donne les droits au groupe
...
then
    ...
    isql -UMIGRAZUR -PMIGRAZUR  -SSQL_${Site_Compil}  -i ${fic_tmp_sql_xzar}  2>&1  | tee -a $Fichier_Trace
    ...
    cp -p ${fic_tmp_sql_xzar}  ${liv_ref_complet}/${Site_Compil}_${Nom_Fic_Source}.sql
...
rm $fic_tmp_c_xzar $fic_tmp_sql_xzar
