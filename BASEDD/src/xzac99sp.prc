/*E*/
/*  Fichier : $Id: xzac99sp.prc,v 1.9 2021/03/08 14:13:27 pc2dpdy Exp $     Release : $Revision: 1.9 $        Date : $Date: 2021/03/08 14:13:27 $
------------------------------------------------------
* GTIE * PROJET MIGRAZUR
------------------------------------------------------
* SOUS-SYSTEME BASEDD
------------------------------------------------------
* MODULE XZAO * FICHIER xzac99.prc
------------------------------------------------------
...
* PNI	08/11/23	: suppression du test sur Manuelle_in SAE-530
------------------------------------------------------ */
...
if exists (select * from sysobjects where name = 'XZAC99' and type = 'P')
	drop procedure XZAC99
go


create procedure XZAC99
	@va_NumeroBAD_in		T_EQUIPEMENT = null,
...
as
	declare	@vl_NumeroAction int, @vl_Status int, @vl_NomMachine char(10),
		@vl_Scenario char(10), @vl_Commande int, @vl_TypeAction tinyint,
	...
	if @va_NumeroBAD_in = 0 select @va_NumeroBAD_in = null
	if @va_Site_in = 0 select @va_Site_in = null
	if @va_HeureLancementFin_in = XDC_DATE_NULLE select @va_HeureLancementFin_in = null

	/*A controle des parametres obligatoires */
	if  @va_NumeroBAD_in = null or @va_Site_in = null or 
	    @va_HeureLancementFin_in = null
		return XDC_ARG_INV

	select @vl_TypeAction = XDC_EQT_BAD
	...
	begin
		if @vl_serveur_pra=XDC_SQLDP
			exec @vl_Status = SQL_DS.PRC..XZAC99 @va_NumeroBAD_in, @va_Site_in, @va_HeureLancementFin_in, 
				@va_Manuelle_in, @va_Scenario_in, @va_NumeroAction_out output, @va_NomSiteLocal_in, XDC_OUI

		else if @vl_serveur_pra=XDC_SQLVC
			exec @vl_Status = SQL_DM.PRC..XZAC99 @va_NumeroBAD_in, @va_Site_in, @va_HeureLancementFin_in, 
				@va_Manuelle_in, @va_Scenario_in, @va_NumeroAction_out output, @va_NomSiteLocal_in, XDC_OUI

		else if @vl_serveur_pra=XDC_SQLCI
			exec @vl_Status = SQL_CI.PRC..XZAC99 @va_NumeroBAD_in, @va_Site_in, @va_HeureLancementFin_in, 
				@va_Manuelle_in, @va_Scenario_in, @va_NumeroAction_out output, @va_NomSiteLocal_in, XDC_OUI
		else return XDC_NOK
	...
	from EXP..ACT_GEN
	where 	EXP..ACT_GEN.equipement = @va_NumeroBAD_in and
		EXP..ACT_GEN.sit = @va_Site_in and
		EXP..ACT_GEN.type = XDC_ACT_BAD
	...
	/********** SAE 530 if @va_Manuelle_in =XDC_NON
	begin ****/
		/*A initialisation des var locales pour la recherche */
		/*A du nom de la machine pilotant la BAD */
		select	@vl_MachineNonTrouvee = XDC_NON,
			@vl_NumEqtBoucle = @va_NumeroBAD_in,
		...
		*/
		if @vl_Site = XDC_CI
			exec @vl_Status = TASRV_CI...ACRP13 @va_NumeroBAD_in, @va_Scenario_in,
				@vl_Commande, @vl_NomMachine
		/* PRA
		else if  @@servername = XDC_SQLVC
		*/
		else if @vl_Site = XDC_VC
			exec @vl_Status = TASRV_VC...ACRP13 @va_NumeroBAD_in, @va_Scenario_in,
				@vl_Commande, @vl_NomMachine
		/* PRA
		else if  @@servername = XDC_SQLDP
		*/
		else if @vl_Site = XDC_DP
			exec @vl_Status = TASRV_DP...ACRP13 @va_NumeroBAD_in, @va_Scenario_in,
				@vl_Commande, @vl_NomMachine
		else if @vl_Site = XDC_CA
			exec @vl_Status = TASRV_CA...ACRP13 @va_NumeroBAD_in, @va_Scenario_in,
				@vl_Commande, @vl_NomMachine
