/*E*/
/*Fichier :  $Id: xzac635sp.prc,v 1.9 2021/03/08 14:26:11 pc2dpdy Exp $      Release : $Revision: 1.9 $        Date : $Date: 2021/03/08 14:26:11 $
------------------------------------------------------
* STERIA *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME  BASEDD
------------------------------------------------------
* FICHIER xzac635sp.prc
------------------------------------------------------
...
*
* Sequence d'appel
* SP	XZAC635_Commande_Fin_Signalisation
*
* Arguments en entree
...
* 	ACRP635 :	Appeler XZET03 pour commander l'equipement
-----------------------------------------------------------------------*/

use PRC
go

drop procedure XZAC635
go

create procedure XZAC635
...
as
	...
	begin
		if @vl_serveur_pra = XDC_SQLVC
			  exec @vl_Status = SQL_DM.PRC..XZAC635  @va_TypEqt_in,  @va_NumeroTunnel_in,  @va_Site_in,  @va_HeureLancementFin_in,  @va_Manuelle_in, @va_NomSiteLocal_in, XDC_OUI
		
		/* modif ABE du 05-02-2020 : sp�cificit� toulon */
		
		else if @vl_serveur_pra = XDC_SQLDP
			  exec @vl_Status = SQL_DS.PRC..XZAC635  @va_TypEqt_in,  @va_NumeroTunnel_in,  @va_Site_in,  @va_HeureLancementFin_in,  @va_Manuelle_in, @va_NomSiteLocal_in, XDC_OUI
		
		else if @vl_serveur_pra = XDC_SQLCI
                          exec @vl_Status = SQL_CI.PRC..XZAC635  @va_TypEqt_in,  @va_NumeroTunnel_in,  @va_Site_in,  @va_HeureLancementFin_in,  @va_Manuelle_in, @va_NomSiteLocal_in, XDC_OUI
	...
	begin
		...
		select @vl_Sequence = XDC_INC_SEQ_RETOUR_NORMAL
		...
		if @vl_Site = XDC_CI
			exec @vl_Status = TASRV_CI...ACRP44 @va_TypEqt_in, @va_NumeroTunnel_in, @vl_Sequence,
				@vl_Commande

		else if @vl_Site = XDC_DP
			exec @vl_Status = TASRV_DP...ACRP44 @va_TypEqt_in, @va_NumeroTunnel_in, @vl_Sequence,
				@vl_Commande

		else if @vl_Site = XDC_VC
			exec @vl_Status = TASRV_VC...ACRP44 @va_TypEqt_in, @va_NumeroTunnel_in, @vl_Sequence,
				@vl_Commande
	
		else if @vl_Site = XDC_CA
			exec @vl_Status = TASRV_CA...ACRP44 @va_TypEqt_in, @va_NumeroTunnel_in, @vl_Sequence,
				@vl_Commande
