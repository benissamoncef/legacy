/*E*/
/* Fichier : $Id: xzac550sp.prc,v 1.5 2021/03/08 14:27:29 pc2dpdy Exp $      Release : $Revision: 1.5 $        Date : $Date: 2021/03/08 14:27:29 $
------------------------------------------------------
...
* LCL	20/11/19	: MOVIS DEM-SAE93 1.4
* GRO	12/10/2020      : MOVIS DEM-SAE93 1.5
* CGR	05/01/21	: MOVIS ajout PRA DEM-SAE93 1.5
...
* 
* Inserer dans la table ACTION et la table E_PRV.
* 
...
* 
* ACRP15 :	Appeler ACRP15 pour afficher sur l'equipement PRV
...
if exists (select * from sysobjects where name = 'XZAC550' and type = 'P')
	drop procedure XZAC550
go

create procedure XZAC550
...
as
	...
	begin
		if @vl_serveur_pra = XDC_SQLVC
			exec @vl_Status = SQL_DM.PRC..XZAC550 @va_Horodate_in, @va_NumEvt_in, @va_CleEvt_in,
						@va_Operateur_in, @va_NumEqt_in, @va_Site_in,
						@va_Bandeau_in, @va_Vitesse_in, @va_Priorite_in,		
						@va_DistanceEvt_in, @va_site_origine_in,
						@va_NumeroAction_out output, XDC_OUI 
		else if @vl_serveur_pra = XDC_SQLDP
			exec @vl_Status = SQL_DS.PRC..XZAC550 @va_Horodate_in, @va_NumEvt_in, @va_CleEvt_in,
						@va_Operateur_in, @va_NumEqt_in, @va_Site_in,
						@va_Bandeau_in, @va_Vitesse_in, @va_Priorite_in,
						@va_DistanceEvt_in, @va_site_origine_in,
						@va_NumeroAction_out output, XDC_OUI 
		else if @vl_serveur_pra = XDC_SQLCI
                        exec @vl_Status = SQL_CI.PRC..XZAC550 @va_Horodate_in, @va_NumEvt_in, @va_CleEvt_in,
						@va_Operateur_in, @va_NumEqt_in, @va_Site_in,
						@va_Bandeau_in, @va_Vitesse_in, @va_Priorite_in,
						@va_DistanceEvt_in, @va_site_origine_in,
						@va_NumeroAction_out output, XDC_OUI
...
/*DEM706*/
	if not exists ( select equipement from EXP..EQT_DSP
			where	equipement = @va_NumEqt_in and type = XDC_EQT_PRV and
				dernier = 1 and
				majeure = XDC_NON and 
	...
	begin tran
		...
		XDC_ACT_PRV,  @va_NumEqt_in, @va_Horodate_in, @va_Priorite_in, 
		XDC_NON, @va_DistanceEvt_in, @va_Operateur_in, @va_site_origine_in)

		insert EXP..ACT_PRV (actio, PRV, bandeau, vitesse, sit)
		values ( @va_NumeroAction_out,  @va_NumEqt_in, @va_Bandeau_in, @va_Vitesse_in, @vl_Site)
	...
	if @vl_Site = XDC_CI
		exec @vl_Status = TASRV_CI...ACRP15 @va_NumEqt_in,
		@va_Bandeau_in, @va_Vitesse_in,
		@va_NumeroAction_out, @vl_NomMachine


	else if @vl_Site = XDC_VC
		exec @vl_Status = TASRV_VC...ACRP15 @va_NumEqt_in,
		@va_Bandeau_in, @va_Vitesse_in,
		@va_NumeroAction_out, @vl_NomMachine
		
	else if @vl_Site = XDC_DP
		exec @vl_Status = TASRV_DP...ACRP15 @va_NumEqt_in,
		@va_Bandeau_in, @va_Vitesse_in,
		@va_NumeroAction_out, @vl_NomMachine
		
	else if @vl_Site = XDC_CA
		exec @vl_Status = TASRV_CA...ACRP15 @va_NumEqt_in,
		@va_Bandeau_in, @va_Vitesse_in,
		@va_NumeroAction_out, @vl_NomMachine
	...
	begin
		...
		update EXP..ACT_PRV set depass_seuil=XDC_FAUX
