/*E*/
/* Fichier : $Id: drol_bad.c,v 1.12 2019/09/24 11:10:36 devgfi Exp $        Release : $Revision: 1.12 $        Date : $Date: 2019/09/24 11:10:36 $
**************************************************
...
* JPL	06/09/10 : Prise en compte du type Fmc Zone de stockage (DEM 942) 1.4
* JPL	18/02/11 : Migration architecture HP ia64 (DEM 971) : suppression warnings mineurs  1.5
* JPL	21/02/11 : Migration architecture HP ia64 (DEM 971) : adequation types parametres et profil fonctions  1.6
* JPL	14/10/11 : Pas d'action automatique SAPIENS pour Fmc simulees (Essai ou Exercice) (DEM 995)  1.7
* JPL	12/07/12 : Utilisation de l'indicateur de bifurcation pour localisation sur bretelle (DEM 1033)  1.8
* PNI	29/08/13 : SAPIENS au CI  DEM 1074/FI 105 1.9 1.10
* JPL	23/09/19 : Type d'evenement "INCENDIE" attendu en configuration et non "INCENDIE DE VEGETATION" (DEM 1352)  1.12
...
#define C_PERT		C_REP_MIG 				"bad.txt"
...
int drol_bad_charge();
static int drol_bad_recherche(XDY_Autoroute ,
		        XDY_PR ,
		        XDY_PR ,
			XDY_Sens ,
			XDY_Horodate ,
			...
			char *,
			int);
XDY_Booleen drol_bad_mode(XDY_Booleen,int,int);
T_CONDITION *drol_bad_decode_condition(char *);

static XDY_Booleen drol_bad_egal(int, int);
static XDY_Booleen drol_bad_different(int, int);
/*
static XDY_Booleen drol_bad_true(int, int);
static XDY_Booleen drol_bad_inferieur();
static XDY_Booleen drol_bad_superieur();
*/

/*fonctions utilisees dans les conditions*/
static XDY_Booleen drol_bad_fmc_simulee();
static XDY_Booleen drol_bad_une_voie_bloquee();
static XDY_Booleen drol_bad_une_voie_ouverte();
static XDY_Booleen drol_bad_toutes_voies_bloquees();
static XDY_Booleen drol_bad_une_voie_neutralisee();
static XDY_Booleen drol_bad_toutes_voies_neutralisees();
static XDY_Booleen drol_bad_toutes_voies_bloquees_neutralisees();
static XDY_Booleen drol_bad_toutes_voies_fermees();
static XDY_Booleen drol_bad_circulation_une_voie();
static XDY_Booleen drol_bad_sur_bau();
static XDY_Booleen drol_bad_sur_section_courante();
static XDY_Booleen drol_bad_sur_lit_arret();
static XDY_Booleen drol_bad_sur_echangeur();
static XDY_Booleen drol_bad_sur_bretelle_entree();
static XDY_Booleen drol_bad_sur_bretelle_sortie();
static XDY_Booleen drol_bad_signale();
static XDY_Booleen drol_bad_confirme();
static XDY_Booleen drol_bad_trafic_dense();
static XDY_Booleen drol_bad_trafic_ralenti();
static XDY_Booleen drol_bad_trafic_bouche();
static XDY_Booleen drol_bad_trafic_bloque();
static XDY_Booleen drol_bad_voies_retrecies();
static XDY_Booleen drol_bad_entree_fermee();
static XDY_Booleen drol_bad_sortie_fermee();
static XDY_Booleen drol_bad_datex();
static XDY_Booleen drol_bad_bouchon_moins_3_km();
static XDY_Booleen drol_bad_bouchon_entre_3_et_5_km();
static XDY_Booleen drol_bad_bouchon_plus_5_km();
static XDY_Booleen drol_bad_prevu();
static XDY_Booleen drol_bad_non_prevu();
static XDY_Booleen drol_bad_travaux_entretien();
static XDY_Booleen drol_bad_stockage();
static XDY_Booleen drol_bad_aire_service();
static XDY_Booleen drol_bad_aire_repos();
int drol_bad_voies(int, int, int, int, int);
static XDY_Booleen drol_bad_matieres();
static XDY_Booleen drol_bad_circulation_basculee_tunnel ();
static XDY_Booleen drol_bad_circulation_non_basculee_tunnel ();

static int ajouter_bad(XDY_Eqt ,
			XDY_District ,
			XDY_Autoroute ,
			XDY_PR ,
			int ,
...
static char *version = "$Id: drol_bad.c,v 1.12 2019/09/24 11:10:36 devgfi Exp $ : drol_bad" ;
...
{  
  ...
  XZST_03EcritureTrace(   XZSTC_FONCTION, "IN  : drol_bad_init");
  vm_noeud2=(T_NOEUD *)malloc(sizeof(T_NOEUD));
  vg_pert_bad=NULL;
  drol_bad_charge();

  XZST_03EcritureTrace(   XZSTC_FONCTION,"OUT drol_bad_init");
...
------------------------------------------------------*/
{
  ...
  T_PERT_BAD *pl_liste;
  int vl_auto;
  char vl_condition[200];
  char vl_libelle[200];
  XZST_03EcritureTrace(XZSTC_DEBUG1, "IN  : drol_bad_charge");
  ...
  {
      pl_liste = (T_PERT_BAD *)malloc(sizeof(T_PERT_BAD));
      vl_str=vl_Lig; 

      sscanf(vl_str,"%[^|]|%[^|]|%d",vl_libelle,vl_condition,&(pl_liste->Automatique));
      pl_liste->Condition = drol_bad_decode_condition(vl_condition);
      
      pl_liste->Suivant = vg_pert_bad;
      strcpy(pl_liste->Libelle, vl_libelle);
      vg_pert_bad = pl_liste;
  }
  fclose(pl_fp);
  XZST_03EcritureTrace(   XZSTC_DEBUG1, "OUT  : drol_bad_charge");
  return (XDC_OK);
}
...
T_CONDITION *drol_bad_decode_condition(char *va_cond)
...
------------------------------------------------------*/
{
  ...
  XZST_03EcritureTrace(   XZSTC_FONCTION, "drol_bad_decode_condition:  IN %s",va_cond);
  ...
  pl_cond->Fonction.Comp.Fct=drol_bad_egal;
  pl_cond->Suivant=NULL;
  pl_tmp=pl_cond;

  /*numero en fonction du type*/
  XZST_03EcritureTrace(   XZSTC_DEBUG1, "drol_bad_decode_condition: type = %s",pl_cond->Libelle);
  ...
  /*traiter le cas particulier de la meteo*/
    ...
    pl_tmp->Fonction.Comp.Fct=drol_bad_egal;
  ...
  /*cas particulier du hors bouchons*/
    ...
    pl_tmp->Fonction.Comp.Fct=drol_bad_different;
  }
  else {
    XZST_03EcritureTrace(   XZSTC_DEBUG1,"drol_bad_decode_condition : pb decodage type !!!");
    pl_cond->Fonction.Comp.Valeur=0;
  }
 
  /*si pas de condition supplementaire*/
  if (vl_indice==strlen(va_cond)) {
    XZST_03EcritureTrace(   XZSTC_DEBUG1, "OUT : drol_bad_decode_condition : que le type");
    return (pl_cond);
  }
  ...
  XZST_03EcritureTrace(   XZSTC_DEBUG1,"drol_bad_decode_condition : apres le type = %s",vl_ch_init);
...
pos_decode:
  ...
  vl_str[vl_indice]='\0';
    ...
    XZST_03EcritureTrace(   XZSTC_DEBUG1, "drol_bad_decode_condition:  cond %s",vl_str);
    strcpy(pl_tmp->Libelle,vl_str);

    /*il s'agit d'une fonction PREDICAT*/
    pl_tmp->Type=C_TYPE_PREDICAT;
    if (!strcmp(vl_str,C_COND_UNE_VOIE_BLOQ)) pl_tmp->Fonction.Predicat.Fct=drol_bad_une_voie_bloquee;
    else if (!strcmp(vl_str,C_COND_UNE_VOIE_OUVERTE)) pl_tmp->Fonction.Predicat.Fct=drol_bad_une_voie_ouverte;
    else if (!strcmp(vl_str,C_COND_TOUTES_VOIES_BLOQ)) pl_tmp->Fonction.Predicat.Fct=drol_bad_toutes_voies_bloquees;
    else if (!strcmp(vl_str,C_COND_TOUTES_VOIES_NEUT)) pl_tmp->Fonction.Predicat.Fct=drol_bad_toutes_voies_neutralisees;
    else if (!strcmp(vl_str,C_COND_UNE_VOIE_NEUT)) pl_tmp->Fonction.Predicat.Fct=drol_bad_une_voie_neutralisee;
    else if (!strcmp(vl_str,C_COND_CIRCULATION_UNE_VOIE)) pl_tmp->Fonction.Predicat.Fct=drol_bad_circulation_une_voie;
    else if (!strcmp(vl_str,C_COND_CONFIRME)) pl_tmp->Fonction.Predicat.Fct=drol_bad_confirme;
    else if (!strcmp(vl_str,C_COND_SIGNALE)) pl_tmp->Fonction.Predicat.Fct=drol_bad_signale;
    else if (!strcmp(vl_str,C_COND_SUR_BAU)) pl_tmp->Fonction.Predicat.Fct=drol_bad_sur_bau;
    else if (!strcmp(vl_str,C_COND_SUR_SECTION_COURANTE)) pl_tmp->Fonction.Predicat.Fct=drol_bad_sur_section_courante;
    else if (!strcmp(vl_str,C_COND_SUR_LIT_ARRET)) pl_tmp->Fonction.Predicat.Fct=drol_bad_sur_lit_arret;
    else if (!strcmp(vl_str,C_COND_SUR_ECHANGEUR)) pl_tmp->Fonction.Predicat.Fct=drol_bad_sur_echangeur;
    else if (!strcmp(vl_str,C_COND_SUR_BRETELLE_ENTREE)) pl_tmp->Fonction.Predicat.Fct=drol_bad_sur_bretelle_entree;
    else if (!strcmp(vl_str,C_COND_SUR_BRETELLE_SORTIE)) pl_tmp->Fonction.Predicat.Fct=drol_bad_sur_bretelle_sortie;
    else if (!strcmp(vl_str,C_COND_TRAFIC_DENSE)) pl_tmp->Fonction.Predicat.Fct=drol_bad_trafic_dense;
    else if (!strcmp(vl_str,C_COND_TRAFIC_RALENTI)) pl_tmp->Fonction.Predicat.Fct=drol_bad_trafic_ralenti;
    else if (!strcmp(vl_str,C_COND_TRAFIC_BOUCHE)) pl_tmp->Fonction.Predicat.Fct=drol_bad_trafic_bouche;
    else if (!strcmp(vl_str,C_COND_TRAFIC_BLOQUE)) pl_tmp->Fonction.Predicat.Fct=drol_bad_trafic_bloque;
    else if (!strcmp(vl_str,C_COND_DATEX)) pl_tmp->Fonction.Predicat.Fct=drol_bad_datex;
    else if (!strcmp(vl_str,C_COND_VOIES_RETRECIES)) pl_tmp->Fonction.Predicat.Fct=drol_bad_voies_retrecies;
    else if (!strcmp(vl_str,C_COND_ENTREE_FERMEE)) pl_tmp->Fonction.Predicat.Fct=drol_bad_entree_fermee;
    else if (!strcmp(vl_str,C_COND_SORTIE_FERMEE)) pl_tmp->Fonction.Predicat.Fct=drol_bad_sortie_fermee;
    else if (!strcmp(vl_str,C_COND_BOUCHON_MOINS_3_KM)) pl_tmp->Fonction.Predicat.Fct=drol_bad_bouchon_moins_3_km;
    else if (!strcmp(vl_str,C_COND_BOUCHON_ENTRE_3_ET_5_KM)) pl_tmp->Fonction.Predicat.Fct=drol_bad_bouchon_entre_3_et_5_km;
    else if (!strcmp(vl_str,C_COND_BOUCHON_PLUS_5_KM)) pl_tmp->Fonction.Predicat.Fct=drol_bad_bouchon_plus_5_km;    
    else if (!strcmp(vl_str,C_COND_PREVU)) pl_tmp->Fonction.Predicat.Fct=drol_bad_prevu;    
    else if (!strcmp(vl_str,C_COND_NON_PREVU)) pl_tmp->Fonction.Predicat.Fct=drol_bad_non_prevu;    
    else if (!strcmp(vl_str,C_COND_TOUTES_VOIES_FERMEES)) pl_tmp->Fonction.Predicat.Fct=drol_bad_toutes_voies_fermees;    
    else if (!strcmp(vl_str,C_COND_TRAVAUX_ENTRETIEN)) pl_tmp->Fonction.Predicat.Fct=drol_bad_travaux_entretien;    
    else if (!strcmp(vl_str,C_COND_STOCKAGE)) pl_tmp->Fonction.Predicat.Fct=drol_bad_stockage;    
    else if (!strcmp(vl_str,C_COND_AIRE_SERVICE)) pl_tmp->Fonction.Predicat.Fct=drol_bad_aire_service;    
    else if (!strcmp(vl_str,C_COND_AIRE_REPOS)) pl_tmp->Fonction.Predicat.Fct=drol_bad_aire_repos;    
    else if (!strcmp(vl_str,C_COND_MATIERES)) pl_tmp->Fonction.Predicat.Fct=drol_bad_matieres;
	else if (!strcmp(vl_str,C_COND_CIRCULATION_BASC_TUNNEL)) pl_tmp->Fonction.Predicat.Fct = drol_bad_circulation_basculee_tunnel;
	else if (!strcmp(vl_str,C_COND_CIRCULATION_NON_BASC_TUNNEL)) pl_tmp->Fonction.Predicat.Fct = drol_bad_circulation_non_basculee_tunnel;

  /*si fin de chaine*/
  if (vl_indice==strlen(va_cond)) {
    XZST_03EcritureTrace(   XZSTC_DEBUG1,"drol_bad_decode_condition : fin de chaine");
    return (pl_cond);
  }
  else {
...
static XDY_Booleen drol_bad_egal(int va_valeur1, int va_valeur2) { return (va_valeur1==va_valeur2); }
static XDY_Booleen drol_bad_different(int va_valeur1, int va_valeur2) { return (va_valeur1!=va_valeur2); }
/*
static XDY_Booleen drol_bad_true(int va_valeur1, int va_valeur2) { return (TRUE); }
static XDY_Booleen drol_bad_superieur(int va_valeur1, int va_valeur2) { return (va_valeur1>va_valeur2); }
static XDY_Booleen drol_bad_inferieur(int va_valeur1, int va_valeur2) { return (va_valeur1<va_valeur2); }
*/

static XDY_Booleen drol_bad_confirme() { return (vm_noeud->Infos_TFM.Confirme); }
static XDY_Booleen drol_bad_signale() { return (vm_noeud->Infos_TFM.Signale); }
...
static XDY_Booleen drol_bad_toutes_voies_neutralisees() { 
  return ( (vm_noeud->Localisation.BAU!=XDC_VOIE_SENS_NORMAL) &&
	   ...
	   (vm_noeud->Localisation.VM1_I!=XDC_VOIE_SENS_INVERSE) &&
	   (vm_noeud->Localisation.VL_I!=XDC_VOIE_SENS_INVERSE) &&
	   (drol_bad_voies(vm_noeud->Localisation.VL,
...
static XDY_Booleen drol_bad_toutes_voies_fermees() {
	return ( drol_bad_toutes_voies_neutralisees() || drol_bad_toutes_voies_bloquees_neutralisees());
}
...
static XDY_Booleen drol_bad_sur_bau() { 
  return ( (drol_bad_voies(vm_noeud->Localisation.VL,vm_noeud->Localisation.VM1,
			   vm_noeud->Localisation.VM2,
			   vm_noeud->Localisation.VR,
...
static XDY_Booleen drol_bad_bouchon_entre_3_et_5_km() {
  return (!drol_bad_bouchon_moins_3_km() && !drol_bad_bouchon_plus_5_km());}
...
static XDY_Booleen drol_bad_datex() { return (FALSE); }

static XDY_Booleen drol_bad_voies_retrecies() { return (vm_noeud->Localisation.Voies_Retrecies); }
...
static XDY_Booleen drol_bad_circulation_basculee_tunnel ()
{
	...
	XZST_03EcritureTrace(XZSTC_WARNING,"drol_bad_circulation_basculee_tunnel : avant XZAE861_Basculement_Tunnel");
	...
	{
		XZST_03EcritureTrace(XZSTC_WARNING,"drol_bad_circulation_basculee_tunnel : pb appel a XZAE861_Basculement_Tunnel");
		return (FALSE);
	}
	else
	{
		XZST_03EcritureTrace(XZSTC_WARNING,"drol_bad_circulation_basculee_tunnel : XZAE861_Basculement_Tunnel retourne %s", vl_basculement);
	}
...
static XDY_Booleen drol_bad_circulation_non_basculee_tunnel ()
{
	XZST_03EcritureTrace (XZSTC_WARNING, "drol_bad_circulation_non_basculee_tunnel : not drol_bad_circulation_basculee_tunnel");
	return (!drol_bad_circulation_basculee_tunnel());
}
...
XDY_Booleen drol_bad_filtre(T_NOEUD *va_noeud, XDY_District va_site, XDY_Horodate va_horodate)
...
------------------------------------------------------*/
{
  ...
  XZST_03EcritureTrace(   XZSTC_FONCTION,"IN : drol_bad_filtre %d/%d",
  ...
  /*on n'active les BAD que sur evenement majeur*/
  while (pl_cas!=NULL)
  {
    XZST_03EcritureTrace(   XZSTC_FONCTION,"drol_bad_filtre %s",pl_cas->Libelle);
    ...
    if (vl_res) {
      XZST_03EcritureTrace(   XZSTC_FONCTION,"drol_bad_filtre OK pour %s ",
					(char *)pl_cas->Libelle);
      /*recherche les BAD candidats*/
      drol_bad_recherche(va_noeud->Localisation.Autoroute, 
      ...
      XZST_03EcritureTrace(   XZSTC_FONCTION,"drol_bad_filtre NOK");
...
/*------------------------------------------------------
* SERVICE RENDU :
* fct de recherche des BAD
------------------------------------------------------
* SEQUENCE D'APPEL :
*/

static int drol_bad_recherche(XDY_Autoroute va_autoroute,
		        XDY_PR va_PR,
		        XDY_PR va_PR_tete,
			XDY_Sens va_sens,
			XDY_Horodate va_horodate,
			XDY_District va_site,
			XDY_Entier va_distance,
...
------------------------------------------------------*/
{
  ...
  XZST_03EcritureTrace(   XZSTC_FONCTION,"IN : drol_bad_recherche aut=%d,PR=%d,sens=%d,site=%d",
						va_autoroute,va_PR,va_sens,va_site);

  if ((va_sens == XDC_SENS_INCONNU) || 
      (va_sens == XDC_SENS_ENTREE_NORD) || 
      (va_sens == XDC_SENS_ENTREE_SUD)  ) {
    XZST_03EcritureTrace(XZSTC_FONCTION, "drol_bad_recherche : OUT evt sur une entree");
    return (XDC_NOK);
  }
  ...
  if ( ((XDY_Sens) va_sens) != XDC_SENS_INCONNU)
        {
                ...
                if ((va_sens==XDC_SENS_SORTIE_NORD) ||
                    ...
                    (va_sens==XDC_SENS_NORD))
                     ...
                     vl_sens=XDC_SENS_SUD;
                /*recherche les BAD en amont*/
                XZAO442_Rechercher_tous_les_BAD_amont((XDY_Autoroute) va_autoroute,
                                                (XDY_Sens) vl_sens,
                                                (XDY_PR) va_PR,
                                                (XDY_PR) va_PR_tete,
                                                (XDY_District) va_site,
                                                ...
                                                (char *) va_perturbation,
						(int)va_automatique,
                                                (XDY_FonctionInt)ajouter_bad,
                                                &flag
                                               );

        }

    XZST_03EcritureTrace(XZSTC_FONCTION, "drol_nav_recherche : apres XZAO442_Rechercher_tous_les_BAD_amont");
return (XDC_OK);
}


static int ajouter_bad(XDY_Eqt va_numero_bad,
                        XDY_District va_district_bad,
                        XDY_Autoroute va_autoroute_bad,
                        XDY_PR va_pr_bad,
                        int va_distance_bad,
                        ...
                        int *pa_resultat)
{
 ...
 if ((va_district_bad!=va_district) && (va_district!=vg_SitePrincipal))
      return (XDC_NOK);

    /*appel XZAT06*/
    if (XZAT36_Utilisation_BAD(     va_horodate, va_numero_bad,
                    &Dispo, Scenario,
                       &NumEvt, &CleEvt, &Priorite, &DistanceEvt) != XDC_OK)
    {
      XZST_03EcritureTrace(XZSTC_WARNING,
            "Etat de %d inconnu a la date %lf. On le considere indispo.",va_numero_bad, va_horodate);
      Dispo = XDC_EQT_MAJEUR;
    }
    if (Dispo > XDC_EQT_MINEUR)
    {
      XZST_03EcritureTrace(XZSTC_FONCTION,"bad indispo, on l'oublie");
      return (XDC_NOK);
    }
    ...
    if ( (va_district==XDC_CI) &&
         (va_distance_bad>DistanceEvt) && (DistanceEvt!=0)) {
      XZST_03EcritureTrace(XZSTC_FONCTION,"bad est utilise par un evt plus proche, on l'oublie");
      return (XDC_NOK);
    }
  ...
  XZST_03EcritureTrace(XZSTC_WARNING,"vg_site=%d;va_pmv.District=%d",vg_site,vm_bad.District);

XZST_03EcritureTrace(XZSTC_WARNING,"vg_presence_site[va_pmv.District]=%d",vg_presence_site[vm_bad.District]);


    if (((vg_site==XDC_CI) && (vg_presence_site[vm_bad.District]==XDC_VRAI)) || drol_bad_fmc_simulee()) {
    ...
    strcpy(vm_bad.Scenario_Courant,Mode);
    strcpy(vm_bad.Scenario,C_SCE_NEUTRALISATION);
    
    if (!strcmp(Scenario,C_SCE_NEUTRALISATION)) {
      XZST_03EcritureTrace(XZSTC_FONCTION,"proposition identique a l'utilisation courante. On la supprime");
      if (XZAP33_Ajout_Prop_BAD(vg_numero_plan,
			  vg_ordre_bad++,
			  va_district_bad,
			  "",
			  255,
			  ...
			  "", 
			  "",
			  C_SCE_NEUTRALISATION,
			  TRUE) != XDC_OK)
        XZST_03EcritureTrace(XZSTC_FONCTION,"drol_bad_message : pb appel a XZAP33_Ajout_Prop_BAD");
        return (XDC_OK);
    }
    ...
    if (vl_mode_auto==0) {
      ...
      if (va_district_bad == vg_site) {
        if (XZAC93_Commande_BAD ( vm_noeud->NumEvt.numero    ,
                                 vm_noeud->NumEvt.cle     ,
                                 vg_num_poste ,
                                 va_numero_bad       ,
                                 va_horodate  ,
                                 ...
                                 vg_site      ) != XDC_OK)
          XZST_03EcritureTrace(XZSTC_WARNING,
                        "ajouter_bad : pb dans l'appel a XZAC93_Commande_BAD");
        else
          XZST_03EcritureTrace(XZSTC_FONCTION,"ajouter_bad : appel a XZAC93 OK");
      }
      else { /*sur districts differents, envoyer un msg a TACLI*/
        if (XZAC193_Commande_BAD_TACLI ( vm_noeud->NumEvt.numero    ,
                                 vm_noeud->NumEvt.cle     ,
                                 vg_num_poste ,
                                 va_numero_bad       ,
                                 va_horodate  ,
                                 ...
                                 vg_site      ) != XDC_OK)
          XZST_03EcritureTrace(XZSTC_WARNING,
                        "ajouter_bad : pb dans l'appel a XZAC193_Commande_BAD_TACLI");
        else
          XZST_03EcritureTrace(XZSTC_FONCTION,"ajouter_bad : appel a XZAC193 OK");
      }
    }
  return (XDC_OK);
}
...
XDY_Booleen drol_bad_mode(XDY_Booleen va_confirme,int va_VL, int va_VR)
...
*/
 ...
 int drol_bad_voies(int VL, int VM1, int VM2, int VR, int Reference)
