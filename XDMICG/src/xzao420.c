/*E*/
/* Fichier : $Id: xzao420.c,v 1.6 2020/05/19 14:33:19 pc2dpdy Exp $        $Revision: 1.6 $        $Date: 2020/05/19 14:33:19 $
------------------------------------------------------
* GTIE *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME  BASEDD
------------------------------------------------------
* MODULE XZAO * FICHIER xzao420.c
------------------------------------------------------
...
* JPL	05/11/18 : Lecture de l'indicateur 'Supprime' en configuration (DEM 1305)  1.5
* LCL	11/07/19 : Passage PAL/IP DEM13337 1.6
------------------------------------------------------*/
...
static char *version = "$Id: xzao420.c,v 1.6 2020/05/19 14:33:19 pc2dpdy Exp $ : xzao420" ;
...
* SERVICE RENDU : 
*
*    Lit le fichier de configuration des PAL et
...
int xzao420_Lire_Config_PAL ( 	FILE  		*pa_Fd,
...
*       XZAOT_AjoutPAL 	contenu de ligne
*       XDY_Entier 		NumLigne
...
{
   static char *version = "$Id: xzao420.c,v 1.6 2020/05/19 14:33:19 pc2dpdy Exp $ : xzao420_Lire_Config_PAL" ;
   
   XZAOT_AjoutPAL	vl_PAL;
   char			vl_Lig[255];
   char			*vl_Texte;
   ...
   */
   ...
   if (pa_Fd == NULL)
   {
     XZST_03EcritureTrace(XZSTC_MESSAGE,"xzao420: fichier non existant");
     return ( XDC_NOK );
   }
          ...
          */
...

          {
             ...
             */
             ...
             {  
                XZST_03EcritureTrace(XZSTC_WARNING,"xzao420 : Saisie  non valide");
                XZST_03EcritureTrace(XZSTC_WARNING,"xzao420 : retourne %d",XDC_NOK);
                return ( XDC_NOK );
             }
             
             strcpy ( vl_PAL.NomPAL, vl_Texte_out );
             ...
             strcpy ( vl_PAL.Autoroute, vl_Texte_out );
             ...
             strcpy ( vl_PAL.NomServeur, vl_Texte_out );
             ...
             vl_PAL.Liaison = atoi ( vl_Texte_out );
             ...
             strcpy ( vl_PAL.Rgs, vl_Texte_out );
             ...
             strcpy ( vl_PAL.NomSite, vl_Texte_out );
             ...
             vl_PAL.Supprime = atoi ( vl_Texte_out );
             ...
             strcpy ( vl_PAL.AdresseIP, vl_Texte_out );
             ...
             */
             ...
             if (pa_FonctionUtil != NULL)
             {
                ...
                */
                ...
                if ( (vl_retcode = ( *pa_FonctionUtil ) ( vl_PAL, vl_num_ligne ) ) != XDC_OK )
                {    
             ...
             else
             {
                ...
                */
                ...
                XZST_03EcritureTrace(XZSTC_MESSAGE,"xzao420: echec de l'appel de la fonction utilisateur");
                return ( XDC_NOK );
             }
          }
       }
...
/* Fin de la fonction xzao420_Lire_Config_PAL */
