/*E*/
/* Fichier : @(#)xzea22.c	1.5      Release : 1.5        Date : 12/13/95
------------------------------------------------------
* GTIE *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME : EQUEXT
------------------------------------------------------
* MODULE xzea  * FICHIER xzea22.c
------------------------------------------------------
...
* Mismer.D	version 1.3	18 Mai 1995	: Ajout type et numero Equipement en parametre d'entree
...
static char *version = "@(#)xzea22.c	1.5 12/13/95 : xzea22" ;
XDY_Fonction		pg_FonctUtil_XZEA_22 = NULL;
T_CB			vg_CB_XZEA22         = NULL;
...
------------------------------------------------------*/
{
   static char *version = "@(#)xzea22.c	1.5 12/13/95 : XZEA_Etat_Telealarme ";
   XZEAT_Telealarme	vl_Etat;
#ifndef _TIBCO_EMS
	T_INT4		vl_TypeEqt, vl_NumEqt, vl_Energie, vl_Local, vl_Lampe;
#else
	XDY_Entier	vl_TypeEqt, vl_NumEqt, vl_Energie, vl_Local, vl_Lampe;
#endif
   int			vl_Resultat = XDC_OK;
   XDY_NomMachine vl_machine="";

   /* Recuperation des parametres du message XDM_ETAT_TELEALARME */
   if ( !TipcMsgRead(pa_Data -> msg,
	       ...
	       NULL) ) 
   {
      XZST_03EcritureTrace ( XZSTC_WARNING," Impossible de lire le MSG XDM_ETAT_TELEALARME dans XZEA_Etat_Telealarme ");
   }
   else
   {	       
      XZST_03EcritureTrace ( XZSTC_FONCTION, 
      "IN  : XZEA_Etat_Telealarme TypeEqt:%d NumEqt:%d Energie:%d Local:%d Lampe:%d.",
      				 vl_TypeEqt,
      				 vl_NumEqt,
      				 vl_Energie,
      				 vl_Local,
      ...
      /*A Renseignement de la structure XZEAT_Telealarme */
      vl_Etat.TypeEqt		= vl_TypeEqt;
      vl_Etat.NumEqt		= vl_NumEqt;
      vl_Etat.Energie		= vl_Energie;
      vl_Etat.Local		= vl_Local;
      vl_Etat.Lampe		= vl_Lampe;

	  if (XZSC_GetDestinataire(pa_Data, vl_machine)!=XDC_OK) {
		XZST_03EcritureTrace(XZSTC_WARNING,"XZEA_Etat_Telealarme : probleme dans recuperation machine destinataire du message");
	  }

      /*A Appel de la fonction utilsateur avec la structure XZEAT_Telealarme en argument */
      pg_FonctUtil_XZEA_22 ( vl_Etat, &vl_Resultat, vl_machine );
      XZST_03EcritureTrace ( XZSTC_FONCTION,  "OUT : XZEA_Etat_Telealarme %d", vl_Resultat );
   }
}
...
int	XZEA22_Abt_Etat_Telealarme (	XDY_Booleen	va_Abonnement,
...
------------------------------------------------------*/
{
static char *version = "@(#)xzea22.c	1.5 12/13/95 : XZEA22_Abt_Etat_Telealarme ";
XDY_Datagroup	pl_NomDG;
int 		vl_ValRet = XDC_OK;
T_BOOL		vl_EtatDg = FALSE;

     XZST_03EcritureTrace ( XZSTC_FONCTION, 
     "IN  : XZEA22_Abt_Etat_Telealarme Abt:%d DGInit:%s Fonct:%ld NomSite:%s TypeEqt:%d NumEqt:%d",
     		va_Abonnement,
     		pa_DGInit,
     		pa_FonctUtilisateur,
     		pa_NomSite,
     ...
     pg_FonctUtil_XZEA_22 = pa_FonctUtilisateur;
     
     /*A Si demande d'abonnement */
     if ( va_Abonnement == XZEAC_DEBUT_ABONNEMENT )
     {
	/*A  Armement Callback sur Reception XDM_ETAT_TELEALARME */
	if ( vg_CB_XZEA22 == NULL )
	{
	   if ( ( vg_CB_XZEA22 = TipcSrvProcessCbCreate (	XDM_IdentMsg(XDM_ETAT_TELEALARME),
							      	XZEA_Etat_Telealarme,
								(T_CB_ARG)NULL ) ) == NULL ) 
	   {	
	      /*B Ecriture Trace */
	      XZST_03EcritureTrace ( XZSTC_WARNING, "Echec armement Callback sur XDM_ETAT_TELEALARME." );
	      vl_ValRet = XDC_NOK;
	   }
	   else
	   {	
	      /*B Ecriture Trace */
	      XZST_03EcritureTrace ( XZSTC_DEBUG1, "Armement Callback sur XDM_ETAT_TELEALARME." );
	   }
	}
	
	

	/*A Abonnement au Datagroup XDG_ETAT_TELEALARME<NomSite> */
	/*A Construction du nom du DataGroup */
	sprintf ( pl_NomDG, "%s_%s", XDG_ETAT_TELEALARME, pa_NomSite );
	...
	/*A Abonnement au Datagroup DGInit passe en parametre */
	   ...
	   /* Envoi du message de demande d'init XDM_EInit_IHM_NICE */
   	   ...
   	   if (!TipcSrvMsgWrite ( 	pl_NomDG,
  			  		...
  			  		XDM_FLG_SRVMSGWRITE,
               				...
               				T_IPC_FT_INT4, XDC_GTC_TELEALARME,
                          		NULL))
      	   {
     ...
     {
        if ( ( va_Abonnement == XZEAC_FERMER_ABONNEMENT ) && ( vg_CB_XZEA22 != NULL ) )
        {
           /*A  Desarmement Callback sur Reception XDM_ETAT_TELEALARME */
	   if ( TutCbDestroy ( vg_CB_XZEA22 ) == FALSE )  
	   {	
	      /*B Ecriture Trace */
	      XZST_03EcritureTrace ( XZSTC_WARNING, "Echec desarmement Callback sur XDM_ETAT_TELEALARME ." );
	      vl_ValRet = XDC_NOK;
	   }
	   else
	   {
	      XZST_03EcritureTrace ( XZSTC_DEBUG1, "Desarmement Callback sur XDM_ETAT_TELEALARME ." );
	      vg_CB_XZEA22 = NULL;
	   }
	}

	/*A Desabonnement au Datagroup XDG_ETAT_TELEALARME<NomSite> */
	/*A Construction du nom du DataGroup */
	sprintf ( pl_NomDG, "%s_%s", XDG_ETAT_TELEALARME, pa_NomSite );
   ...
   XZST_03EcritureTrace ( XZSTC_FONCTION, "OUT : XZEA22_Abt_Etat_Telealarme Retour:%d.", vl_ValRet );
