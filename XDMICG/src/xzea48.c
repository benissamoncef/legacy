/*E*/
/*Fichier :  $Id: xzea48.c,v 1.1 2008/02/13 13:25:00 devgtie Exp $      Release : $Revision: 1.1 $        Date : $Date: 2008/02/13 13:25:00 $
------------------------------------------------------
* GTIE *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME : EQUEXT
------------------------------------------------------
* MODULE xzea  * FICHIER xzea48.c
------------------------------------------------------
...
static char     *version             = "@(#)xzea48.c    1.1 09/21/07 : xzea48" ;
XDY_Fonction    pg_FonctUtil_XZEA_48 = NULL;
T_CB            vg_CB_XZEA48         = NULL;
...
*/
 ...
 void    XZEA_Etat_EqtLTN ( T_IPC_CONN                  va_Cnx,
...
------------------------------------------------------*/
{
    static char       *version    = "@(#)xzea48.c    1.1 09/21/07 : XZEA_Etat_EqtLTN ";
    ...
    if ( !TipcMsgRead( pa_Data -> msg,
                       ...
                       NULL) ) 
    {
        XZST_03EcritureTrace ( XZSTC_WARNING," Impossible de lire le MSG XDM_ETAT_EQUIPEMENT dans XZEA_Etat_EqtLTN ");
    }
    else
    {
        XZST_03EcritureTrace ( XZSTC_FONCTION, 
                               "IN  : XZEA_Etat_EqtLTN Domaine:%d TypeEqt:%d NumTube:%d TypeEquipement:%d NumEquipement:%d EtatEquipement:%d EtatTechnique:%d Mode:%d EtatDisc:%d ModeRafale:%d.",
                               vl_Domaine,
							   vl_TypeEqt,
                               vl_NumEqt,
                               vl_TypeEquipement,
	  ...
	  if (XZSC_GetDestinataire(pa_Data, vl_machine)!=XDC_OK) {
		XZST_03EcritureTrace(XZSTC_WARNING,"XZEA_Etat_EqtLTN : probleme dans recuperation machine destinataire du message");
	  }

      /*A Appel de la fonction utilsateur avec la structure XZEAT_Equipement en argument */
      pg_FonctUtil_XZEA_48 ( vl_Etat, &vl_Resultat, vl_machine );
      XZST_03EcritureTrace ( XZSTC_FONCTION,  "OUT : XZEA_Etat_EqtLTN %d", vl_Resultat );
   }
}
...
int    XZEA48_Abt_Etat_EqtLTN ( XDY_Booleen    va_Abonnement,
...
------------------------------------------------------*/
{
static char      *version  = "@(#)xzea48.c    1.1 09/21/07 : XZEA48_Abt_Etat_EqtLTN ";
...
XDY_Entier       vl_NumEqt  = va_NumEqt;
    ...
    XZST_03EcritureTrace ( XZSTC_FONCTION, 
                           "IN  : XZEA48_Abt_Etat_EqtLTN Abt:%d DGInit:%s Fonct:%ld NomSite:%s TypeEqt:%d NumEqt:%d.",
                           va_Abonnement,
                           pa_DGInit,
                           pa_FonctUtilisateur,
                           pa_NomSite,
    ...
    pg_FonctUtil_XZEA_48 = pa_FonctUtilisateur;

    /*A Si demande d'abonnement */
    if ( va_Abonnement == XZEAC_DEBUT_ABONNEMENT )
    {
        /*A  Armement Callback sur Reception XDM_ETAT_EQUIPEMENT */
        if ( vg_CB_XZEA48 == NULL )
        {
            if ( ( vg_CB_XZEA48 = TipcSrvProcessCbCreate ( XDM_IdentMsg(XDM_ETAT_EQUIPEMENT),
                                                           XZEA_Etat_EqtLTN,
                                                           (T_CB_ARG)NULL ) ) == NULL ) 
            {    
                /*B Ecriture Trace */
    ...
    {
        if ( ( va_Abonnement == XZEAC_FERMER_ABONNEMENT ) && ( vg_CB_XZEA48 != NULL ) )
        {
            /*A  Desarmement Callback sur Reception XDM_ETAT_EQUIPEMENT */
            if ( TutCbDestroy ( vg_CB_XZEA48 ) == FALSE )  
            {    
                /*B Ecriture Trace */
    ...
    XZST_03EcritureTrace ( XZSTC_FONCTION, "OUT : XZEA48_Abt_Etat_EqtLTN Retour:%d.", vl_ValRet );
