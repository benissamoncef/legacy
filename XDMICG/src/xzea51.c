/*E*/
/*Fichier :  $Id: xzea51.c,v 1.1 2008/02/13 13:25:00 devgtie Exp $      Release : $Revision: 1.1 $        Date : $Date: 2008/02/13 13:25:00 $
------------------------------------------------------
* GTIE *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME : EQUEXT
------------------------------------------------------
* MODULE xzea  * FICHIER xzea51.c
------------------------------------------------------
...
static char   *version             = "@(#)xzea51.c    1.1 09/21/07 : xzea51" ;
XDY_Fonction  pg_FonctUtil_XZEA_51 = NULL;
T_CB          vg_CB_XZEA51         = NULL;
...
*/
 ...
 void   XZEA_Etat_Sequence_ECL ( T_IPC_CONN                  va_Cnx,
...
------------------------------------------------------*/
{
    static char       *version    = "@(#)xzea51.c    1.1 09/21/07 : XZEA_Etat_Sequence_ECL ";
    ...
    /* Recuperation des parametres du message XDM_ETAT_SEQECL */
    if ( !TipcMsgRead( pa_Data -> msg,
                       ...
                       NULL) ) 
    {
        XZST_03EcritureTrace ( XZSTC_WARNING," Impossible de lire le MSG XDM_ETAT_SEQECL dans XZEA_Etat_Sequence_ECL ");
    }
    else
    {           
		XZST_03EcritureTrace ( XZSTC_FONCTION, 
							   "IN  : XZEA_Etat_Sequence_ECL TypeEqt:%d NumEqt:%d SeqCourant:%d SeqChange:%d EtatSeq:%d.",
							   vl_TypeEqt,
							   vl_NumEqt,
							   vl_SeqCourante,
							   vl_SeqChange,
        ...
        if (XZSC_GetDestinataire(pa_Data, vl_machine)!=XDC_OK) {
	        XZST_03EcritureTrace(XZSTC_WARNING,"XZEA_Etat_Sequence_ECL : probleme dans recuperation machine destinataire du message");
	  }

		/*A Appel de la fonction utilsateur avec la structure XZEAT_Sequence en argument */
		pg_FonctUtil_XZEA_51 ( vl_Etat, &vl_Resultat, vl_machine );
		XZST_03EcritureTrace ( XZSTC_FONCTION,  "OUT : XZEA_Etat_Sequence_ECL %d", vl_Resultat );
    }
}
...
int    XZEA51_Abt_Etat_Sequence_ECL ( XDY_Booleen    va_Abonnement,
...
*
* CONDITION D'UTILISATION
*   Ce service est demande pour s'abonner aux etats des sequences eqt eclairage de la GTC Nice
*
* FONCTION 
...
------------------------------------------------------*/
{
static char     *version  = "@(#)xzea51.c    1.1 09/21/07 : XZEA51_Abt_Etat_Sequence_ECL ";
...
XDY_Entier       vl_NumEqt  = va_NumEqt;
    ...
    XZST_03EcritureTrace ( XZSTC_FONCTION, 
                           "IN  : XZEA51_Abt_Etat_Sequence_ECL Abt:%d DGInit:%s Fonct:%ld NumTube:%d NomSite:%s.",
                           va_Abonnement,
                           pa_DGInit,
                           pa_FonctUtilisateur,
                           va_NumEqt,
    ...
    pg_FonctUtil_XZEA_51 = pa_FonctUtilisateur;

    /*A Si demande d'abonnement */
    if ( va_Abonnement == XZEAC_DEBUT_ABONNEMENT )
    {
        /*A  Armement Callback sur Reception XDM_ETAT_SEQECL */
        if ( vg_CB_XZEA51 == NULL )
        {
            if ( ( vg_CB_XZEA51 = TipcSrvProcessCbCreate ( XDM_IdentMsg(XDM_ETAT_SEQECL),
                                                           XZEA_Etat_Sequence_ECL,
                                                           (T_CB_ARG)NULL ) ) == NULL ) 
            {
                /*B Ecriture Trace */
                XZST_03EcritureTrace ( XZSTC_WARNING, "Echec armement Callback sur XDM_ETAT_SEQECL." );
                vl_ValRet = XDC_NOK;
            }
            else
            {    
                /*B Ecriture Trace */
                XZST_03EcritureTrace ( XZSTC_DEBUG1, "Armement Callback sur XDM_ETAT_SEQECL." );
            }
        }

        /*A Abonnement au Datagroup XDG_ETAT_SEQ_ECL<NomSite> */
        /*A Construction du nom du DataGroup */
        sprintf ( pl_NomDG, "%s_%s", XDG_ETAT_SEQ_ECL, pa_NomSite );
    ...
    {
        if ( ( va_Abonnement == XZEAC_FERMER_ABONNEMENT ) && ( vg_CB_XZEA51 != NULL ) )
        {
            /*A  Desarmement Callback sur Reception XDM_ETAT_SEQECL */
            if ( TutCbDestroy ( vg_CB_XZEA51 ) == FALSE )  
            {    
                /*B Ecriture Trace */
                XZST_03EcritureTrace ( XZSTC_WARNING, "Echec desarmement Callback sur XDM_ETAT_SEQECL ." );
                vl_ValRet = XDC_NOK;
            }
            else
            {
                XZST_03EcritureTrace ( XZSTC_DEBUG1, "Desarmement Callback sur XDM_ETAT_SEQECL ." );
                vg_CB_XZEA51 = NULL;
            }
        }

        /*A Desabonnement au Datagroup XDG_ETAT_SEQ_ECL<NomSite> */
        /*A Construction du nom du DataGroup */
        sprintf ( pl_NomDG, "%s_%s", XDG_ETAT_SEQ_ECL, pa_NomSite );
    ...
    XZST_03EcritureTrace ( XZSTC_FONCTION, "OUT : XZEA51_Abt_Etat_Sequence_ECL Retour:%d.", vl_ValRet );
