/*E*/
/*Fichier :  $Id: xzea42.c,v 1.2 2020/11/03 17:57:48 pc2dpdy Exp $      Release : $Revision: 1.2 $        Date : $Date: 2020/11/03 17:57:48 $
------------------------------------------------------
* GTIE *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME : EQUEXT
------------------------------------------------------
* MODULE xzea  * FICHIER xzea42.c
------------------------------------------------------
...
static char     *version             = "@(#)xzea42.c    1.1 09/21/07 : xzea42" ;
XDY_Fonction    pg_FonctUtil_XZEA_42 = NULL;
T_CB            vg_CB_XZEA42         = NULL;
...
*/
 ...
 void    XZEA_Etat_EqtSIG ( T_IPC_CONN                  va_Cnx,
...
------------------------------------------------------*/
{
    static char       *version    = "@(#)xzea42.c    1.1 09/21/07 : XZEA_Etat_EqtSIG ";
    ...
    if ( !TipcMsgRead( pa_Data -> msg,
		       ...
		       T_IPC_FT_INT2, &vl_ModeRafale,
                       NULL) ) 
    {
        XZST_03EcritureTrace ( XZSTC_WARNING," Impossible de lire le MSG XDM_ETAT_EQUIPEMENT dans XZEA_Etat_EqtSIG ");
    }
    else
    {
        XZST_03EcritureTrace ( XZSTC_FONCTION, 
                               "IN  : XZEA_Etat_EqtSIG Domaine: %d TypeEqt:%d NumTube:%d TypeEquipement:%d NumEquipement:%d EtatEquipement:%d EtatTechnique:%d Mode:%d EtatDisc:%d ModeRafale:%d.",
                               vl_Domaine,
                               vl_TypeEqt,
                               vl_NumEqt,
                               vl_TypeEquipement,
        ...
        if (XZSC_GetDestinataire(pa_Data, vl_machine)!=XDC_OK) {
	        XZST_03EcritureTrace(XZSTC_WARNING,"XZEA_Etat_EqtSIG : probleme dans recuperation machine destinataire du message");
	  }

      /*A Appel de la fonction utilsateur avec la structure XZEAT_Equipement en argument */
      pg_FonctUtil_XZEA_42 ( vl_Etat, &vl_Resultat, vl_machine );
      XZST_03EcritureTrace ( XZSTC_FONCTION,  "OUT : XZEA_Etat_EqtSIG %d", vl_Resultat );
   }
}
...
int    XZEA42_Abt_Etat_EqtSIG ( XDY_Booleen    va_Abonnement,
...
------------------------------------------------------*/
{
static char      *version  = "@(#)xzea42.c    1.1 09/21/07 : XZEA42_Abt_Etat_EqtSIG ";
...
XDY_Entier       vl_NumEqt  = va_NumEqt;
    ...
    XZST_03EcritureTrace ( XZSTC_FONCTION, 
                           "IN  : XZEA42_Abt_Etat_EqtSIG Abt:%d DGInit:%s Fonct:%ld NomSite:%s TypeEqt:%d NumEqt:%d.",
                           va_Abonnement,
                           pa_DGInit,
                           pa_FonctUtilisateur,
                           pa_NomSite,
    ...
    pg_FonctUtil_XZEA_42 = pa_FonctUtilisateur;

    /*A Si demande d'abonnement */
    if ( va_Abonnement == XZEAC_DEBUT_ABONNEMENT )
    {
        /*A  Armement Callback sur Reception XDM_ETAT_EQUIPEMENT */
        if ( vg_CB_XZEA42 == NULL )
        {
            if ( ( vg_CB_XZEA42 = TipcSrvProcessCbCreate ( XDM_IdentMsg(XDM_ETAT_EQUIPEMENT),
                                                           XZEA_Etat_EqtSIG,
                                                           (T_CB_ARG)NULL ) ) == NULL ) 
            {    
                /*B Ecriture Trace */
        ...
        /*A Construction du DatGroup XDM_EInit_IHM_NICERenovee<NomSite> */
    ...
    {
        if ( ( va_Abonnement == XZEAC_FERMER_ABONNEMENT ) && ( vg_CB_XZEA42 != NULL ) )
        {
            /*A  Desarmement Callback sur Reception XDM_ETAT_EQUIPEMENT */
            if ( TutCbDestroy ( vg_CB_XZEA42 ) == FALSE )  
            {    
                /*B Ecriture Trace */
    ...
    XZST_03EcritureTrace ( XZSTC_FONCTION, "OUT : XZEA42_Abt_Etat_EqtSIG Retour:%d.", vl_ValRet );
