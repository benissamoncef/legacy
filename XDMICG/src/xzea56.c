/*E*/
/*Fichier :  $Id: xzea56.c,v 1.1 2008/02/13 13:25:01 devgtie Exp $      Release : $Revision: 1.1 $        Date : $Date: 2008/02/13 13:25:01 $
------------------------------------------------------
...
XDY_Fonction  pg_FonctUtil_XZEA_56 = NULL;
T_CB          vg_CB_XZEA56         = NULL;
...
*/
 ...
 void   XZEA_Etat_Sequence_EXP ( T_IPC_CONN                  va_Cnx,
...
------------------------------------------------------*/
{
    static char       *version    = "@(#)xzea53.c    1.1 09/21/07 : XZEA_Etat_Sequence_EXP ";
    ...
    /* Recuperation des parametres du message XDM_ETAT_SEQEXP */
    if ( !TipcMsgRead( pa_Data -> msg,
                       ...
                       NULL) ) 
    {
        XZST_03EcritureTrace ( XZSTC_WARNING," Impossible de lire le MSG XDM_ETAT_SEQEXP dans XZEA_Etat_Sequence_EXP ");
    }
    else
    {           
		XZST_03EcritureTrace ( XZSTC_FONCTION, 
							   "IN  : XZEA_Etat_Sequence_EXP TypeEqt:%d NumEqt:%d SeqCourant:%d SeqChange:%d EtatSeq:%d.",
							   vl_TypeEqt,
							   vl_NumEqt,
							   vl_SeqCourante,
							   vl_SeqChange,
        ...
        if (XZSC_GetDestinataire(pa_Data, vl_machine)!=XDC_OK) {
	        XZST_03EcritureTrace(XZSTC_WARNING,"XZEA_Etat_Sequence_EXP : probleme dans recuperation machine destinataire du message");
	  }

		/*A Appel de la fonction utilsateur avec la structure XZEAT_Sequence en argument */
		pg_FonctUtil_XZEA_56 ( vl_Etat, &vl_Resultat, vl_machine );
		XZST_03EcritureTrace ( XZSTC_FONCTION,  "OUT : XZEA_Etat_Sequence_EXP %d", vl_Resultat );
    }
}
...
int    XZEA56_Abt_Etat_Sequence_EXP ( XDY_Booleen    va_Abonnement,
...
------------------------------------------------------*/
{
static char     *version  = "@(#)xzea53.c    1.1 09/21/07 : XZEA56_Abt_Etat_Sequence_EXP ";
...
XDY_Entier       vl_NumEqt  = va_NumEqt;
    ...
    XZST_03EcritureTrace ( XZSTC_FONCTION, 
                           "IN  : XZEA56_Abt_Etat_Sequence_EXP Abt:%d DGInit:%s Fonct:%ld NumTube:%d NomSite:%s.",
                           va_Abonnement,
                           pa_DGInit,
                           pa_FonctUtilisateur,
                           va_NumEqt,
    ...
    pg_FonctUtil_XZEA_56 = pa_FonctUtilisateur;

    /*A Si demande d'abonnement */
    if ( va_Abonnement == XZEAC_DEBUT_ABONNEMENT )
    {
        /*A  Armement Callback sur Reception XDM_ETAT_SEQEXP */
        if ( vg_CB_XZEA56 == NULL )
        {
            if ( ( vg_CB_XZEA56 = TipcSrvProcessCbCreate ( XDM_IdentMsg(XDM_ETAT_SEQEXP),
                                                           XZEA_Etat_Sequence_EXP,
                                                           (T_CB_ARG)NULL ) ) == NULL ) 
            {
                /*B Ecriture Trace */
                XZST_03EcritureTrace ( XZSTC_WARNING, "Echec armement Callback sur XDM_ETAT_SEQEXP." );
                vl_ValRet = XDC_NOK;
            }
            else
            {    
                /*B Ecriture Trace */
                XZST_03EcritureTrace ( XZSTC_DEBUG1, "Armement Callback sur XDM_ETAT_SEQEXP." );
            }
        }

        /*A Abonnement au Datagroup XDG_ETAT_SEQ_EXP<NomSite> */
        /*A Construction du nom du DataGroup */
        sprintf ( pl_NomDG, "%s_%s", XDG_ETAT_SEQ_EXP, pa_NomSite );
    ...
    {
        if ( ( va_Abonnement == XZEAC_FERMER_ABONNEMENT ) && ( vg_CB_XZEA56 != NULL ) )
        {
            /*A  Desarmement Callback sur Reception XDM_ETAT_SEQEXP */
            if ( TutCbDestroy ( vg_CB_XZEA56 ) == FALSE )  
            {    
                /*B Ecriture Trace */
                XZST_03EcritureTrace ( XZSTC_WARNING, "Echec desarmement Callback sur XDM_ETAT_SEQEXP ." );
                vl_ValRet = XDC_NOK;
            }
            else
            {
                XZST_03EcritureTrace ( XZSTC_DEBUG1, "Desarmement Callback sur XDM_ETAT_SEQEXP ." );
                vg_CB_XZEA56 = NULL;
            }
        }

        /*A Desabonnement au Datagroup XDG_ETAT_SEQ_EXP<NomSite> */
        /*A Construction du nom du DataGroup */
        sprintf ( pl_NomDG, "%s_%s", XDG_ETAT_SEQ_EXP, pa_NomSite );
    ...
    XZST_03EcritureTrace ( XZSTC_FONCTION, "OUT : XZEA56_Abt_Etat_Sequence_EXP Retour:%d.", vl_ValRet );
