/*E*/
/*Fichier : @(#)xzea03.c	1.8      Release : 1.8        Date : 12/13/95
------------------------------------------------------
* GTIE *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME : EQUEXT
------------------------------------------------------
* MODULE xzea  * FICHIER xzea03.c
------------------------------------------------------
...
* Mismer.D	version 1.5	08 Nov 1994	: Suppression arg NumEqt pour XZEA03 
...
static char *version = "@(#)xzea03.c	1.8 12/13/95 : xzea03" ;
XDY_Fonction		pg_FonctUtil_XZEA_03 = NULL;
T_CB			vg_CB_XZEA03         = NULL;
...
------------------------------------------------------*/
{
   static char *version = "@(#)xzea03.c	1.8 12/13/95 : XZEA_EMes_6mm_Voie ";
   ...
   /* Recuperation des parametres du message XDM_EMES_6mn_Voie */
   ...
   "IN  :  XZEA_EMes_6mm_Voie Horodate:%lf Numero:%d Periode:%d Sens:%d Voie:%d Q:%d V:%d TO:%d PL:%d.",
   ...
   pg_FonctUtil_XZEA_03 ( vl_Mes, &vl_Resultat );
   XZST_03EcritureTrace ( XZSTC_FONCTION,  "OUT : XZEA_EMes_6mm_Voie %d", vl_Resultat );
}
...
int	XZEA03_Abt_Mesures_6mn_Voie (	XDY_Booleen	va_Abonnement,
...
T_BOOL		vl_EtatDg = FALSE;
     ...
     XZST_03EcritureTrace ( XZSTC_FONCTION, 
     "IN  : XZEA03_Abt_Mesures_6mn_Voie Abt:%d DGInit:%s Fonct:%ld NomSite:%s .",
     		va_Abonnement,
     		pa_DGInit,
     		pa_FonctUtilisateur,
     		pa_NomSite );
     ...
     pg_FonctUtil_XZEA_03 = pa_FonctUtilisateur;
     
     /*A Si demande d'abonnement */
     if ( va_Abonnement == XZEAC_DEBUT_ABONNEMENT )
     {
	/*A  Armement Callback sur Reception XDM_EMES_6mn_Voie */
	if ( vg_CB_XZEA03 == NULL )
	{
	   if ( ( vg_CB_XZEA03 = TipcSrvProcessCbCreate (	XDM_IdentMsg(XDM_EMES_6mn_Voie),
							      	XZEA_EMes_6mm_Voie,
								(T_CB_ARG)NULL ) ) == NULL ) 
	   {	
	      /*B Ecriture Trace */
	      XZST_03EcritureTrace ( XZSTC_WARNING, "Echec armement Callback sur XDM_EMES_6mn_Voie ." );
	      vl_ValRet = XDC_NOK;
	   }
	   else
	   {	
	      /*B Ecriture Trace */
	      XZST_03EcritureTrace ( XZSTC_DEBUG1, "Armement Callback sur XDM_EMES_6mn_Voie ." );
	   }
	}
	...
	/*A Abonnement au Datagroup DGInit passe en parametre */
     ...
     {
        if ( ( va_Abonnement == XZEAC_FERMER_ABONNEMENT ) && ( vg_CB_XZEA03 != NULL ) )
        {
           /*A  Desarmement Callback sur Reception XDM_EMES_6mn_Voie */
	   if ( TutCbDestroy ( vg_CB_XZEA03 ) == FALSE )  
	   {	
	      /*B Ecriture Trace */
	      XZST_03EcritureTrace ( XZSTC_WARNING, "Echec desarmement Callback sur XDM_EMES_6mn_Voie ." );
	      vl_ValRet = XDC_NOK;
	   }
	   else
	   {
	      XZST_03EcritureTrace ( XZSTC_DEBUG1, "Desarmement Callback sur XDM_EMES_6mn_Voie ." );
	      vg_CB_XZEA03 = NULL;
	   }
	}
   ...
   XZST_03EcritureTrace ( XZSTC_FONCTION, "OUT : XZEA03_Abt_Mesures_6mn_Voie Retour:%d.", vl_ValRet );
