/*E*/
/*Fichier : $Id: xzao175.c,v 1.3 2016/07/13 12:43:09 devgfi Exp $      Release : $Revision: 1.3 $        Date : $Date: 2016/07/13 12:43:09 $
------------------------------------------------------
* GTIE *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME  BASEDD
------------------------------------------------------
* MODULE XZAO * FICHIER xzao175.c
------------------------------------------------------
...
static char *version = "$Id: xzao175.c,v 1.3 2016/07/13 12:43:09 devgfi Exp $ : xzao175" ;
...
* SERVICE RENDU : 
*
*    Lit le fichier de configuration des Demi echangerus et
...
int xzao175_Lire_Config_Demi_Ech ( 	FILE  		*pa_Fd,
...
*       XZAOT_ConfDemiEch 	contenu de ligne
*       XDY_Entier 		NumLigne
...
{
   static char *version = "$Id: xzao175.c,v 1.3 2016/07/13 12:43:09 devgfi Exp $ : xzao175_Lire_Config_Demi_Ech" ;
   
   XZAOT_ConfDemiEch	vl_DemiEch;
   char			vl_Lig[255];
   char			*vl_Texte;
   ...
   */
   ...
   if (pa_Fd == NULL)
   {
     XZST_03EcritureTrace(XZSTC_MESSAGE,"xzao175: fichier non existant");
     return ( XDC_NOK );
   }
          ...
          */
...

          {
             ...
             */
             ...
             {  
                XZST_03EcritureTrace(XZSTC_WARNING,"xzao175 : Saisie  non valide");
                XZST_03EcritureTrace(XZSTC_WARNING,"xzao175 : retourne %d",XDC_NOK);
                return ( XDC_NOK );
             }
             
             strcpy ( vl_DemiEch.NomDemiEch, vl_Texte_out );
             ...
             strcpy ( vl_DemiEch.Autoroute, vl_Texte_out );
             ...
             strcpy ( vl_DemiEch.NomServeur, vl_Texte_out );
             ...
             vl_DemiEch.PR = atoi ( vl_Texte_out );
             ...
             vl_DemiEch.Sens = atoi ( vl_Texte_out );
             ...
             vl_DemiEch.AdresseEtat = atoi ( vl_Texte_out );
             ...
             vl_DemiEch.AdresseCommande = atoi ( vl_Texte_out );
             ...
             vl_DemiEch.AdresseDiscordance = atoi ( vl_Texte_out );
             ...
             vl_DemiEch.AdresseTelealarme = atoi ( vl_Texte_out );
             ...
             */
             ...
             if (pa_FonctionUtil != NULL)
             {
                ...
                */
                ...
                if ( (vl_retcode = ( *pa_FonctionUtil ) ( vl_DemiEch, vl_num_ligne ) ) != XDC_OK )
                {    
             ...
             else
             {
                ...
                */
                ...
                XZST_03EcritureTrace(XZSTC_MESSAGE,"xzao175: echec de l'appel de la fonction utilisateur");
                return ( XDC_NOK );
             }
          }
       }
