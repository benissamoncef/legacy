/*E*/
/*Fichier : @(#)xzao443.c	1.1      Release : 1.1        Date :  12/11/00
------------------------------------------------------
* STERIA *  PROJET MIGRAZUR
------------------------------------------------------
* SOUS-SYSTEME  BASEDD
------------------------------------------------------
* MODULE XZAO * FICHIER xzao443.c
------------------------------------------------------
...
#include "xzao.h"
#include "xzao443.h"
static char *version = "@(#)xzao443.c	1.1 1.1 12/11/00 : xzao443.c" ;
...
int XZAO443_Rechercher_les_PMVA_ds_pert (	XDY_Autoroute	va_autoroute_in,
						XDY_Sens	va_sens_in,
						XDY_PR		va_pr_in,
...
{
	CS_CHAR *rpc_name = XZAOC_XZAO443_RPC_NAME;
	CS_CONTEXT      *context;
	...
	*/
	...
	XZST_03EcritureTrace( XZSTC_INTERFACE,"IN : XZAO443_Rechercher_les_PMVA_ds_pert aut=%d, sens = %d,PR=%d, PR tete=%d, Dmax=%d, Dmaxbifurc=%d,PR bifamont=%d,PR bifamont tete=%d,aut bifamont=%d,aut bifamont tete=%d, PR deb bifamont=%d, PR deb bifamont tete=%d",
						va_autoroute_in,
						va_sens_in,
						va_pr_in,
						va_pr_tete_in,
	...
	*/
	...
	{
	 	XZST_03EcritureTrace(XZSTC_WARNING,"xzao443: ct_cmd_alloc() a echoue");
		XZST_03EcritureTrace(XZSTC_INTERFACE, "OUT : XZAO443_Rechercher_les_PMVA_ds_pert retourne %d", retcode);
	...
	*/
	...
	{
		XZST_03EcritureTrace(XZSTC_WARNING,"xzao443: ct_command() a echoue");
		XZST_03EcritureTrace(XZSTC_INTERFACE, "OUT : XZAO443_Rechercher_les_PMVA_ds_pert retourne %d", retcode);
		asql_clean_cmd(connection, cmd);
		return (XDC_NOK);
	}
	...
	*/
	...
	{
    	   XZST_03EcritureTrace(XZSTC_WARNING,"xzao443: asql_ctparam_tinyint_input(va_Autoroute_in) a echoue");
	   XZST_03EcritureTrace(XZSTC_INTERFACE, "OUT : XZAO443_Rechercher_les_PMVA_ds_pert retourne %d", retcode);
	   asql_clean_cmd(connection, cmd);
	   return (XDC_NOK);
	}
	...
	{
	   XZST_03EcritureTrace(XZSTC_WARNING,"xzao443: asql_ctparam_tinyint_input(va_Sens_in) a echoue");
           XZST_03EcritureTrace(XZSTC_INTERFACE, "OUT : XZAO443_Rechercher_les_PMVA_ds_pert retourne %d", retcode);
  	   asql_clean_cmd(connection, cmd);
	   return (XDC_NOK);
        }
	...
	{
	   XZST_03EcritureTrace(XZSTC_WARNING,"xzao443: asql_ctparam_int_input(va_distance_in) a echoue");
           XZST_03EcritureTrace(XZSTC_INTERFACE, "OUT : XZAO443_Rechercher_les_PMVA_ds_pert retourne %d", retcode);
  	   asql_clean_cmd(connection, cmd);
	   return (XDC_NOK);
        }
	...
	{
	   XZST_03EcritureTrace(XZSTC_WARNING,"xzao443: asql_ctparam_int_input(va_Dmax_bifurc_in) a echoue");
           XZST_03EcritureTrace(XZSTC_INTERFACE, "OUT : XZAO443_Rechercher_les_PMVA_ds_pert retourne %d", retcode);
  	   asql_clean_cmd(connection, cmd);
	   return (XDC_NOK);
        }
	...
	{
	   XZST_03EcritureTrace(XZSTC_WARNING,"xzao443: asql_ctparam_int_input(va_pr_bifamont_in) a echoue");
           XZST_03EcritureTrace(XZSTC_INTERFACE, "OUT : XZAO443_Rechercher_les_PMVA_ds_pert retourne %d", retcode);
  	   asql_clean_cmd(connection, cmd);
	   return (XDC_NOK);
        }
	...
	{
	   XZST_03EcritureTrace(XZSTC_WARNING,"xzao443: asql_ctparam_int_input(va_pr_bifamont_tete_in) a echoue");
           XZST_03EcritureTrace(XZSTC_INTERFACE, "OUT : XZAO443_Rechercher_les_PMVA_ds_pert retourne %d", retcode);
  	   asql_clean_cmd(connection, cmd);
	   return (XDC_NOK);
        }
	...
	{
	   XZST_03EcritureTrace(XZSTC_WARNING,"xzao443: asql_ctparam_tinyint_input(va_aut_bifamont_in) a echoue");
           XZST_03EcritureTrace(XZSTC_INTERFACE, "OUT : XZAO443_Rechercher_les_PMVA_ds_pert retourne %d", retcode);
  	   asql_clean_cmd(connection, cmd);
	   return (XDC_NOK);
        }
	...
	{
	   XZST_03EcritureTrace(XZSTC_WARNING,"xzao443: asql_ctparam_tinyint_input(va_aut_bifamont_tete_in) a echoue");
           XZST_03EcritureTrace(XZSTC_INTERFACE, "OUT : XZAO443_Rechercher_les_PMVA_ds_pert retourne %d", retcode);
  	   asql_clean_cmd(connection, cmd);
	   return (XDC_NOK);
        }
	...
	{
	   XZST_03EcritureTrace(XZSTC_WARNING,"xzao443: asql_ctparam_int_input(va_pr_debut_bifamont_in) a echoue");
           XZST_03EcritureTrace(XZSTC_INTERFACE, "OUT : XZAO443_Rechercher_les_PMVA_ds_pert retourne %d", retcode);
  	   asql_clean_cmd(connection, cmd);
	   return (XDC_NOK);
        }
	...
	{
	   XZST_03EcritureTrace(XZSTC_WARNING,"xzao443: asql_ctparam_int_input(va_pr_debut_bifamont_tete_in) a echoue");
           XZST_03EcritureTrace(XZSTC_INTERFACE, "OUT : XZAO443_Rechercher_les_PMVA_ds_pert retourne %d", retcode);
  	   asql_clean_cmd(connection, cmd);
	   return (XDC_NOK);
        }
	...
	*/
	...
	{
		XZST_03EcritureTrace(XZSTC_WARNING,"xzao443: ct_send() a echoue");
		XZST_03EcritureTrace(XZSTC_INTERFACE, "OUT : XZAO443_Rechercher_les_PMVA_ds_pert retourne %d", retcode);
		asql_clean_cmd(connection, cmd);
		return (XDC_NOK);
	}
				...
				*/
				switch ((int)res_type)
				{
					case  CS_ROW_RESULT:
						...
						XZST_03EcritureTrace(XZSTC_INFO,"xzao443: ROW RESULTS");
						break;

					case  CS_PARAM_RESULT:
						XZST_03EcritureTrace(XZSTC_INFO,"xzao443 : PARAMETER RESULTS");
						break;

					case  CS_STATUS_RESULT:
						retcode = asql_fetch (cmd, &SPstatus);
						XZST_03EcritureTrace(XZSTC_INFO,"xzao443 : CS_STATUS_RESULT = %d", retcode);
						XZST_03EcritureTrace(XZSTC_INFO,"xzao443 : STATUS REUSULT PROCEDURE STOCKEE = %d", SPstatus);
				...
				case CS_MSG_RESULT:
					...
					*/
					...
					{
						XZST_03EcritureTrace(XZSTC_WARNING,"xzao443: ct_res_info(msgtype) a echoue");
						XZST_03EcritureTrace(XZSTC_INTERFACE, "OUT : XZAO443_Rechercher_les_PMVA_ds_pert retourne %d", retcode);
						asql_clean_cmd(connection, cmd);
						return (XDC_NOK);
					}
				...
				case CS_CMD_SUCCEED:
					...
					*/
					XZST_03EcritureTrace(XZSTC_INFO,"xzao443: ct_results retourne CS_CMD_SUCCEED.");
					break;
				...
				case CS_CMD_DONE:
					...
					*/
					XZST_03EcritureTrace(XZSTC_INFO,"xzao443: ct_results retourne CS_CMD_DONE.");
					break;
				...
				case CS_CMD_FAIL:
					...
					*/
					XZST_03EcritureTrace(XZSTC_WARNING,"xzao443: ct_results retourne CS_CMD_FAIL.");
					XZST_03EcritureTrace(XZSTC_INTERFACE, "OUT : XZAO443_Rechercher_les_PMVA_ds_pert retourne %d", retcode);
					asql_clean_cmd(connection, cmd);
					return (XDC_NOK);
				...
				default:
					...
					*/
					XZST_03EcritureTrace(XZSTC_WARNING,"xzao443: ct_results retourne un resultat de type inattendu");
					XZST_03EcritureTrace(XZSTC_INTERFACE, "OUT : XZAO443_Rechercher_les_PMVA_ds_pert retourne %d", retcode);
					asql_clean_cmd(connection, cmd);
					return (XDC_NOK);
				}
	...
	*va_Resultat_out = 0;
	...
	XZST_03EcritureTrace(XZSTC_INFO,"xzao443 : nb de PMV = %d",vl_Nb_Ligne_Lue);
