/*E*/
/* Fichier : @(#)xzea16.c	1.4      Release : 1.4        Date : 12/13/95
------------------------------------------------------
* GTIE *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME : EQUEXT
------------------------------------------------------
* MODULE xzea  * FICHIER xzea16.c
------------------------------------------------------
...
static char *version = "@(#)xzea16.c	1.4 12/13/95 : xzea16" ;
XDY_Fonction		pg_FonctUtil_XZEA_16 = NULL;
T_CB			vg_CB_XZEA16         = NULL;
...
------------------------------------------------------*/
{
   static char *version = "@(#)xzea16.c	1.4 12/13/95 : XZEA_Etat_Capteur ";
   XZEAT_Capteur	vl_Etat;
#ifndef _TIBCO_EMS
	T_INT4		vl_NumTube, vl_NumCapt, vl_EtatCapt;
#else
	XDY_Entier	vl_NumTube, vl_NumCapt, vl_EtatCapt;
#endif
   int			vl_Resultat = XDC_OK;
   
		  
   /* Recuperation des parametres du message XDM_ETAT_CAPTEUR */
   if ( !TipcMsgRead(pa_Data -> msg,
               T_IPC_FT_INT4,        &vl_NumTube,
               T_IPC_FT_INT4,        &vl_NumCapt,
               T_IPC_FT_INT4,        &vl_EtatCapt,
	       NULL) ) 
   {
      XZST_03EcritureTrace ( XZSTC_WARNING," Impossible de lire le MSG XDM_ETAT_CAPTEUR dans XZEA_Etat_Capteur ");
   }
   else
   {	       
      XZST_03EcritureTrace ( XZSTC_FONCTION, 
      "IN  : XZEA_Etat_Capteur NumTube:%d NumCapt:%d EtatCapt:%d .",
      				 vl_NumTube,
      				 vl_NumCapt,
      				 vl_EtatCapt );

      /*A Renseignement de la structure XZEAT_Capteur */
      vl_Etat.NumCapt		= vl_NumCapt;
      vl_Etat.NumTube		= vl_NumTube;
      vl_Etat.EtatCapt		= vl_EtatCapt;
	       
      /*A Appel de la fonction utilsateur avec la structure XZEAT_Capteur en argument */
      pg_FonctUtil_XZEA_16 ( vl_Etat, &vl_Resultat );
      XZST_03EcritureTrace ( XZSTC_FONCTION,  "OUT : XZEA_Etat_Capteur %d", vl_Resultat );
   }
}
...
/*------------------------------------------------------
* SERVICE RENDU : 
*  < Abonnement aux etats des capteur de la GTC de Nice ( Lits d'arret et extincteurs  >
------------------------------------------------------
* SEQUENCE D'APPEL :
*/

int	XZEA16_Abt_Etat_Capteur (	XDY_Booleen	va_Abonnement,
...
------------------------------------------------------*/
{
static char *version = "@(#)xzea16.c	1.4 12/13/95 : XZEA16_Abt_Etat_Capteur ";
XDY_Datagroup	pl_NomDG;
int 		vl_ValRet = XDC_OK;
T_BOOL		vl_EtatDg = FALSE;

     XZST_03EcritureTrace ( XZSTC_FONCTION, 
     "IN  : XZEA16_Abt_Etat_Capteur Abt:%d DGInit:%s Fonct:%ld NomSite:%s.",
     		va_Abonnement,
     		pa_DGInit,
     		pa_FonctUtilisateur,
     		pa_NomSite );
     ...
     pg_FonctUtil_XZEA_16 = pa_FonctUtilisateur;
     
     /*A Si demande d'abonnement */
     if ( va_Abonnement == XZEAC_DEBUT_ABONNEMENT )
     {
	/*A  Armement Callback sur Reception XDM_ETAT_CAPTEUR */
	if ( vg_CB_XZEA16 == NULL )
	{
	   if ( ( vg_CB_XZEA16 = TipcSrvProcessCbCreate (	XDM_IdentMsg(XDM_ETAT_CAPTEUR),
							      	XZEA_Etat_Capteur,
								(T_CB_ARG)NULL ) ) == NULL ) 
	   {	
	      /*B Ecriture Trace */
	      XZST_03EcritureTrace ( XZSTC_WARNING, "Echec armement Callback sur XDM_ETAT_CAPTEUR." );
	      vl_ValRet = XDC_NOK;
	   }
	   else
	   {	
	      /*B Ecriture Trace */
	      XZST_03EcritureTrace ( XZSTC_DEBUG1, "Armement Callback sur XDM_ETAT_CAPTEUR." );
	   }
	}
	
	

	/*A Abonnement au Datagroup XDG_ETAT_CAPTEUR<NomSite> */
	/*A Construction du nom du DataGroup */
	sprintf ( pl_NomDG, "%s_%s", XDG_ETAT_CAPTEUR, pa_NomSite );
	...
	/*A Abonnement au Datagroup DGInit passe en parametre */
     ...
     {
        if ( ( va_Abonnement == XZEAC_FERMER_ABONNEMENT ) && ( vg_CB_XZEA16 != NULL ) )
        {
           /*A  Desarmement Callback sur Reception XDM_ETAT_CAPTEUR */
	   if ( TutCbDestroy ( vg_CB_XZEA16 ) == FALSE )  
	   {	
	      /*B Ecriture Trace */
	      XZST_03EcritureTrace ( XZSTC_WARNING, "Echec desarmement Callback sur XDM_ETAT_CAPTEUR ." );
	      vl_ValRet = XDC_NOK;
	   }
	   else
	   {
	      XZST_03EcritureTrace ( XZSTC_DEBUG1, "Desarmement Callback sur XDM_ETAT_CAPTEUR ." );
	      vg_CB_XZEA16 = NULL;
	   }
	}

	/*A Desabonnement au Datagroup XDG_ETAT_CAPTEUR<NomSite> */
	/*A Construction du nom du DataGroup */
	sprintf ( pl_NomDG, "%s_%s", XDG_ETAT_CAPTEUR, pa_NomSite );
   ...
   XZST_03EcritureTrace ( XZSTC_FONCTION, "OUT : XZEA16_Abt_Etat_Capteur Retour:%d.", vl_ValRet );
