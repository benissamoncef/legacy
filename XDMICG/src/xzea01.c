/*E*/
/*  Fichier : @(#)xzea01.c	1.6      Release : 1.6        Date : 12/13/95
------------------------------------------------------
* GTIE *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME : EQUEXT
------------------------------------------------------
* MODULE xzea  * FICHIER xzea01.c
------------------------------------------------------
...
static char *version = "@(#)xzea01.c	1.6 12/13/95 : xzea01" ;
XDY_Fonction		pg_FonctUtil_XZEA_01 = NULL;
T_CB			vg_CB_XZEA01         = NULL;
...
------------------------------------------------------*/
{
   static char *version = "@(#)xzea01.c	1.6 12/13/95 : XZEA_Mes_6gl_Voie ";
   ...
   /* Recuperation des parametres du message XDM_EMES_6gl_Voie */
   ...
   pg_FonctUtil_XZEA_01 ( vl_Mes, &vl_Resultat );
   XZST_03EcritureTrace ( XZSTC_FONCTION,  "OUT : XZEA_Mes_6gl_Voie %d", vl_Resultat );
}
...
int	XZEA01_Abt_Mesures_6gl_Voie (	XDY_Booleen	va_Abonnement,
...
------------------------------------------------------*/
{
static char *version = "@(#)xzea01.c	1.6 12/13/95 : XZEA01_Abt_Mesures_6gl_Voie ";
XDY_Datagroup	pl_NomDG;
int 		vl_ValRet = XDC_OK;
T_BOOL		vl_EtatDg = FALSE;

     XZST_03EcritureTrace ( XZSTC_FONCTION, 
     "IN  : XZEA01_Abt_Mesures_6gl_Voie Abt:%d DGInit:%s Fonct:%ld NomSite:%s Numero:%d Sens:%d Voie:%d.",
     		va_Abonnement,
     		pa_DGInit,
     		pa_FonctUtilisateur,
     		pa_NomSite,
     ...
     pg_FonctUtil_XZEA_01 = pa_FonctUtilisateur;
     
     /*A Si demande d'abonnement */
     if ( va_Abonnement == XZEAC_DEBUT_ABONNEMENT )
     {
	/*A  Armement Callback sur Reception XDM_EMES_6gl_Voie */
	if ( vg_CB_XZEA01 == NULL )
	{
	   if ( ( vg_CB_XZEA01 = TipcSrvProcessCbCreate (	XDM_IdentMsg(XDM_EMES_6gl_Voie),
							      	XZEA_Mes_6gl_Voie,
								(T_CB_ARG)NULL ) ) == NULL ) 
	   {	
	      /*B Ecriture Trace */
	      XZST_03EcritureTrace ( XZSTC_WARNING, "Echec armement Callback sur XDM_EMES_6gl_Voie ." );
	      vl_ValRet = XDC_NOK;
	   }
	   else
	   {	
	      /*B Ecriture Trace */
	      XZST_03EcritureTrace ( XZSTC_DEBUG1, "Armement Callback sur XDM_EMES_6gl_Voie ." );
	   }
	}
	
	

	/*A Abonnement au Datagroup XDG_EMES_RADT_6GL_R<NumeroSensVoie> */
	/*A Construction du nom du DataGroup */
	sprintf ( pl_NomDG, "%s%d%d%d", XDG_EMES_RADT_6GL_R,va_Numero, va_Sens, va_Voie );
	...
	/*A Abonnement au Datagroup DGInit passe en parametre */
	   ...
	   /* Envoi du message de demande d'init XDM_EInit_IHM_6gl_Voie */
           /*A Construction du DatGroup XDG_EInit_IHM_RADT_<NomSite> */
           XDG_EncodeDG2 ( pl_NomDG, XDG_EInit_IHM_RADT, pa_NomSite );
   	
           /*A Envoi du message XDM_EInit_IHM_6gl_Voie en fournissant les valeur recues  */
   
   	   if (!TipcSrvMsgWrite ( 	pl_NomDG,
      			  		XDM_IdentMsg(XDM_EInit_IHM_6gl_Voie),
  			  		XDM_FLG_SRVMSGWRITE,
					...
					T_IPC_FT_INT2, va_Sens,
                          		NULL))
      	   {
  		/*A Trace envoi message XDM_EInit_IHM_6gl_Voie non effectue */    	
         	XZST_03EcritureTrace(XZSTC_WARNING, "Echec envoi message XDM_EInit_IHM_6gl_Voie.");
	   }
	   else
      	   {
  		/*A Trace envoi message XDM_EInit_IHM_6gl_Voie */    	
         	XZST_03EcritureTrace(XZSTC_DEBUG1, "Envoi du message XDM_EInit_IHM_6gl_Voie sur %s.", pl_NomDG );
	   }
     }
     else
     {
        if ( ( va_Abonnement == XZEAC_FERMER_ABONNEMENT ) && ( vg_CB_XZEA01 != NULL ) )
        {
           /*A  Desarmement Callback sur Reception XDM_EMES_6gl_Voie */
	   if ( TutCbDestroy ( vg_CB_XZEA01 ) == FALSE )  
	   {	
	      /*B Ecriture Trace */
	      XZST_03EcritureTrace ( XZSTC_WARNING, "Echec desarmement Callback sur XDM_EMES_6gl_Voie ." );
	      vl_ValRet = XDC_NOK;
	   }
	   else
	   {
	      XZST_03EcritureTrace ( XZSTC_DEBUG1, "Desarmement Callback sur XDM_EMES_6gl_Voie ." );
	      vg_CB_XZEA01 = NULL;
	   }
	}

	/*A Desabonnement au Datagroup XDG_EMES_RADT_6GL_R<NumeroSensVoie> */
	/*A Construction du nom du DataGroup */
	sprintf ( pl_NomDG, "%s%d%d%d", XDG_EMES_RADT_6GL_R, va_Numero, va_Sens, va_Voie );
   ...
   XZST_03EcritureTrace ( XZSTC_FONCTION, "OUT : XZEA01_Abt_Mesures_6gl_Voie Retour:%d.", vl_ValRet );
