/*E*/
/*Fichier : @(#)xzea14.c	1.8      Release : 1.8        Date : 12/13/95
------------------------------------------------------
* GTIE *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME : EQUEXT
------------------------------------------------------
* MODULE xzea  * FICHIER xzea14.c
------------------------------------------------------
...
* Mismer.D	version 1.5	02 Jan 1995	:
* Mismer.D	version 1.6	28 Jan 1995	:
* Fontaine.C	version 1.7	23 Mai 1995	: Reprise historique
* Mismer.D	version 1.8	29 Nov 1995	: Correction condition fermeture datagroup d'init
...
static char *version = "@(#)xzea14.c	1.8 12/13/95 : xzea14" ;
XDY_Fonction		pg_FonctUtil_XZEA_14 = NULL;
T_CB			vg_CB_XZEA14         = NULL;
...
------------------------------------------------------*/
{
   static char *version = "@(#)xzea14.c	1.8 12/13/95 : XZEA_Etat_RAU ";
...
XDY_Entier		vl_Numero, vl_NumPoste, vl_CodeCmd, vl_CrCmd,
   ...
   if ( !TipcMsgRead(pa_Data -> msg,
	       ...
	       NULL) )
   {
      XZST_03EcritureTrace ( XZSTC_WARNING," Impossible de lire le MSG XDM_ETAT_RAU dans XZEA_Etat_RAU ");
   }
   else
   {
      XZST_03EcritureTrace ( XZSTC_FONCTION, "IN  : XZEA_Etat_RAU  Numero:%d Num Poste:%d Code Cmd:%d CR Cmd:%d Com RAU:%d Com GN:%d",
       			     vl_Numero, 
       			     vl_NumPoste,
       			     vl_CodeCmd, 
       			     vl_CrCmd,
	  ...
	  if (XZSC_GetDestinataire(pa_Data, vl_machine)!=XDC_OK) {
		XZST_03EcritureTrace(XZSTC_WARNING,"XZEA_Etat_RAU : probleme dans recuperation machine destinataire du message");
	  }

      /*A Appel de la fonction utilsateur avec la structure XZEAT_Etat_PAU en argument */
      pg_FonctUtil_XZEA_14 ( vl_Etat, &vl_Resultat, vl_machine );
      XZST_03EcritureTrace ( XZSTC_FONCTION,  "OUT : XZEA_Etat_RAU %d", vl_Resultat );
   }
}
...
int	XZEA14_Abt_Etat_RAU (		XDY_Booleen	va_Abonnement,
...
*
* CONDITION D'UTILISATION
*   Ce service est demande pour s'abonner aux etats de tous le PAU d'un site
*
* FONCTION 
...
------------------------------------------------------*/
{
static char *version = "@(#)xzea14.c	1.8 12/13/95 : XZEA14_Abt_Etat_RAU ";
XDY_Datagroup	pl_NomDG;
int 		vl_ValRet = XDC_OK;
T_BOOL		vl_EtatDg = FALSE;

     XZST_03EcritureTrace ( XZSTC_FONCTION, 
     "IN  : XZEA14_Abt_Etat_RAU Abt:%d DGInit:%s Fonct:%ld NomSite:%s.",
     		va_Abonnement,
     		pa_DGInit,
     		pa_FonctUtilisateur,
     		pa_NomSite );
     ...
     pg_FonctUtil_XZEA_14 = pa_FonctUtilisateur;
     
     /*A Si demande d'abonnement */
     if ( va_Abonnement == XZEAC_DEBUT_ABONNEMENT )
     {
	/*A  Armement Callback sur Reception XDM_ETAT_RAU */
	if ( vg_CB_XZEA14 == NULL )
	{
	   if ( ( vg_CB_XZEA14 = TipcSrvProcessCbCreate (	XDM_IdentMsg(XDM_ETAT_RAU),
							      	XZEA_Etat_RAU,
								(T_CB_ARG)NULL ) ) == NULL ) 
	   {	
	      /*B Ecriture Trace */
	...
	/*A Abonnement au Datagroup DGInit passe en parametre */
     ...
     {
        if ( ( va_Abonnement == XZEAC_FERMER_ABONNEMENT ) && ( vg_CB_XZEA14 != NULL ) )
        {
           /*A  Desarmement Callback sur Reception XDM_ETAT_RAU */
	   if ( TutCbDestroy ( vg_CB_XZEA14 ) == FALSE )  
	   {	
	      /*B Ecriture Trace */
   ...
   XZST_03EcritureTrace ( XZSTC_FONCTION, "OUT : XZEA14_Abt_Etat_RAU Retour:%d.", vl_ValRet );
