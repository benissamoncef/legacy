/*E*/
/*Fichier :  $Id: xzea45.c,v 1.1 2008/02/13 13:24:59 devgtie Exp $      Release : $Revision: 1.1 $        Date : $Date: 2008/02/13 13:24:59 $
------------------------------------------------------
* GTIE *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME : EQUEXT
------------------------------------------------------
* MODULE xzea  * FICHIER xzea45.c
------------------------------------------------------
...
static char     *version             = "@(#)xzea45.c    1.1 09/21/07 : xzea45" ;
XDY_Fonction    pg_FonctUtil_XZEA_45 = NULL;
T_CB            vg_CB_XZEA45         = NULL;
...
*/
 ...
 void    XZEA_Etat_EqtENR ( T_IPC_CONN                  va_Cnx,
...
------------------------------------------------------*/
{
    static char       *version    = "@(#)xzea45.c    1.1 09/21/07 : XZEA_Etat_EqtENR ";
    ...
    if ( !TipcMsgRead( pa_Data -> msg,
                       ...
                       NULL) ) 
    {
        XZST_03EcritureTrace ( XZSTC_WARNING," Impossible de lire le MSG XDM_ETAT_EQUIPEMENT dans XZEA_Etat_EqtENR ");
    }
    else
    {
        XZST_03EcritureTrace ( XZSTC_FONCTION, 
                               "IN  : XZEA_Etat_EqtENR Domaine:%d TypeEqt:%d NumTube:%d TypeEquipement:%d NumEquipement:%d EtatEquipement:%d EtatTechnique:%d Mode:%d EtatDisc:%d ModeRafale:%d.",
							   vl_Domaine,
                               vl_TypeEqt,
                               vl_NumEqt,
                               vl_TypeEquipement,
      ...
      if (XZSC_GetDestinataire(pa_Data, vl_machine)!=XDC_OK) {
	    XZST_03EcritureTrace(XZSTC_WARNING,"XZEA_Etat_EqtENR : probleme dans recuperation machine destinataire du message");
	  }

      /*A Appel de la fonction utilsateur avec la structure XZEAT_Equipement en argument */
      pg_FonctUtil_XZEA_45 ( vl_Etat, &vl_Resultat, vl_machine );
      XZST_03EcritureTrace ( XZSTC_FONCTION,  "OUT : XZEA_Etat_EqtENR %d", vl_Resultat );
   }
}
...
int    XZEA45_Abt_Etat_EqtENR ( XDY_Booleen    va_Abonnement,
...
------------------------------------------------------*/
{
static char      *version  = "@(#)xzea45.c    1.1 09/21/07 : XZEA45_Abt_Etat_EqtENR ";
...
XDY_Entier       vl_NumEqt  = va_NumEqt;
    ...
    XZST_03EcritureTrace ( XZSTC_FONCTION, 
                           "IN  : XZEA45_Abt_Etat_EqtENR Abt:%d DGInit:%s Fonct:%ld NomSite:%s TypeEqt:%d NumEqt:%d.",
                           va_Abonnement,
                           pa_DGInit,
                           pa_FonctUtilisateur,
                           pa_NomSite,
    ...
    pg_FonctUtil_XZEA_45 = pa_FonctUtilisateur;

    /*A Si demande d'abonnement */
    if ( va_Abonnement == XZEAC_DEBUT_ABONNEMENT )
    {
        /*A  Armement Callback sur Reception XDM_ETAT_EQUIPEMENT */
        if ( vg_CB_XZEA45 == NULL )
        {
            if ( ( vg_CB_XZEA45 = TipcSrvProcessCbCreate ( XDM_IdentMsg(XDM_ETAT_EQUIPEMENT),
                                                           XZEA_Etat_EqtENR,
                                                           (T_CB_ARG)NULL ) ) == NULL ) 
            {    
                /*B Ecriture Trace */
    ...
    {
        if ( ( va_Abonnement == XZEAC_FERMER_ABONNEMENT ) && ( vg_CB_XZEA45 != NULL ) )
        {
            /*A  Desarmement Callback sur Reception XDM_ETAT_EQUIPEMENT */
            if ( TutCbDestroy ( vg_CB_XZEA45 ) == FALSE )  
            {    
                /*B Ecriture Trace */
    ...
    XZST_03EcritureTrace ( XZSTC_FONCTION, "OUT : XZEA45_Abt_Etat_EqtENR Retour:%d.", vl_ValRet );
