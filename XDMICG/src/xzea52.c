/*E*/
/*Fichier :  $Id: xzea52.c,v 1.1 2008/02/13 13:25:00 devgtie Exp $      Release : $Revision: 1.1 $        Date : $Date: 2008/02/13 13:25:00 $
------------------------------------------------------
* GTIE *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME : EQUEXT
------------------------------------------------------
* MODULE xzea  * FICHIER xzea52.c
------------------------------------------------------
...
static char   *version             = "@(#)xzea52.c    1.1 09/21/07 : xzea52" ;
XDY_Fonction  pg_FonctUtil_XZEA_52 = NULL;
T_CB          vg_CB_XZEA52         = NULL;
...
*/
 ...
 void   XZEA_Etat_Sequence_VEN ( T_IPC_CONN                  va_Cnx,
...
------------------------------------------------------*/
{
    static char       *version    = "@(#)xzea52.c    1.1 09/21/07 : XZEA_Etat_Sequence_VEN ";
    ...
    /* Recuperation des parametres du message XDM_ETAT_SEQVEN */
    if ( !TipcMsgRead( pa_Data -> msg,
                       ...
                       NULL) ) 
    {
        XZST_03EcritureTrace ( XZSTC_WARNING," Impossible de lire le MSG XDM_ETAT_SEQVEN dans XZEA_Etat_Sequence_VEN ");
    }
    else
    {           
		XZST_03EcritureTrace ( XZSTC_FONCTION, 
							   "IN  : XZEA_Etat_Sequence_VEN TypeEqt:%d NumEqt:%d SeqCourant:%d SeqChange:%d EtatSeq:%d.",
							   vl_TypeEqt,
							   vl_NumEqt,
							   vl_SeqCourante,
							   vl_SeqChange,
        ...
        if (XZSC_GetDestinataire(pa_Data, vl_machine)!=XDC_OK) {
	        XZST_03EcritureTrace(XZSTC_WARNING,"XZEA_Etat_Sequence_VEN : probleme dans recuperation machine destinataire du message");
	  }

		/*A Appel de la fonction utilsateur avec la structure XZEAT_Sequence en argument */
		pg_FonctUtil_XZEA_52 ( vl_Etat, &vl_Resultat, vl_machine );
		XZST_03EcritureTrace ( XZSTC_FONCTION,  "OUT : XZEA_Etat_Sequence_VEN %d", vl_Resultat );
    }
}
...
int    XZEA52_Abt_Etat_Sequence_VEN ( XDY_Booleen    va_Abonnement,
...
*
* CONDITION D'UTILISATION
*   Ce service est demande pour s'abonner aux etats des sequences eqt ventilation de la GTC Nice
*
* FONCTION 
...
------------------------------------------------------*/
{
static char     *version  = "@(#)xzea52.c    1.1 09/21/07 : XZEA52_Abt_Etat_Sequence_VEN ";
...
XDY_Entier       vl_NumEqt  = va_NumEqt;
    ...
    XZST_03EcritureTrace ( XZSTC_FONCTION, 
                           "IN  : XZEA52_Abt_Etat_Sequence_VEN Abt:%d DGInit:%s Fonct:%ld NumTube:%d NomSite:%s.",
                           va_Abonnement,
                           pa_DGInit,
                           pa_FonctUtilisateur,
                           va_NumEqt,
    ...
    pg_FonctUtil_XZEA_52 = pa_FonctUtilisateur;

    /*A Si demande d'abonnement */
    if ( va_Abonnement == XZEAC_DEBUT_ABONNEMENT )
    {
        /*A  Armement Callback sur Reception XDM_ETAT_SEQVEN */
        if ( vg_CB_XZEA52 == NULL )
        {
            if ( ( vg_CB_XZEA52 = TipcSrvProcessCbCreate ( XDM_IdentMsg(XDM_ETAT_SEQVEN),
                                                           XZEA_Etat_Sequence_VEN,
                                                           (T_CB_ARG)NULL ) ) == NULL ) 
            {
                /*B Ecriture Trace */
                XZST_03EcritureTrace ( XZSTC_WARNING, "Echec armement Callback sur XDM_ETAT_SEQVEN." );
                vl_ValRet = XDC_NOK;
            }
            else
            {    
                /*B Ecriture Trace */
                XZST_03EcritureTrace ( XZSTC_DEBUG1, "Armement Callback sur XDM_ETAT_SEQVEN." );
            }
        }

        /*A Abonnement au Datagroup XDG_ETAT_SEQ_VEN<NomSite> */
        /*A Construction du nom du DataGroup */
        sprintf ( pl_NomDG, "%s_%s", XDG_ETAT_SEQ_VEN, pa_NomSite );
    ...
    {
        if ( ( va_Abonnement == XZEAC_FERMER_ABONNEMENT ) && ( vg_CB_XZEA52 != NULL ) )
        {
            /*A  Desarmement Callback sur Reception XDM_ETAT_SEQVEN */
            if ( TutCbDestroy ( vg_CB_XZEA52 ) == FALSE )  
            {    
                /*B Ecriture Trace */
                XZST_03EcritureTrace ( XZSTC_WARNING, "Echec desarmement Callback sur XDM_ETAT_SEQVEN ." );
                vl_ValRet = XDC_NOK;
            }
            else
            {
                XZST_03EcritureTrace ( XZSTC_DEBUG1, "Desarmement Callback sur XDM_ETAT_SEQVEN ." );
                vg_CB_XZEA52 = NULL;
            }
        }

        /*A Desabonnement au Datagroup XDG_ETAT_SEQ_VEN<NomSite> */
        /*A Construction du nom du DataGroup */
        sprintf ( pl_NomDG, "%s_%s", XDG_ETAT_SEQ_VEN, pa_NomSite );
    ...
    XZST_03EcritureTrace ( XZSTC_FONCTION, "OUT : XZEA52_Abt_Etat_Sequence_VEN Retour:%d.", vl_ValRet );
