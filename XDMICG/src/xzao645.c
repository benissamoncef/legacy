/*E*/
/*Fichier :  $Id: xzao645.c,v 1.2 2010/12/06 19:18:05 gesconf Exp $      Release : $Revision: 1.2 $        Date : $Date: 2010/12/06 19:18:05 $
------------------------------------------------------
* GTIE *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME  BASEDD
------------------------------------------------------
* MODULE XZAO * FICHIER xzao645.c
------------------------------------------------------
...
static char *version = "@(#)xzao645.c    1.1 09/26/07 : xzao645" ;
...
int xzao645_Lire_Config_EXP ( FILE            *pa_Fd,
...
*       XZAOT_ConfEXP     contenu de ligne
*       XDY_Entier        NumLigne
...
{
    static char *version = "@(#)xzao645.c    1.1 09/26/07 : xzao645_Lire_Config_EXP" ;

    XZAOT_ConfEXP    vl_EXP;
    char             vl_Lig[255];
    char             *vl_Texte;
    ...
    */
    ...
    if (pa_Fd == NULL)
    {
        XZST_03EcritureTrace(XZSTC_MESSAGE,"xzao645: fichier non existant");
        return ( XDC_NOK );
    }
            ...
            */
            ...
            if (vl_retcode != XDC_OK)
            {
                ...
                */
                ...
                {  
                    XZST_03EcritureTrace(XZSTC_WARNING,"xzao645 : Saisie  non valide");
                    XZST_03EcritureTrace(XZSTC_WARNING,"xzao645 : retourne %d",XDC_NOK);
                    return ( XDC_NOK );
                }

                strcpy ( vl_EXP.NomEXP, vl_Texte_out );
                ...
                strcpy ( vl_EXP.Autoroute, vl_Texte_out );
                ...
                strcpy ( vl_EXP.NomServeur, vl_Texte_out );
                ...
                vl_EXP.AdresseEtat = atoi ( vl_Texte_out );
                ...
                vl_EXP.AdresseCommande = atoi ( vl_Texte_out );
                ...
                vl_EXP.AdresseDiscordance = atoi ( vl_Texte_out );
                ...
                vl_EXP.AdresseTelealarme = atoi ( vl_Texte_out );
                ...
                */
                ...
                if (pa_FonctionUtil != NULL)
                {
                    ...
                    */
                    if ( (vl_retcode = ( *pa_FonctionUtil ) ( vl_EXP, vl_num_ligne ) ) != XDC_OK )
                    {    
                ...
                else
                {
                    ...
                    */
                    XZST_03EcritureTrace(XZSTC_MESSAGE,"xzao645: echec de l'appel de la fonction utilisateur");
                    return ( XDC_NOK );
                }
            }
        }
...
/* Fin de la fonction xzao645_Lire_Config_EXP */
