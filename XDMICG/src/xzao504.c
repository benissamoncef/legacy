/*E*/
/* Fichier : $Id: xzao504.c,v 1.1 2021/06/03 07:28:54 pc2dpdy Exp $        $Revision: 1.1 $        $Date: 2021/06/03 07:28:54 $
------------------------------------------------------
* GTIE *  PROJET MIGRAZUR 
------------------------------------------------------
* SOUS-SYSTEME  BASEDD
------------------------------------------------------
* MODULE XZAO * FICHIER xzao504.c
------------------------------------------------------
...
* HISTORIQUE :
*
* ABE	31/03/21	: Creation 1.1 DEM-SAE244
------------------------------------------------------*/
...
static char *version = "@(#)xzao504.c	1.1 1.1 28/07/20" ;
...
int xzao504_Lire_Config_Zon_IMU ( 	FILE  		*pa_Fd,
...
{
   static char *version = "@(#)xzao504.c	1.1 02/10/20 : xzao504_Lire_Config_Zon_IMU" ;
   ...
   */
   ...
   if (pa_Fd == NULL)
   {
     XZST_03EcritureTrace(XZSTC_MESSAGE,"xzao504: fichier non existant");
     return ( XDC_NOK );
   }
   ...
   while ( ( vl_retcode = XZAO000_LireLigneFich ( pa_Fd, vl_Lig, sizeof(vl_Lig), &vl_Resultat_out ) ) != XDC_NOK )
   {
       ...
       */
       ...
       { 
          ...
          */
...

          {
             ...
             */
             ...
             {  
                XZST_03EcritureTrace(XZSTC_WARNING,"xzao504 : Saisie Numero Eqt  non valide");
                XZST_03EcritureTrace(XZSTC_WARNING,"xzao504 : retourne %d",XDC_NOK);
                return ( XDC_NOK );
             }
	     ...
	     if ( ( vl_retcode = XZAO000_Verif_Coherence_Nombre ( vl_Texte, XZAO_LONG_INT, vl_Texte_out ) ) != XDC_OK )
	     {
	        XZST_03EcritureTrace(XZSTC_WARNING,"xzao504 : Saisie Id non valide");
		XZST_03EcritureTrace(XZSTC_WARNING,"xzao504 : retourne %d",XDC_NOK);
		return ( XDC_NOK );
	     }
	     ...
	     vl_Texte = strtok ( NULL, XZAO_TAB_SEPAR );
             ...
             {
	        XZST_03EcritureTrace(XZSTC_WARNING,"xzao504 : Saisie Libelle  non valide");
		XZST_03EcritureTrace(XZSTC_WARNING,"xzao504 : retourne %d",XDC_NOK);
		return ( XDC_NOK );
	     }
	     strcpy ( vl_Zone_IMU.Libelle, vl_Texte_out );
	     XZST_03EcritureTrace(XZSTC_WARNING,"xzao504 : %s",vl_Zone_IMU.Libelle);
             ...
             */
             ...
             if (pa_FonctionUtil != NULL)
             {
                ...
                */
                ...
                {    
                   XZST_03EcritureTrace(XZSTC_WARNING,"xzao504: appel de FonctionUtilisateur s'est mal deroule");
                   return ( XDC_NOK );
                }  
	     } 
             else
             {
                ...
                */
                ...
                XZST_03EcritureTrace(XZSTC_MESSAGE,"xzao504: echec de l'appel de la fonction utilisateur");
                return ( XDC_NOK );
             }
          }
       }
...
/* Fin de la fonction xzao504_Lire_Config_Zon_IMU */
